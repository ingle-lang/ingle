// selfhost/checker.ig — the Ingle type checker, written in Ingle (Stage 3 / M3 of the self-hosting
...
// ones), then exact diagnostic parity against `inglec --emit=bytecode` over the corpus.
//
// Mirrors stage-0's design: SemType is a plain int with million-strided bands (so type equality is `==`,
// which suits Ingle — no enum `==`); a multi-pass pipeline registers all type/function NAMES, then
...
// SemType is a plain int (stage-0 check.c): equality is `==`. Kept POSITIVE (Ingle top-level constants
...
fn builtin_names() -> [string] {
    return ["print", "println", "concat", "hash", "abs", "ceil", "floor", "round", "sqrt", "pow",
        "parse_float", "char_code", "from_char_code", "byte_slice", "from_bytes", "float_bits", "read_line", "read_file", "write_file", "list_dir",
...
// clone_bools returns an independent copy of a bool array (a moved-state snapshot — Ingle arrays alias on

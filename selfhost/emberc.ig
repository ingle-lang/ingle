...
fn main() -> int {
    let argv = args()
    if argv.len() < 1 {
        println("usage: inglec-self <file.ig>")
        exit(1)
    }
    let entry = argv[0]
    ...
    if ck.check(read_file(entry)) {
        ...
        // 2. CODEGEN every (merged) module. With an output path, write a RUNNABLE `.igb` container (the
    ...
    // `exit` sets the real process code AND suppresses the runtime's `=> N` echo, so inglec-self's output

// selfhost/parser.ig — the Ingle parser, written in Ingle (Stage 2 / M2 of the self-hosting bootstrap,
// docs/design/self-hosting.md). It consumes the self-hosted lexer's `[Token]` and builds an AST whose
// `--emit=ast`-format dump (src/ast_print.c) is diffed byte-for-byte against `inglec --emit=ast` over the
// corpus (tests/run-selfhost.sh). The dump driver is selfhost/parse_dump.ig.
//

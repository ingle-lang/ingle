// std/draw — immediate-mode drawing over Ingle's native graphics backend (MANIFESTO
// §5g). The UI is a pure function of state, drawn fresh every frame: there is no
// retained widget tree, so app state is just your own Ingle values and ownership

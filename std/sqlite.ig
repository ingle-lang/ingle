// std/sqlite — embedded SQL over Ingle's FFI, backed by the vendored SQLite amalgamation. SQLite is
// the one database that fits Ingle's empty-dependency-tree rule: a single public-domain C file, no
...
// own the raw handles, and fold the raw result codes into Ingle's Result/bool error model.
extern "c" {

// kernel/hello.ig — Ingle on bare metal (kernel milestones 1–2; docs/design/kernel-freestanding.md).
//
// A freestanding Ingle program that boots on QEMU `aarch64 virt` with NO OS and NO libc. Milestone 1
// proved the scalar subset (heap-free, output via a direct `extern "c"` to the UART); milestone 2
// links the REAL Ingle runtime compiled `-DEMBER_FREESTANDING` over a bump allocator, so HEAP-BACKED
...
fn main() -> int {
    ...
    println("Hello from Ingle — running on bare metal with a heap!")
    println("sum of [10, 20, 12] = {sum}")

    // The freestanding entry surfaces this as the exit code; the boot stub forwards it via
    // semihosting, so QEMU exits 42 — a value computed by Ingle, with aggregates, on bare metal.
    return sum
}

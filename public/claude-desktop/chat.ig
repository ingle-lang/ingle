// chat.ig — a CLI that talks to Claude over HTTPS (Phase 1 of the Claude Desktop app).
//
// Ingle has no networking; it borrows libcurl through the FFI, wrapped as the reusable `std/http`
...
// Ingle. JSON braces are written `\{` `\}` because `{...}` is string interpolation in Ingle.
//
//   make net
//   export ANTHROPIC_API_KEY=sk-ant-...
//   build/inglec-net --emit=run public/claude-desktop/chat.ig "your message"
...
// `\{`/`\}`/`\"` are literal JSON; `{model}`/`{max_tokens}`/`{esc}` are Ingle interpolation holes.
...
fn main() -> int {
    ...
    let a = args()
    var msg = "Say hello from the Ingle programming language in one short sentence."
    if a.len() > 0 {
        msg = a[0]
    }
    println("> {msg}")

// flare_chat.ig — the Claude desktop app on std/flare (the declarative widget kit), the Flare-native
// sibling of the raw-intrinsics gui.ig. It dogfoods Flare — sidebar, scrollable transcript, wrapped
...
//   # or: make net-graphics && ANTHROPIC_API_KEY=… build/inglec-net-gfx --emit=run public/claude-desktop/flare_chat.ig
//
// It needs inglec-NET-gfx (libcurl for the std/http transport + the parallel runtime for the worker fiber/channels),
// like gui.ig. Without ANTHROPIC_API_KEY it still runs; sending just prints a reminder.
//
...
// run_read_file executes the read_file tool: parse the path, read it, cap the size. read_file is an Ingle
...
// insert_int returns `arr` with `v` inserted before index `idx` (idx >= len → appended). Ingle arrays have
...
fn store_path() -> string {
    ...
    return env("HOME") + "/.ingle-claude-history.json"
}
...
fn main() -> int {
    ...
    loop {
        ...
        if f.menu("Help") {
            if f.menu_item("About Claude (Ingle)") {
                f.toast("A Claude desktop app written in Ingle + Flare")
            }
            f.menu_end()
        }
        f.menubar_end()

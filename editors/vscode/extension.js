...
// (Only the internal VS Code language id stays `ember` — like the C-side EmberRt, a non-user-facing
// identifier tied to the grammar scope `source.ember`. Everything the user sees is Ingle: the display
// name, the LSP channel "Ingle Language Server", the binary inglec, `.ig`, and the ingleLsp.* settings.)
...
function activate(context) {
  const cfg = workspace.getConfiguration("ingleLsp");
  // Defaults follow `make install` (PREFIX defaults to ~/.ingle), resolved against the user's home
  // dir so the extension is portable across machines. Override via the ingleLsp.* settings.
  ...
  client = new LanguageClient("ingleLsp", "Ingle Language Server", serverOptions, clientOptions);
  client.start();

...
# under real parallelism) and runs each under build/inglec-mn with a WATCHDOG: a hang (timeout) is a
...
# run <name> <expected-stdout-substring> <expected-exit> <file.ig>
...
EOF
run_case "headline-8000-fibers" "total=8000" 0 "$WD/many.ig"
...
run_case "fan-in-out-compute" "total=127992000" 0 "$WD/fanio.ig"

# ---- deep nested nurseries (divide & conquer): each level spawns two children ----
cat > "$WD/nested.ig" <<'EOF'
...
EOF
run_case "deadlock-detected" "deadlock: every task in the nursery is blocked" 65 "$WD/deadlock.ig"

# ---- structured cancellation: one task errors → its siblings are cancelled, error reported, NO hang ----
cat > "$WD/cancel.ig" <<'EOF'
...
EOF
run_case "cancel-on-error" "integer overflow" 65 "$WD/cancel.ig"
...
run_case "pipeline-with-close" "total=124750" 0 "$WD/pipe.ig"
...
EOF
run_case "main-parks-at-join" "got=3000000" 0 "$WD/join_park.ig"

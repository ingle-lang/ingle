#!/bin/sh
# ledger.sh — the DRIVER half of Ledger, Ingle's resource-LINEARITY fuzzer (OFI-049).
#
...
#
#   EXPECT accept  → inglec must type-check it AND the native (C) backend must compile it. A rejection
...
#   EXPECT reject  → inglec must reject it with a LINEARITY diagnostic (a leak / borrowed-Ptr / discard
...
(cd "$ROOT" && make >/dev/null 2>&1) || { say "inglec build failed"; exit 1; }
...
while [ "$i" -le "$COUNT" ]; do
    ...
    if [ "$want" = "accept" ]; then
        if [ "$has_err" = 1 ]; then
            ...
            cp "$prog" "$FINDS/overstrict_$i.ig"
            say "── [OVER-STRICT] seed=$i  inglec rejected a balanced program → ${FINDS#$ROOT/}/overstrict_$i.ig"
            ...
            if "$EMB" -o "$TMP/bin" "$prog" >/dev/null 2>&1; then ok=$((ok + 1)); else
                overstrict=$((overstrict + 1))
                cp "$prog" "$FINDS/native_$i.ig"
                say "── [NATIVE-FAIL] seed=$i  VM accepted but native backend rejected → ${FINDS#$ROOT/}/native_$i.ig"
            fi
        fi
    else   # want = reject
        if [ "$has_err" = 0 ]; then
            unsound=$((unsound + 1))
            cp "$prog" "$FINDS/unsound_$i.ig"
            say "── [UNSOUND]   seed=$i  a LEAK compiled with no error → ${FINDS#$ROOT/}/unsound_$i.ig"
        elif printf '%s' "$out" | grep -qE "$LEAK_RE"; then
            ok=$((ok + 1))
        else
            overreach=$((overreach + 1))
            cp "$prog" "$FINDS/overreach_$i.ig"

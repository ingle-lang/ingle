...
ALL="build parallel test selfhost bootstrap opcheck ceilings ledger crucible"
case "${1:-}" in
    "")     GATES="$ALL" ;;
    fast)   GATES="build parallel test selfhost bootstrap opcheck ceilings ledger" ;;
    *)      GATES="$*" ;;
esac
...
for g in $GATES; do
    case "$g" in
        ...
        bootstrap) run_gate bootstrap make bootstrap ;;

...
// with no heap allocation. Scalars are all that Phase 1 needs; the recursive struct/enum
// value walker (so an Err payload renders as Err("io") not <obj>) is a Phase 2 follow-up.
typedef struct {
    const char *name;        // role label ("index", "len", "divisor"); borrowed/static
    char        rendered[64]; // the value, already stringified (a long Err payload truncates — OFI-111)
} FaultValue;

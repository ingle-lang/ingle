---
title: The Ingle Book
nav_exclude: true
noindex: true
sitemap: false
description: Ingle by Firelight — a friendly, honest field guide to writing Ingle as it stands today. Every example was compiled and run before it was written down.
---
...
> [Chapter 23: The "Not Yet" List](#chapter-23--the-not-yet-list), clearly fenced off, and
> nowhere else.
>
...
[Chapter 21: The Whole Toolbox](#chapter-21--the-whole-toolbox). It can also compile your program
to a **standalone native binary**, with no interpreter anywhere in sight — that's
[Chapter 22: Compiling to Native](#chapter-22--compiling-to-native). For now, `--emit=run` is all
you need.
...
| `make parallel` | `build/inglec-par` | Same language, multicore runtime: `spawn`/`nursery`/channels run on real OS threads ([Chapter 14](#chapter-14--concurrency)). |
| `make mn` | `build/inglec-mn` | The **M:N** green-thread scheduler — many fibers multiplexed onto a few OS threads, with structured cancellation-on-failure. Opt-in while it clears a wider soak ([Chapter 14](#chapter-14--concurrency)). |
...
| `make db` | `build/inglec-db` | Links the vendored SQLite amalgamation so a program can `import "std/sqlite"` ([Chapter 15](#chapter-15--modules-and-the-standard-library)). The one C file lives in-tree, so this still needs no system package. |
...
- [Chapter 6 — Arrays and Iteration](#chapter-6--arrays-and-iteration)
- [Chapter 7 — Structs and Methods](#chapter-7--structs-and-methods)
- [Chapter 8 — Enums and Pattern Matching](#chapter-8--enums-and-pattern-matching)

**Part III — Bigger Ideas**
- [Chapter 9 — Errors and Optionals](#chapter-9--errors-and-optionals)
- [Chapter 10 — Generics and Interfaces](#chapter-10--generics-and-interfaces)
- [Chapter 11 — Functions as Values, and Closures](#chapter-11--functions-as-values-and-closures)
- [Chapter 12 — Ownership Without Tears](#chapter-12--ownership-without-tears)
- [Chapter 13 — Memory, the Quiet Way](#chapter-13--memory-the-quiet-way)
- [Chapter 14 — Concurrency](#chapter-14--concurrency)
- [Chapter 15 — Modules and the Standard Library](#chapter-15--modules-and-the-standard-library)
- [Chapter 16 — Talking to C](#chapter-16--talking-to-c)

**Part IV — Knowing You're Right**
- [Chapter 17 — Contracts](#chapter-17--contracts)
- [Chapter 18 — The Verification Loop](#chapter-18--the-verification-loop)
- [Chapter 19 — The Tape, and Errors as Data](#chapter-19--the-tape-and-errors-as-data)
- [Chapter 20 — Crucible: The Memory Fuzzer](#chapter-20--crucible-the-memory-fuzzer)

**Part V — Reference**
- [Chapter 21 — The Whole Toolbox](#chapter-21--the-whole-toolbox)
- [Chapter 22 — Compiling to Native](#chapter-22--compiling-to-native)
- [Chapter 23 — The "Not Yet" List](#chapter-23--the-not-yet-list)
- [Chapter 24 — One-Page Cheat Sheet](#chapter-24--one-page-cheat-sheet)

**Part VI — Things People See**
- [Chapter 25 — Flare: Interfaces You Can Read](#chapter-25--flare-interfaces-you-can-read)
...
Here is a complete, working Ingle program. Not a fragment, not "imagine the rest" — the whole
thing.
...
> breaks — and Ingle does essentially the same trick, emitting an invisible terminator only
...
A binding gives a name to a value. Ingle has two keywords for it, and the entire difference
...
For the common "update in place," the **compound assignment** operators are shorthand: `count += 1`
is exactly `count = count + 1`. The set is `+=`, `-=`, `*=`, `/=`, `%=`, and the bitwise `&=`, `|=`,
`^=`.
...
This is a theme you'll see again and again: **Ingle's error messages are written to be
...
> so default that we forgot it was a choice. Ingle is quietly siding with the old guard: you
...
Three kinds of value will carry most of your programs: numbers, strings, and booleans. Ingle
...
Integers can be written in **other bases**, and long numbers grouped with underscores for
legibility — the `_`s are purely visual and change nothing:
...
fn main() -> int {
    ...
    let bin = 0b1010        // 10
    let oct = 0o17          // 15
    let million = 1_000_000 // underscores just group digits
    println(hex + bin + oct)
    println(million)
    return 0
}
...
(A leading zero is *not* octal — `010` is ten, not eight; Ingle sidesteps that old C footgun. Write
`0o` when you mean octal.)
...
```
error[integer_overflow]: integer overflow
```

This is a deliberate safety choice. A silent wrap is one of the great sources of
catastrophic, hard-to-find bugs; Ingle would rather halt and tell you. (Floating-point follows
...
- `s.parse_int()` — try to read the string as an integer; gives back `Some(n)` or `None` (see
  [Chapter 9](#chapter-9--errors-and-optionals)).
...
[Chapter 15](#chapter-15--modules-and-the-standard-library). The four above are baked into the
language itself.
...
> like PCG — *depend* on integer arithmetic wrapping round at the type's width. So Ingle gives you
...
- **Arguments are checked for count and type**, with no coercion. Call `add(fib(10), 1)` and
  Ingle confirms you passed exactly two `int`s.
- **Order doesn't matter.** `fib` can call itself, and any function can call any other
  function defined anywhere in the file. Ingle reads all the signatures first, then checks the
...
### Calling a function method-style

For a value that has no methods of its own — an **enum** or a **scalar** (`int`, `bool`, `float`)
— you may write a call the other way round, putting the first argument in front with a dot.
`name(c)` and `c.name()` are the same call:
...
fn main() -> int {
    println(Green.name())   // same as name(Green)
    println(7.triple())     // same as triple(7)
    return 0
}
```
...
It's a reading convenience — no new function, no method — and it's what lets the `Option`/`Result`
combinators in [Chapter 9](#chapter-9--errors-and-optionals) read so naturally (`o.unwrap_or(0)`).
The rule is deliberately narrow: a **struct**, a **string**, and an **array** already have their
own methods, so `x.f()` on those looks up *their* methods only — it won't reach for a free function.
...
> an entire note in 1982 arguing for exactly this convention. Ingle simply makes it the only
...
parameter (full details in [Chapter 12](#chapter-12--ownership-without-tears)). Through a `let`
...
([Chapter 10](#chapter-10--generics-and-interfaces)); and — this one's new, and worth a section of
...
> type has the methods, it qualifies." *Nominal* typing — Ingle's choice — says "it qualifies only
...
fn main() -> int {
    ...
    let mail: Email = Email("ada@ingle.dev")
    println("user {id} <{mail}>")
    return 0
}
```
```
user 42 <ada@ingle.dev>
=> 0
```
...
`match` handles more than one-variant-per-arm. Four extras cover almost everything you'll reach for:

- **Literal patterns** — match a concrete `int`, `string`, or `bool`: `case 0`, `case "quit"`,
  `case true`.
- **Or-patterns** — one arm for several possibilities with `|`: `case 1 | 2 | 3`.
- **Guards** — an extra condition with `if`: `case n if n < 0`. A guarded arm fires only when the
  condition holds, so on its own it doesn't count toward exhaustiveness.
- **One-level nesting** — destructure a variant's payload in place: `case Some(Point(x, y))` binds the
  point's fields directly, and `case Some(Ok(v))` reaches through two enum layers at once.
...
fn corner(o: Option<Point>) -> int {
    match o {
        case Some(Point(x, y)) { return x + y }   // reach right into the payload
        case None              { return 0 }
    }
}
...
fn main() -> int {
    ...
    println(corner(Some(Point { x: 3, y: 4 })))
    println(corner(None))
    return 0
}
```
...
Exhaustiveness still holds throughout: matching on an `int` or `string` needs a `case _` (there are
too many values to list), a `bool` is covered by `true` + `false`, and nesting is one level deep — for
anything deeper, `match` again inside the arm.
...
> have them now. Ingle was never going to *not* have them. They were the first thing on the
> list.
...
written the other way round ([Chapter 4](#chapter-4--functions)) — they read as if they were
...
— Ingle infers them from what you pass and what you expect back:
...
> function. For a concrete type you'd hit the same rule; for a generic `T`, which Ingle treats
...
[Chapter 15](#chapter-15--modules-and-the-standard-library).) What's *not* here yet: bounds on a
...
[Chapter 23](#chapter-23--the-not-yet-list).

> **Static versus dynamic, in one breath.** A bound (`<T: Ord>`) and an interface-as-a-value
> ([Chapter 7](#chapter-7--structs-and-methods)) are the two halves of polymorphism, and Ingle
...
> Ingle compiles a generic *once* — `Box<int>` and `Box<string>` share one compiled body. This
...
> c` a comparison or a generic? Ingle resolves it with a lookahead rule: it reads a generic only
...
> But a bare, unannotated `let f = |x| x + 1` is an **error** — Ingle can't guess what `x` is.
...
`std/list` (more on imports in [Chapter 15](#chapter-15--modules-and-the-standard-library)):
...
annotations — Ingle infers all of that. The *safe* direction (borrowing) is the one you get for
...
call and dangle. Ingle catches that too:
...
with `let` can be read through two names at once — Ingle duplicates it for you, because it's
...
> **Fireside trivia.** Earlier in Ingle's development, a generic function that aliased a struct
...
Here's a question you may not have thought to ask, because Ingle has carefully arranged for you
...
one thing naive reference-counting can't collect; Ingle's value model makes them impossible to
build in the first place. For the programs Ingle's ownership model allows, there is nothing a
tracing garbage collector would reclaim that this misses.
...
owner, and Ingle gives you two blessed tools rather than leaving you to alias by hand.
...
> categories of security exploit are built on it. Ingle's one set of rules — single ownership,
...
Doing several things at once is where sharp edges tend to appear. Ingle takes a deliberately
...
fn main() -> int {
    let name = str.trim("  ingle  ")
    println("Hello, " + str.to_upper(name) + "!")   // Hello, INGLE!
...
code, and `http.close(h)` frees it. The handle is a linear `Ptr` ([Chapter 16](#chapter-16--talking-to-c)),
...
[Chapter 25](#chapter-25--flare-interfaces-you-can-read).
...
library. Ingle reaches across to C through an **`extern "c"` block**, where you declare the
foreign function using ordinary Ingle syntax:
...
Ingle; the signature you write *is* the contract the compiler type-checks against. That's the
one place Ingle's guarantees stop and you're vouching for the C side — so if you declare a
...
There's no marshalling library involved: Ingle flattens the struct to its scalar fields, and a
...
call**: Ingle lends C a pointer, C uses it while the call runs, and Ingle keeps ownership and frees
nothing C owns.
...
to C (`fread`, `fclose`), and never look inside from Ingle — its lifetime is C's business. And the
...
home: a C function that returns a `char*` can be declared to **return a `string`**. Ingle copies the
bytes into an owned Ingle string on the way back and lets C free its own buffer — the safe "copy on
...
(a `[u8]` has no NUL terminator, so it needs a length channel first) and the "Ingle **adopts** the
...
honest wrinkle for [Chapter 18](#chapter-18--the-verification-loop): `--emit=replay` records a C
...
> symbol) while C does not, so `extern "C"` means "don't mangle this — speak plain C." Ingle
...
This part is about a single idea: stating what your code is supposed to do in a way the machine
can check, fuzz, prove, and replay. None of those four moves is new on its own — contracts,
property-based testing, theorem-proving, and record-replay all have long histories — but wiring
...
```
error[precondition_failed]: precondition failed in 'pos' (requires, line 2)
```
...
> The idea spent decades admired-but-niche. What's new in Ingle isn't contracts themselves —
...
it *false*. `inglec --emit=check` generates many inputs that satisfy a function's `requires`,
...
fragment — **linear integer arithmetic** over a function's integer parameters — Ingle can
...
The bane of debugging is the bug you can't reproduce. Ingle's nondeterminism comes from a small,
...
same executable contract, in one toolchain, is what Ingle is reaching for.
...
The final piece of Ingle's "knowing you're right" story is *observability*: the language is
...
Run any program with `--tape` (or `--emit=trace`) and Ingle writes a **tape**: one JSON object
...
or refinement is violated — Ingle stops with a structured **fault** that carries the offending
...
Why would a language make its errors and traces machine-readable? Because Ingle is built
...
> **Fireside trivia.** Ingle's tape is emitted from inside the VM's instruction dispatch, so it
...
> against each other and watches the allocator for double-frees and leaks. Ingle frees memory
> deterministically from tracked ownership ([Chapter 13](#chapter-13--memory-the-quiet-way)), and
> that deterministic freedom is exactly the thing that needs a fuzzer pointed at it.

### The generator: one seed, one program

The generator takes a seed and prints one valid Ingle program to standard output. Same seed, same
...
But a virtual machine is not how you *ship* software. For that, Ingle has a second path: it can
lower your program to **C**, hand that to the system C compiler, and produce a **standalone native
binary** — a real executable, no interpreter, no runtime to install, nothing of Ingle left on the
surface.
...
That is a native executable. It links against Ingle's small runtime — allocation, the drop
...
Two implementations of anything drift apart unless something forces them together, so Ingle runs a
...
```
```
error[postcondition_failed]: postcondition failed in 'bad' (ensures, line 2)
```
...
[Not Yet List](#chapter-23--the-not-yet-list).)
...
> a language that learns to speak it inherits the entire world C already runs on. Ingle's `--emit=c`
...
**designed but not yet built** — they appear in Ingle's manifesto, its spec, or its examples as
...
- **Generic bounds on methods and enums** — bounds (`<T: Ord>`, including several at once like
  ...
  [Chapter 7](#chapter-7--structs-and-methods) and [Chapter 10](#chapter-10--generics-and-interfaces).
...
- **FFI returning C-owned memory** — Ingle passes scalars, all-scalar structs, strings
  ...
  a length channel) and an "Ingle adopts and frees the pointer" form for ownership-transfer APIs —
...
> arrived. Ingle keeps two such lists in its own repository: a manifesto of decisions and a
...
The chapters so far built the language up piece by piece: values, functions, structs and enums,
generics, ownership, concurrency, contracts. This last part spends all of it at once. Flare —
Ingle's UI layer — is where structs become components, enums become the thing you `match` to paint
a screen, closures hang off buttons, and the render loop from
[Chapter 13](#chapter-13--memory-the-quiet-way) grows into an application. It's the book's capstone,
and there's no new syntax in it — only the old syntax pointed at a window.

## Chapter 25 — Flare: Interfaces You Can Read

Flare is Ingle's component-style UI layer. Components are functions, props are arguments, state is
...
model from [Chapter 13](#chapter-13--memory-the-quiet-way): the UI is a pure function of your state,
...
[Chapter 12](#chapter-12--ownership-without-tears) stays clean even while you build something live
and interactive.
...
[Chapter 12](#chapter-12--ownership-without-tears)) and emits some widgets. Here's a self-contained
...
- **Prose** — `f.paragraph(text, width)` word-wraps plain text to a pixel width. `f.rich_text(text,
  ...
  [Chapter 8](#chapter-8--enums-and-pattern-matching) doing exactly what it's for.
...
> whole stack, layout solver included, is itself a handful of readable Ingle files.
...
([Chapter 22](#chapter-22--compiling-to-native)) — each of which had been
"not yet" only a day or two earlier. It
covers only what had been built and tested by the time of writing; the
[Not Yet List](#chapter-23--the-not-yet-list) marks the boundary. A later pass — this one — folded

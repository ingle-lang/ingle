...
|-----|------|-------------|
| OFI-183 | **A double-free (over-release) of an FFI-returned string, exposed by `std/json.parse`, silently kills the fiber it runs on — the real cause of Inglenook's "Ollama model picker spins forever" (and, almost certainly, the earlier intermittent Anthropic first-reply hang).** Traced from the UI tape: switching Inglenook to Ollama animated a "Discovering models" spinner for ~15s then never resolved (frames kept climbing = UI alive, worker starved), and the user switched back. Root-caused by reproducing `ollama.discover()` headlessly. `discover()` does `let body = http.get(base + "/api/tags")` (an **FFI `ret_is_string`** result — `make_string` at vm.c:3467, refcount 1), then `match json.parse(body) { … uses the result … }`, then returns. Under a **drop-trace + ASan** build (`-DEMBER_DROP_TRACE=1 -fsanitize=address -DEMBER_PARALLEL -DEMBER_NET`) this is a deterministic **`heap-use-after-free` in `reclaim` (runtime.c:158/162)**: the ~1.4 KB `body` string is `pooled_free`d, then reclaimed a **second** time. Instrumenting `OP_DROP` proved `discover` drops `body`'s slot (3) exactly **once** at its epilogue — so the second free is an **aliased over-release**: something in the *parse-and-use* path holds a reference to `body`'s whole `ObjString` without a matching retain, and dropping it frees `body` again. The freed block returns to the recycle pool; the pool free-list is now corrupt, and the **next allocations reuse it** — when this runs on a spawned worker fiber (Inglenook's `disco_worker`/`stream_worker` do their networking off the render thread), the corruption lands on the fiber's own `Fiber.frames`, nulling the live `CallFrame.ip`. The worker's VM loop then unwinds early (release build: silent `run()→VM_OK` with the result never `send`; ASan build: SEGV at the dispatch `*frame->ip` vm.c:2076). Main is left parked forever in `recv` → the picker spins. **The "race" was an illusion**: the double-free happens every call, but whether the corrupted pool block is fatal depends on allocation timing, and the DNS path (`localhost` → macOS threaded resolver + GCD workqueue threads) fragments the heap enough to make it fatal ~85% of the time; a literal-IP base (`127.0.0.1`, no DNS) is fatal far less often — which is why it first looked network/DNS-specific. TSan is clean (not a cross-thread data race); an 8 MB worker stack doesn't help (not stack overflow); plain ASan alone misses it (the **pool recycles, never `free()`s** — needs `EMBER_DROP_TRACE`, cf. [[ember-asan-available]]). Note it EVADED Crucible/Ledger: it needs the specific FFI-string → `json.parse` → used-result shape, and it only bites through the pool. | **OPEN — root-caused, not yet fixed (2026-07-06).** Reliable repro: `oll.discover("http://127.0.0.1:11434")` in a loop under a `-DEMBER_DROP_TRACE=1` + ASan + NET + PARALLEL build → immediate `heap-use-after-free in reclaim`. Minimal shapes that do NOT trigger it (empty match arm; `let v = json.parse(body)` without using `v`) narrow it to **using the parse result** creating a second, un-retained owner of the source string. Next step: find the exact alias — audit `json.parse`/`Parser`/`json.get`/`json.as_str` and the checker's ownership for the input `text`: either the parser keeps a live reference to the whole source string without a retain (fix: retain, or copy), or the move-checker fails to mark `body` consumed when the parse result borrows from it (fix: treat it as a borrow that keeps `body` alive for the caller's single drop). Add a self-contained regression test (an FFI-string source — `read_file` — through `json.parse` + used result) to `tests/` and wire a `DROP_TRACE`+ASan lane so the pool-only UAF class is caught by CI, not just found by hand. Two independent hardening fixes were made along the way (they do NOT fix this bug): `CURLOPT_NOSIGNAL` on all libcurl handles (threaded-libcurl correctness) and `CURLOPT_CONNECTTIMEOUT` on the streaming/POST handles (bounds a genuine cold-connect stall to ~15 s instead of 300 s). See [[inglenook-ide]]. |

...
| OFI-143 | `std/sqlite` runs on the bytecode **VM only** (`make db`). The native backend (`inglec -o` → C, linking `libember_rt.a`) builds the runtime's FFI registry from `cextern.c` *without* `-DEMBER_SQLITE`, so the SQLite wrappers aren't registered there — the cextern indices the checker resolves wouldn't line up with the runtime table, so a native build of a `std/sqlite` program would mis-dispatch or fail to link. | **PARTIALLY ADDRESSED 2026-07-13** — native `std/sqlite` binaries are now *producible* and shipping: `make quog` compiles Quog (`public/quog/quog.ig`, a full `std/sqlite` app) to a standalone native binary via `inglec-db --emit=c quog.ig > quog.c` then `cc … quog.c src/runtime.c src/cextern.c build/sqlite3.o -lm -o quog`, recompiling `cextern.c` on the link line with the **same** `-DEMBER_SQLITE=1` as the emit compiler so the `em_ffi(idx)` registry indices align. Proven: `build/quog` is a self-contained ~1.5 MB executable (only `libSystem`; SQLite statically linked), full CLI + `serve` + `verify` all correct. **STILL OPEN:** the *one-shot* `inglec -o app app.ig` path — `compile_native` (`src/main.c:877-946`) has graphics/parallel/serial link branches but **no `EMBER_SQLITE` branch**, so it always links the sqlite-less `libember_rt.a` and would silently mis-dispatch a `std/sqlite` program. Proper closure = add a `db`-flavoured `RT_LIB_DB` (`libember_rt_db.a`, `cextern.c`+`sqlite3.o` with `-DEMBER_SQLITE`) + an `#elif EMBER_SQLITE` branch in `compile_native`, so `inglec-db -o` works directly and the foot-gun is removed. |

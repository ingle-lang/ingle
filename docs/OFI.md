...
|-----|------|-------------|
| OFI-181 | **An interpolation-parse error inside a string literal mis-reports its location — it is attributed to line 1 of the ENTRY file with a garbled `unexpected character (near '\')`, not the offending string.** Writing an un-escaped `{` in a string (e.g. a code snippet `"fn f() {\n ...}"`) opens a `{…}` interpolation hole; the parser then tries to parse the following bytes as an expression and fails, but the diagnostic points at `<entry>.ig:1:1`, `1:17`, … regardless of where the string actually is (surfaced twice building Inglenook's tests — a `\n`-bearing code sample in a string). The lexer/parser lose the real source coordinate for an interpolation-embedded expression error; the "near '\'" is the `\n` escape inside the mis-parsed hole. Wastes time chasing a phantom line-1 error when the fix is `\{`/`\}`. | **OPEN** — diagnostics precision. The interpolation-hole sub-parse should carry the hole's real `file:line:col` (offset from the string literal's start) into the error, and "expected an expression" should win over the raw `\` lex complaint. Related to the OFI-111a source-coordinate threading. Low priority (a clear error, just mislocated); the language behaviour is correct (`\{` is documented). |

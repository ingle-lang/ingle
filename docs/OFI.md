...
| OFI-190 | **Filesystem breadth + path helpers** — `exists`/`delete`/`rename`/`mkdir`/`stat`/`append`; `join`/`basename`/`ext`/`dirname`. | **PARTIAL — `mkdir` shipped 2026-07-11** (`std/fs.mkdir` over a `em_mkdir` FFI, for Quog's `.quog/` layout). The rest (exists/delete/rename/stat + the path-string helpers) remain OPEN. |

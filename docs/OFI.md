...
| OFI-133 | Toasts had no INTERACTIVE affordance — a notification could not carry an action (the reversible-"Undo" pattern), so a destructive action like deleting a conversation was instant and unrecoverable. | **CLOSED 2026-06-23.** `f.toast_action(text, label, token)` renders an accent action button on the pill; a RELEASE over it fires `token` for one frame via `f.take_action()` and dismisses the toast (manual release-edge hit-test against the direct-drawn pill). Dogfooded as Undo-delete: deleting a conversation snapshots it (title+turns, `.clone()`d — Ingle's linearity REFUSED the move-out of a field/loop-var, catching a real aliasing bug at compile time) and shows "Conversation deleted · [Undo]"; the token re-inserts + re-selects it. Regression `flare_toast_action.em` (press ≠ fire; release fires + dismisses); the no-action pill stayed byte-identical (graphics 43/0). |
...
**Tests:** `tests/run/newtype_basic.em`, `newtype_ops.em` (compare/show/map-key/unwrap/string), `newtype_soundness.em` (the 3 review cases), `error_newtype_mismatch.em`, `error_newtype_arith.em`, `error_newtype_not_iterable.em`, + `tests/native/newtype.em` (VM==native differential).
...
**Tests:** `tests/run/refinement.em` (construct/read/predicate-fn), `refinement_nested.em` + `refinement_multislot.em` (nesting + the multi-slot gap), `refinement_method_self.em`, `error_refinement_arith`/`_self_cycle`/`_nested`, `tests/fault/refinement_violation.em` + `refinement_multislot.em`. **v1 limitations (not bugs):** string-base refinements, refined mutable struct fields (no auto-invariant), and non-pure constructor args are deferred; static discharge of the check via the prover is a future bonus (today every check is runtime). Completes the type-system campaign (OFI-149 + OFI-150).
...
- `calc.em` — a complete `lex → parse → eval` pipeline: byte-level scanning, a recursive `enum Expr` with both `Box<Expr>` and `[Expr]` recursion, exhaustive `match`, a `Map<string,int>` env, `Result`+`?`.
- `symtab.em` — string interner (2000 symbols, deduped over repeats), keyword `Set`, `[Map<string,int>]` scope stack with shadowing.
- `recursion_scale.em` — a 150-deep `Box` spine (under the VM's 256-frame cap), an 8000-wide `[Tree]`, a depth-16 balanced tree (65 536 leaves).
- `file_io.em` — `write_file`/`read_file` round-trip + UTF-8 byte/char ops, the self-hosted lexer's input path.
...
kind byte. **Showcase + regression:** FNV-1a in pure Ingle — `tests/run/wrapping_arith.em`,
...
`std/map.em`); rewiring built-in-key Hash witnesses to pure-Ingle hashing is now *possible* but is a

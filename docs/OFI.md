...
| OFI-213 | **Ingle → WASM client host** — the browser render target ("IngleScript" in full): emit C via the self-hosting AST→C backend, add a WASI/JS platform shim for the runtime, compile with emscripten/clang-wasm, and hydrate the SSR page so the same Flare components run events client-side. Additive — reuses the freestanding-shim retarget the kernel work proved, not a new backend. Endgame of [ingle-web](https://github.com/ingle-lang/ingle/blob/main/docs/design/ingle-web.md). | OPEN |

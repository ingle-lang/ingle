...
| OFI-191 | **`std/encoding.ig`** — base64 + hex encode/decode. Pure-Ingle, no runtime change. | OPEN |
| OFI-190 | **Filesystem breadth + path helpers** — `exists`/`delete`/`rename`/`mkdir`/`stat`/`append`; `join`/`basename`/`ext`/`dirname`. | OPEN |
...
| OFI-188 | **Time + sleep** — wall-clock/epoch time + a `sleep` that parks the *fiber* not the OS thread; wall-clock joins the nondet record/replay set. | OPEN |

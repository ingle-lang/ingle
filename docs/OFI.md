...
nav_exclude: true
sitemap: false
description: "Opportunities For Improvement — bugs, design flaws, and improvements tracked while building Ingle."
layout: default
---
...
[manifesto](https://github.com/ingle-lang/ingle/blob/main/MANIFESTO.md) found while building Ingle. Raise here instead of coding around them.
...
**Status (2026-06-25): nothing on the critical path.** The compiler + stdlib are sound — the language-correctness sweeps (OFI-062..064 erased-generic double-frees, 083 slice overflow, 095 `_`-discard, 100..105 external review) all landed regression-tested + adversarially reviewed, and the unified **Fault** error-artifact campaign (OFI-106..111) is in (compile-diagnostic render + native parity are the tracked remainders). Recent work has centred on **Flare**, the declarative UI layer: visual polish, deluxe docking + persistence (OFI-112..119), then a **60fps performance + animation/notifications campaign** (OFI-126..133) — a `measure_text` cache, immediate-mode list virtualization, a real-time spring timestep, idle-CPU event-gating, and the presence/fade/toast/undo stack — all dogfooded in the Claude desktop app. **2026-06-25 OFI back-burner sweep:** the LLM-first interpolation gap **OFI-139 CLOSED** (a `Show` interface — `"{value}"` renders any type with `fn show(self) -> string`, fixing the single most common first-write LLM error), which surfaced + closed a latent owned-temp-string-hole leak (**OFI-146**); and the M:N GUI-thread soundness bug **OFI-138 / OFI-089 CLOSED** (the main/GL fiber is now pinned to worker 0 across park/resume, so graphics-on-M:N is sound, not just app-worked-around). A follow-on sweep of the low-priority back-burner then closed the **Ollama-provider trio** (OFI-135 OpenAI-format tools — verified live calling `read_file` on qwen3-coder; OFI-136 async model discovery off the render thread; OFI-137 cold-load hint) and the **Flare polish pair** (OFI-115 content-sized atomic buttons + `_fill` opt-in; OFI-068 a freetype-drift-tolerant graphics-golden comparator).

**2026-06-25 backlog-crunch (a 13-OFI parallel audit → close-the-bounded-wins batch, adversarially reviewed):** closed the LLM-first surface gaps **OFI-140** (named enum-variant construction `Circle(radius: 2.0)`) and **OFI-098** (`_` is the discard at every scope — `fn`/`struct`/`enum _` rejected), the **OFI-111(c)** u64-overflow-render wart (operands now print unsigned, in overflow AND div/mod-by-zero faults), **OFI-097** (LSP refuses bare `_`), plus the doc/decision closes **OFI-144** (blank-line convention documented), **OFI-109** (rich Faults scoped to the VM by architecture), **OFI-018** (cross-thread frees = accepted sound trade-off), **OFI-121** (resolved-with-residual by virtualization). The adversarial review caught two real over-rejection/render bugs pre-close (enum-name-qualified named construction; u64 div/mod dividend) and surfaced **OFI-147** (top-level constant annotation never checked — `let X: int = "hello"` compiled), filed + fixed same day. (Prior systems-language widening: **OFI-122** `resource struct`, **OFI-123** full-range u64 literals + width-accurate native scalar locals.)

**The remaining open set is all deliberate-deferral or measure-first**, in three buckets: **(1) Fault precision Phase 2/3** — the recursive struct/enum value walker (`Err("io")` not `<obj>`) + per-byte columns/per-fn source path (OFI-111 a/b), compile-diagnostics→agent-Fault render + `--check` counterexamples (OFI-110 c/d) — the next real Fault session, medium effort; **(2) M:N scaling** — work-stealing deques (OFI-087) + right-sized fiber stacks for the 100k-fiber tier (OFI-088), both measure-first; **(3) native widening** — `std/sqlite` native FFI link (OFI-143, **CLOSED 2026-07-13** — `inglec-db -o` links a `RT_LIB_DB` runtime variant, so a `std/sqlite` program compiles to a standalone binary; Quog ships as one). The remainder is long-deferred perf (OFI-050 linear symbol scan, measure-first) and accepted edges/wontfix (OFI-044/043/020/009/051/072).

**Next — priority order (set 2026-06-26, after an adversarial validation pass — the "chocolate-teacup check" — on refinement/newtypes; verdict: build, scoped).** Do the OFI work first, then the type-system campaign:

1. **Fault precision Phase 2/3 (moat completion).** ✅ **OFI-111b (value walker) + OFI-111a (true `file:line:col` + per-module source file) DONE 2026-06-26** — the agent-facing Fault now carries an Err/None payload as DATA (`Err("io")`, `MyErr { code: 5 }`) and points at the exact failing column in the right module file, closing the LLM self-correction loop. Remaining in this bucket: `--check` counterexamples → Fault (**OFI-110d** — next); lower priority (already served by `--diagnostics=json`): compile-diagnostics → agent Fault (OFI-110c); deferred: deterministic repro (OFI-111d, gated on OFI-044) and the human source-excerpt caret.
2. ✅ **Newtypes (OFI-149) SHIPPED 2026-06-26** — `type UserId = int`: a distinct nominal type over a scalar/string base at zero runtime cost; cross-type use is a compile error (kills the unit-confusion / swapped-argument bug class, a top LLM-codegen error); auto-inherits the base's compare/hash/show (so newtypes sort, key a `Map`, interpolate); arithmetic via explicit unwrap. Both backends, 412/0, 7 gates, ASan; a 3-bug adversarial review (a band-overlap SEGV, a width-literal ctor bug, a string-newtype double-free) all fixed.
3. ✅ **Refinement types (OFI-150) SHIPPED 2026-06-26** — `type Percent = int where 0 <= self && self <= 100`: a predicate over `self` checked at construction (`Percent(150)` traps with a `refinement_violation` Fault), reusing the contract machinery (debug-checked, release-elided). Sound at any nesting depth via codegen `self`-substitution. 421/0, 7 gates, ASan; a 4-bug adversarial pass (3 from the review + a multi-slot-sibling soundness gap I caught) all fixed.
...
|-----|------|-------------|
| OFI-224 | **UMBRELLA — v1.0 cutover: retire the C frontend, commit to the self-hosted compiler.** The execution of [OFI-218](https://github.com/ingle-lang/ingle/blob/main/docs/design/selfhost-full-language.md) §6(a) "staged retirement." Self-hosting itself is DONE (P0–P6 landed; `make bootstrap` rebuilds the compiler frontend-free from a checked-in seed; Inglenook compiles pixel-identically through the self-hosted C-emit). This campaign is the CLEANUP that makes the bootstrapped compiler the **default + installed** `inglec` and archives `src/`: (**G0**) fresh baseline; (**G1**) a persistent installable self-hosted `inglec` + re-home `bootstrap-refresh` off `$(BIN)`; (**G2**) complete the OFI-218 §3 oracle succession (behaviour corpus + VM↔C-emit differential + reproduction fixed point + fuzzers re-pointed — the fuzzers are self-contained, so this is an env-var re-point); (**G3**) triage every residual self-hosting OFI to closed-or-documented; (**G4**) flip `make`/`install`/CI + the ~15 flavored `$(SOURCES)` targets to the self-hosted compiler; (**G5**) the tooling decision (port the LSP off the C frontend — needs selfhost-checker source positions, "Inglenook-scale" — vs. retain stage-0 as a frozen tooling host); (**G6**) archive the 6 frontend `.c`, keeping `stage0-v0.3.42` as the from-C re-bootstrap oracle. Full plan + the three forks for Karl (esp. "does v1.0 require the LSP port, or ship at G4/G5(b)?"): [docs/design/v1-cutover.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/v1-cutover.md). | **OPEN — plan drafted + Fork-1 RATIFIED 2026-07-25 (Karl): v1.0 = G4/G5(b).** The birthday is the bootstrapped compiler as default + installed build/run toolchain with stage-0 frozen as a tooling host; the LSP port + full `src/` archival (G6) defer to **v1.1**. Recon verified against the live tree. **G0 (fresh baseline) in progress.** Rationale: ship the birthday on "reproduces itself + compiles real apps + honest known-limitations", where Go 1.5 / Rust 1.0 stood. |
| OFI-223 | **Self-hosted C-emit (`selfhost/cgen_c.ig`) cannot resolve a field access on a CROSS-MODULE user struct.** `pr.decls` where `pr: ps.ParseResult` (a struct defined in `parser.ig`, read in `checker.ig`) hits `internal error: unsupported field access \`.decls\``: the C-emit's field resolver only knows the CURRENT module's structs, so a field of an imported struct falls through to the OFI-173/202 error path. Stage-0's `cgen_c.c` emits it fine (it has the whole-program struct table). Fix = register cross-module struct field layouts in the self-hosted C-emit (thread the imported modules' structs, as `codegen` already threads `mod_of`). Surfaced OFI-218 P4 closing `error_enum_named` — a checker→parser side-channel first returned a `ParseResult` struct. | **OPEN — worked around.** `ps.parse_bad_call` returns a plain `bool` instead of a `ParseResult`, sidestepping the cross-module field access; the parser side-channel still works. No other corpus code does cross-module user-struct field access, so the gap was latent until now. Low priority. |
| OFI-222 | **Self-hosted checker validates named enum-variant construction only PARTIALLY** (the stage-0 OFI-140 checks). It rejects the two purely-SYNTACTIC errors — a duplicate named argument, and a mix of positional and named — via a parser side-channel (`ps.parse_bad_call`); it does NOT yet reject *no-such-field*, *missing-field*, or *a named argument on a plain function*, because those need the argument NAMES to reach the checker. The parser discards names today; carrying them means an `ECall` arg-names AST field, which forces a binding at every `case ECall(callee, args)` site (~70) across parser/checker/codegen/cgen_c, then checker validation against the variant's fields mirroring stage-0 (`src/check.c:2190-2271`, which clears arg_names to positional before codegen so `--emit=ast`/bytecode stay identical). Surfaced closing the OFI-218 P4 reject-parity miss `error_enum_named`. | **OPEN — sound subset done.** The dup/mixed check closes the corpus miss and can never false-reject (stage-0 rejects those in every call). Full field validation deferred (no other corpus program needs it beyond the subset). |
| OFI-221 | **Self-hosted C-emit does not context-type an empty-array literal `[]` passed as a call ARGUMENT.** Stage-0 (`src/cgen_c.c`, checker-typed) emits a bare `[]` arg to a `[bool]`/`[int]` param as `em_array(&g_em, 0, <AEK>)` with the param's element AEK (bool→11, int→4); the self-hosted C-emit has no callee-param context at the arg site and defaults the AEK to 0 (boxed). Only the arg site is affected — an *annotated* empty binding (`let e: [bool] = []`) is typed correctly. Surfaced by OFI-206 map work passing `[], []` for the new `lam_pstr`/`lam_pkind` params of `emit_fn_body`. | **OPEN — worked around, not fixed.** `emit_program` now passes typed-empty *bindings* (`let eb: [bool] = []` / `let ei: [int] = []`) instead of bare `[]`, so cgen_c.ig reproduction stays byte-identical. The underlying gap (arg-site context typing) remains; fix = thread the callee's param element-AEK into `emit_call_arg` for an empty-array literal. Low priority (no corpus program hits it). |
| OFI-220 | **Self-hosted C-emit does not RETAIN an enum-payload array assigned out into a caller variable.** `case TyFn(fps, fret) { outer = fps }` — stage-0 wraps the payload read in `own_into_slot` (the variable co-owns the array the enum still holds); the self-hosted C-emit emits a plain `outer = fps` (no retain) → a latent under-retain (double-free risk on drop). Surfaced by OFI-206 map work (`hof_srcs_of` originally did `fparams = fps`). | **CLOSED 2026-07-23 (`55a0be4`, OFI-218 Phase 4 un-dodging).** `emit_assign_value` now emits `own_into_slot` for a non-drop (borrowed) array binding read — an enum-payload array or a borrow-array param stored into a new owner is cloned (arrays are unique-owner), the borrow's owner keeps its reference (stage-0's moves_local==2). With the gap closed, `hof_srcs_of` reverted to `case TyFn(fps,..) { fparams = fps }` and the two dodge-helpers were removed. Gated fixture `payload_array_store.ig`; make selfhost 1498/0. |
| OFI-219 | **Self-hosted C-emit mistypes a NESTED-array element `xs[i]` (of a `[[T]]`) as a STRING.** `is_string_expr`'s `EIndex` arm returns true for any boxed (AEK 0) non-struct element, so `a[i].len()` on a `[[int]]` emits `em_str_len` instead of `em_array_len` (and `is_array_expr` has no `EIndex` arm, so it can't correct it). Stage-0's checker types it correctly. Surfaced by OFI-206 map work using `[[int]]` internal tables. | **CLOSED 2026-07-23 (`235f58d`, OFI-218 Phase 4 un-dodging).** Two new scope facets: `sc_elem_is_array` (the element is itself an array) + `sc_elem_elem_kind` (the inner scalar kind), set at every array push (annotated / inferred / param). `is_string_expr`'s nested-index arm excludes an element-is-array; `is_array_expr` gains an `EIndex` arm for it; `index_elem_kind` resolves `grid[i]`'s inner scalar kind; `index_elem_refcounted` treats a nested-array element as a unique-owner BORROW. `grid[i].len()`→em_array_len, `grid[i][j]` reads plain, `grid[i]`→a `[T]` param passes as a borrow. Nested arrays are now first-class in the self-hosted C-emit; gated fixture `nested_array.ig`; make selfhost 1501/0. The flat OFI-206 tables are now a simplicity choice, no longer required. |
| OFI-218 | **UMBRELLA — Selfhost Full-Language: extend Ingle in Ingle** (Karl, 2026-07-22: "the whole 9 yards"). The self-hosted compiler reproduces itself (both fixed points green) but covers a language SUBSET, its own sources dodge the holes (OFI-173/176/177), and new features land C-first and get mirrored — so "extending the language in Ingle" is not yet real (OFI-174 is the honest gate). The campaign closes it in 7 phases: **P0** truth + tripwires (kill the silent-stub class); **P1** Tier-2 erased-generic memory model (retain/drop discipline, deep-clone, mono inferencer); **P2** Tier-3 witness-dictionary subsystem (hidden args + instance-stored witnesses, `CALL_INDIRECT` dispatch, natives, spawn path); **P3** C-emit completion (lambda lifting OFI-206, OFI-202/203, the two OFI-173 lowerings); **P4** fidelity sweep + THE UN-DODGING (fix OFI-176/177 properly, revert every source workaround, checker reject-parity incl. OFI-199, OFI-200 — full corpus, OFI-174 closes); **P5** THE FLIP — first Ingle-first feature (`panic` + `unwrap`/`expect` land in selfhost as canonical, C mirrors after; oracle succession per the plan: behaviour corpus + VM↔C-emit differential + reproduction fixed point replace byte-identity-to-stage-0); **P6** bootstrap independence (checked-in generated-C seed, `make bootstrap` with no `src/` in the loop, stage-0 demoted to mirror/tooling host). Full plan: [docs/design/selfhost-full-language.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/selfhost-full-language.md). | **GOAL REACHED 2026-07-24 — Ingle is developed IN Ingle (P0–P6 landed; P4 has 1 diagnostic miss, P5 `unwrap`/`expect` deferred).** **THE FLIP + BOOTSTRAP:** **P5** shipped `panic(msg)` — the first Ingle-first feature: designed + built in `selfhost/*.ig` first (checker divergence + both backends), proven on the successor oracles, THEN mirrored to stage-0 byte-identical (`e98150b`). **P6** shipped `make bootstrap` (`dcc281a`): Ingle rebuilds its own compiler from a checked-in C seed (`bootstrap/seed/inglec_boot.c`) with the `src/` FRONTEND out of the loop — `make clean && make bootstrap` compiles only the runtime, never the frontend. Extending the language is now edit `selfhost/*.ig` → `make bootstrap`. **P4 reject-parity:** 16 misses → 1 (`error_resource_clone_match`, needs Result/Option generic-payload typing — diagnostic-only, does not block extension); 0 false-rejects; `make selfhost` 1507/0. **Remaining (follow-ons, not blockers):** the 1 reject-parity miss; the `unwrap`/`expect` tag-based intrinsic (mapped, `?`-comparable); re-point crucible/ceilings/ledger at the bootstrapped compiler; stage-0 tooling port + archival. History below. **P0/P1 DONE 2026-07-22:** the four `selfhost/cgen_c.ig` silent `INT_VAL(0)` stub-fallbacks + ten `codegen.ig` emit-nothing fallbacks are now hard `internal_error` exits (70) — a coverage hole FAILS THE BUILD instead of miscompiling (the OFI-173 general fix). **P1 (the erased-generic memory model + instance keying) COMPLETE:** nested type-arg preservation (`ty_key_full` + depth-aware key helpers + `field_receiver_targs` substitution), variable-arg concrete-element keying (`map<[int]>` vs `map<[string]>` via dual `amelem`/`selems`), transitive param seeding + 2-level return-type propagation (`sort<[int]>`→`_sort_range`→`_merge`), and generic-method return propagation (`b2 = b.replaced(8)`→`b2.get()` retargets). Corpus byte-identity breadth **300→310/552**; `make selfhost` 1477/0; reproduction fixed point holds. 7 of the 10-file target cluster now byte-identical; the 3 residuals (`generic_method`, `generic_literal_vs_lt`, `stdlib_list`) RUN CORRECT and diverge only on OFI-176 (owning-temp arg-masking → Phase 4, 2 files) and OFI-206 (lambda-instance typing → Phase 3, 1 file) — the exact open OFIs later phases close, not new work. **P1 discovery:** Tier-3 witnesses (OFI-174's "cliff") were already largely done on the VM backend (`bounded_generic`/`multibound_generic`/`hash_eq_bound`/`generic_bounded_ctor` pass), and the OFI-063 deep-clone needed nothing (already byte-identical) — OFI-174's failure list was stale. Full detail: [docs/design/selfhost-full-language.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/selfhost-full-language.md) §8. |
| OFI-217 | **`f.code_editor` / `f.code` render no scrollbar — neither vertical nor horizontal.** The Flare code editor and read-only code viewer scroll via caret-follow + mouse-wheel over encapsulated per-key `voff`/`hoff` state (`std/flare.ig` `_paint_code_editor`), but draw NO scrollbar track or thumb — so there is no visual indication of scroll position and nothing to grab/drag, unlike `f.scroll_begin`/`scroll_end` regions which route through `std/ui`'s scrollbar (proportional thumb + drag, `std/ui.ig:1965-2032`). Bites Inglenook's Editor 1 / Editor 2 panes on any file taller or wider than the pane (the Diff panel too). Fix: render a proportional scrollbar affordance on BOTH axes for the code editor/viewer — reuse the `std/ui` thumb/drag machinery, or draw a bar in `_paint_code_editor` keyed off `voff`/`hoff` vs content extent. Raised by Karl 2026-07-21 using the Inglenook editor. | **CLOSED 2026-07-21** (the editor). `f.code_editor` now has draggable **vertical + horizontal scrollbars** plus **mouse-wheel** scrolling (Shift = horizontal). New shared geometry `_code_scroll_geom` (a `CodeScroll` struct) is computed identically by the input (widget) and paint sides so the drawn thumb is exactly what you grab; the max line width for the horizontal bar is cached by buffer length (`_code_content_w`). Two coupled fixes made it work cleanly: (1) the text hit-rect passed to `_code_edit` is shrunk by the bar widths, so a press on a bar scrolls instead of starting a text selection; (2) **caret-follow now runs only when the caret actually moves** (tracked via a `/pcar` state) — previously it ran every frame and would have yanked a wheel/drag scroll straight back to the caret. Verified: `tests/graphics/flare_editor_marks` re-blessed (only the two scrollbar draws added, geometry checked), new `tests/graphics/ui_code_scroll.ig` drives a deterministic thumb-drag (scrolls the viewport WITHOUT focusing the text — FAILS pre-fix, PASSES post), graphics 56/0, and a live Inglenook run shows the bar on a 1400-line file. **REMAINING:** the read-only `f.code` viewer needs no vertical bar (it grows to its content height and is scrolled by its containing `scroll_begin` region, which has its own scrollbar); a horizontal bar for very long lines in that viewer is a minor separate gap, left open here. |
| OFI-216 | **`tests/graphics/flare_tooltip.ig` is non-deterministic — it depends on the physical mouse position.** The tooltip anchors to the LIVE cursor, and the graphics golden harness (`tests/run-graphics.sh`) normalizes only the `"mouse":[x,y]` field to `[_,_]` — NOT the tooltip layer's `shadow`/`round`/`stroke`/`text` draw coords, which track the cursor. So the test passes or fails by where the real cursor happens to sit at run time (verified against the clean baseline: fails at cursor ≈(160,125), passes near the golden's ≈(14,20) within the 3px `tape-cmp` tolerance) — a flaky gate, not a code regression. Fix: inject a fixed mouse position for the tooltip case (as the other graphics tests inject input state) so the anchor is deterministic, or normalize the tooltip-layer coordinates in the comparator. Surfaced 2026-07-21 building the Inglenook Diff panel. | **CLOSED 2026-07-21** — `flare_tooltip.ig` now PINS the cursor (`f.ui.mx = 40` / `f.ui.my = 30`, set each frame after `f.begin()` syncs the real device) so the tip card anchors deterministically at (54, 48) regardless of the physical mouse. Re-blessed; the graphics suite passes 56/0 on repeated runs. (The `f.ui`-field injection technique — set the input fields, then build — is the general way to test UI interactions headlessly, since there is no mouse-injection API.) |
| OFI-215 | **VM heap-buffer-overflow copying a value-struct element into an array (`AEK_INLINE_STRUCT`)** — appending a struct element read out of another inline-struct array (`for e in src { dst.append(e) }`) memcpy's `elem_size` bytes from a SOURCE whose struct `data` pointer is wrong: it aliases the end of a short/freed `make_string` region, so the 32-byte (2-`Value`) copy reads past a 64-byte allocation. ASan: `heap-buffer-overflow READ of size 32` at `src/vm.c:2862` (the `AEK_INLINE_STRUCT` branch of `OP_ARRAY_APPEND`), source allocated by `make_string` (`src/runtime.c:541`). Deterministic segfault (exit 139); reproduces for a value-struct with an int OR a string field — the aliased/uninitialised-pointer signature. Minimal repro: `struct TE { id: string }` then `var s:[TE]=[]; s.append(TE{id:"x"}); var d:[TE]=[]; for e in s { d.append(e) }; for e in d { println(e.id) }`. Sibling of the value-struct-in-aggregate family (OFI-062/063/203). **Surfaced 2026-07-11 dogfooding Quog** (a `[TreeEntry]` tree representation) — the dogfood working as intended; Quog moved to a string-serialised tree to stay on solid paths. | **CLOSED 2026-07-11.** **ROOT CAUSE:** `value_box` (`include/ember_rt.h:661`, used by `array_box`) has NO `AEK_INLINE_STRUCT` case — it falls to `default: memcpy(&v, p, sizeof(Value))`, reading only the FIRST 16 bytes of the inline struct data and returning them as a boxed `Value`. So `OP_FOR_ARRAY` (`src/vm.c:2533`, `array_box(a, idx)`) binds the loop var `e` to the struct's *first field* (here a string `Value`), not the struct; then `AS_STRUCT(e)->data` in `OP_ARRAY_APPEND` (`vm.c:2862`) dereferences that string as a struct → reads into its char buffer → the overflow. `OP_INDEX` (`vm.c:2760`) does it correctly (`alloc_instance` + full-`elem_size` memcpy + `struct_elem_retain`). **The fix is subtle:** the loop var is declared a BORROW and never dropped (`src/codegen.c:2323`, and `emit_drops_and_pops` starts after it), so simply making `e` an owned copy would LEAK per iteration, and worse, the `append(e)` move path (`memcpy` + `reclaim` the shell, no sub-field retain) would double-free/UAF a retained copy. Correct fix = element-type-aware for-loop lowering (value-struct elements iterate as owned + dropped, or as a genuine borrowed struct view), coordinated across `value_box`/`OP_FOR_ARRAY`/`OP_ARRAY_APPEND` + the checker's move/copy decision, in BOTH backends, with ASan + Crucible + a reclaim-double-drop pass. The adversarial-panel bug class (OFI-062/063). **FIX (both backends, no leak, no double-free):** the key insight is the loop var stays a BORROW to the checker, so a body that hands the element off CLONES it (`own_into_slot` / `moves_local=2`) and the source copy stays live — so an unconditional per-iteration drop of the materialised copy is always safe. **VM** (`src/vm.c` `OP_FOR_ARRAY`): for `AEK_INLINE_STRUCT` bind an OWNED per-iteration copy (`alloc_instance` + `memcpy elem_size` + `struct_elem_retain`, exactly as `OP_INDEX`) and `drop_value` the previous copy at the top of each step + at exit (the initial int placeholder and any moved-out slot drop as no-ops). **Native** (`src/cgen_c.c` for-loop): `em_index` already read the element correctly but pushed the loop var `drop=0` → a per-iteration LEAK; now `drop=1` for value-struct elements so `emit_drops` releases the box each pass (safe because `own_into_slot` clones for `append`). **Plumbing:** added `for_.elem_struct_id` to the AST (`include/ast.h`, init −1 in `src/parser.c`), stamped by the checker (`src/check.c` `STMT_FOR`, `array_inline_struct_id`) so the native backend knows the element is a value struct; the checker's ownership model is otherwise UNCHANGED (loop var stays `owned=0`). **Verified exhaustively:** ASan clean on VM + native across read-only / `append`-move / int-field / nested-loop / nested inline-struct-field / indexed `for (i,e)` / empty-array; the reclaim double-drop detector clean; all 8 `make verify` gates green (`test` 452/0 incl. native differential, `selfhost` 1453/0, `ledger` linearity, `crucible`, `ceilings`, `opcheck`). Regression: `tests/run/for_value_struct.ig` (VM golden) + `tests/native/for_value_struct.ig` (VM==binary differential). Closes the untested `for e in [ValueStruct]` corner. |
| OFI-214 | **`std/markdown` drops hard-wrapped list-item continuations** — an indented continuation line under a `- ` bullet (`  more text`) parses as a separate `Para`, so a wrapped bullet renders as a broken list + a stray paragraph (seen rendering PLAN.md through `std/html`). Needs lazy-continuation / hanging-indent handling in `parse`. Low priority (Quog's served content — commit messages, diffs — is rarely hard-wrapped). Surfaced 2026-07-11 building the Quog web dogfood. | OPEN |
| OFI-213 | **Ingle → WASM client host** — the browser render target ("IngleScript" in full): emit C via the self-hosting AST→C backend, add a WASI/JS platform shim for the runtime, compile with emscripten/clang-wasm, and hydrate the SSR page so the same Flare components run events client-side. Additive — reuses the freestanding-shim retarget the kernel work proved, not a new backend. Endgame of [ingle-web](https://github.com/ingle-lang/ingle/blob/main/docs/design/ingle-web.md). | **IN PROGRESS 2026-07-22 (first light + interactivity)** — **M1 first light:** an Ingle program `--emit=c` → emcc → wasm runs in the browser; the real `runtime.c`+`cextern.c` compile under emscripten unchanged (the AST→C retarget the kernel proved, now to wasm). **M2 a LIVE Flare component in the browser:** `make wasm APP=<file.ig>` (opt-in, needs `brew install emscripten`) emits C then emcc-links runtime + `graphics_headless` + the `std/web` DOM bridge with `-sASYNCIFY` + `tools/wasm-shell.html`. A DOM click → the shell's delegated listener queues the widget's `data-fl-id` → the Ingle loop's `web_next_click()` feeds `f.set_click(id)` → the matching button's `_fired(id)` fires with NO geometry (the web never `solve()`s, so `rects` is empty and this path is desktop-inert) → `web.set_html(f.html())` repaints. Bridge = 3 cextern builtins (`web_set_html`/`web_next_click`/`web_sleep` — EM_JS under `__EMSCRIPTEN__`, inert stubs elsewhere, guarded `#if EMBER_GFX_HEADLESS`) + `std/web` + flare `_click_id`/`set_click`/`_fired` + `data-fl-id` in `html()`. Verified in-browser: `examples/web/counter.ig` increments live. `make test-web` compile-checks the example. **M3 hydration + DOM-diff DONE:** `ingleMorph` (tools/wasm-shell.html) patches the live DOM IN PLACE (index-based reconcile) instead of replacing innerHTML — no flicker, and focus/selection/scroll survive; `web_set_html` calls it. `make wasm-hydrate` renders the initial frame SERVER-SIDE (`examples/web/counter_ssr.ig` via inglec-web) and bakes it into `#app`, so `build/wasm/index.html` shows the counter with ZERO JS (curl proves it) and the wasm client HYDRATES onto that exact DOM. Verified in-browser: the server-rendered nodes keep their identity through hydration + clicks (tagged SSR node survives → true hydration, not replacement). **The full "IngleScript" loop is closed: one Flare component renders on desktop (raylib), server (SSR HTML), and browser (WASM, hydrated) from a single source.** **M4 full interactivity DONE:** web-click extended past buttons to `nav_item` + `checkbox` (`_fired(id)`, geometry-free), and **text input** — `_FIELD`→`<input>` / `_TAREA`→`<textarea>`; the browser owns the caret/IME, a delegated `input` listener queues `id\tvalue` → `web_next_input()` → `f.set_input(id, value)` (a `_web_inputs` map reset each `begin()` so programmatic value changes still win) → `text_field`/`text_area` return it. `std/web.pump(f)` drains this frame's clicks + inputs in one call; the morph reconciles `<input>.value` (applies programmatic changes, preserves the caret during typing). Verified in-browser: **`examples/web/todo.ig` is a working todo app** — type → Add (+ field clears), checkbox toggle, stable-id delete — all WASM from one Flare component. **M5 network:** `web.fetch(url)` — a real HTTP GET awaited via ASYNCIFY (`EM_ASYNC_JS`), returning the body (or `{"_fetch_error":…}`); call on-demand, not per-frame. **`examples/web/repo.ig` loads the Ingle repo LIVE from the GitHub API, parses it with `std/json`, and renders it** (verified in-browser: name + description + stars). Data-driven Flare web apps work. **M6 the widget kit is COMPLETE on the web:** slider (`<input type=range>`), tabs (clickable chips), dropdown (`<select>`, options carried on a new `rextra` queue column via `_queue_x`), and modal overlays (a `_MODAL_BEGIN` float → a fixed dimmed scrim + centred panel; a scrim click dismisses via `_fired`). `examples/web/gallery.ig` exercises all of them live. So one Flare component gives buttons, checkbox, text field/area, nav, slider, tabs, dropdown, and modal on desktop AND web. Follow-ons: Enter/keydown submit (input events only today); anchored popovers/menus (need geometry the web lacks); a served (not build-baked) SSR+wasm page. | **substantially DONE 2026-07-22 — W4 milestones 1–4 shipped (full interactivity)** |
| OFI-212 | **Flare HTML render backend (Tier B)** — a `Backend`/`Surface` value on `struct Flare` + an HTML `finish()` that walks the existing semantic display list (`rnode/rkind/rtext/rid`, `std/flare.ig:379`) to HTML+CSS, so EVERY Flare component renders native *or* web from one source (write a Quog UI once, ship desktop + website). Route the ~17 gfx primitives + a `measure_text` oracle through it (also `ui.card`/`_tf_draw`/`_paint_code`); `measure_text` stubs in HTML mode (the browser lays out). See [ingle-web](https://github.com/ingle-lang/ingle/blob/main/docs/design/ingle-web.md). | **IN PROGRESS 2026-07-21 (SSR renders + serves headless)** — shipped `flare.html()`: a SECOND terminal walk of the same `lo` + paint queue `finish()` paints (NOT the `Backend`/`Surface` object — that's deferred to W4), mapping each layout node's flex intent → CSS flexbox and each widget kind → a semantic tag (`_BUTTON`→`<button>`, `_H1`→`<h1>`, `_LINK`→`<a>`, `_PANEL`→a surface `<div>`), with an inlined dark theme + an honest `data-fl-kind` fallback for unmapped kinds. The `measure_text` oracle landed two ways: `INGLE_HEADLESS` (skip `InitWindow`, `!IsWindowReady()`→codepoint-estimate width) in `src/graphics.c`, AND a **raylib-free web build flavor** — `make web` → `build/inglec-web`, `src/graphics_headless.c` stubs every `ember_gfx_*` symbol as pure C (no raylib/FreeType/GL, links only libSystem). A Flare component serves over `std/http_server` headless, verified in-browser (correct HTTP/1.1, semantic tags, routing). Regression `tests/web/ssr.ig` + `make test-web` (exact golden; SSR output is deterministic). Desktop path unchanged (graphics 56/56). **Text reflow DONE** (`_is_prose_block`/`_html_prose` collapse a wrapped rich_text paragraph back into one `<p>` the browser re-wraps). **Theme-from-`ui.Style` DONE** (`_flare_css` emits a `:root` of CSS custom properties from the live palette — a page follows theme_light/theme_dark; the earlier hard-coded dark CSS was a fidelity bug vs. the `theme_light` default). **Native binaries DONE** (`inglec-web -o server server.ig` → a standalone binary linking only libSystem, via `RT_LIB_WEB` = runtime+cextern+graphics_headless archived with the web defines + a `compile_native` web branch in `src/main.c`; `tests/run-web.sh` also native-compiles each case and asserts the binary's output == the golden). **Remains:** overlays/scroll/modal/dock coverage (render as plain divs today); and the `Backend`/`Surface` seam proper (W4 wants it). |
| OFI-211 | **Socket primitive + HTTP/1.1 server in `std`** — no server-side networking today (`std/http` is a libcurl CLIENT). Add ~a dozen socket wrappers (`socket`/`bind`/`listen`/`accept`/`recv`/`send`) via the `src/cextern.c` registry (`g_sigs`/`g_fns`), or the direct-extern FFI (OFI-167) for the fd-only calls, behind an opt-in build flag; write HTTP/1.1 parsing + routing in pure Ingle over `nursery`/`spawn` (accept loop = spawn-per-connection). The `serve`/`sync` engine for Quog (W2). Wants OFI-197 (`select`/timeout) for graceful shutdown; TLS deferred to a reverse proxy / later flag. | **CLOSED 2026-07-11 (server; sync is separate)** — shipped a purpose-built TCP FFI in `src/cextern.c` (`em_tcp_listen` hides socket+SO_REUSEADDR+bind(INADDR_ANY)+listen behind one "listen on port" call, so no `sockaddr` crosses the boundary; `em_tcp_accept`/`em_recv`/`em_send`/`em_close`; SIGPIPE ignored; DEFAULT build, no dep) + `std/http_server.ig` (HTTP/1.1 in pure Ingle: `Server`/`Conn` `resource struct`s that auto-close their fd, `listen`/`accept`/`read_request` (method+path) / `respond`/`ok_html`/`not_found`). Dogfooded: **`quog serve` renders the repo's history + per-commit pages as a browsable website** via `std/html`, verified with curl (correct HTTP/1.1 responses) and in-browser (styled, links navigate). SEQUENTIAL (one connection at a time) — per-connection fibers (`nursery`/`spawn`, wants OFI-197) + `sync` (additive object exchange over HTTP) are the follow-ons. TLS still out (reverse proxy). |
| OFI-210 | **Typed `[u8]` file I/O** (`read_bytes`/`write_bytes`) — `read_file`/`write_file` are `string`-typed even though the runtime is already byte-safe (`fopen` `rb`/`wb`, length-prefixed buffers, `src/runtime.c:2455-2491`), so bytes round-trip today; add a `[u8]`-typed API for type-honesty, a thin wrapper over the same buffer + `from_bytes`/`.bytes()`. Ergonomic, NOT a blocker (Quog can use the string path now). | OPEN |
| OFI-209 | **`bind_blob`/`column_blob` in `std/sqlite`** — the module exposes `bind_int/f64/text/null` + `column_int/f64/text` but has NO blob accessor, though `column_type` already names BLOB=4 (`std/sqlite.ig:45-56`). Quog's `object(... data BLOB)` store needs it; storing raw bytes in a TEXT column truncates at the first NUL (`sqlite3_column_text`). Small: mirror `bind_text` over `sqlite3_bind_blob`/`sqlite3_column_blob`. Alternative sidestep: base64-in-TEXT (`std/encoding`, +33% size). Phase-1 prerequisite. | **CLOSED 2026-07-11** — shipped `bind_blob` / `column_bytes` / `column_blob` in `std/sqlite` (`src/cextern.c` wrappers over `sqlite3_bind_blob`/`_column_blob` using the existing `'b'` buffer marshalling — read-only in, pre-sized mut buffer out), gated by `tests/db/blob.ig` (a payload with an embedded NUL + `0xFF` round-trips byte-exact and its SHA-256 content id is stable). Quog's object store uses it. |
| OFI-208 | **`std/html` — Markdown/AST → HTML (SSR content leaf, Tier A)** — render `std/markdown`'s pure `Block`/`Span` AST to HTML by reusing the parser and swapping the emitter; HTML escaping + a styled, self-contained page wrapper. Quog's read-only web view. | **CLOSED 2026-07-11** — shipped `std/html.ig` (`render_markdown`/`render_block`/`render_spans`/`escape`/`page`), gated by `tests/run/html.ig`; renders PLAN.md to a styled light/dark page, XSS-escaped. Full component retarget is OFI-212. |
| OFI-207 | **SHA-256 in `std`** — content addressing for Quog; pure Ingle (32-bit modular arithmetic via `wrapping_add` + the width-aware bitwise operators), no runtime change. | **CLOSED 2026-07-11** — shipped `std/sha256.ig` (`digest([u8])` / `digest_str(string)` → the 32 digest bytes), verified against the FIPS 180-4 vectors including a multi-block (56-byte) message, gated by `tests/run/sha256.ig`. Render as a text id with `std/encoding.to_hex`. Named `digest` (not `hash`) to avoid the built-in FNV-1a `hash(string)->int`. |
| OFI-206 | **The self-hosted C-EMIT backend (`selfhost/cgen_c.ig`) has NO first-class-function / closure support** — it cannot compile a program that passes a function as a value (`em_closure`) or calls a function-typed parameter (`rt_call_closure`). Even a plain HOF (`fn apply(f: fn(int)->int, x: int) { return f(x) }`) diverges from stage-0: the fn-value argument emits EMPTY and the `f(x)` call stubs to `INT_VAL(0)`. The self-hosted VM codegen (`selfhost/codegen.ig`) DOES support closures (~91 lines: OP_CLOSURE / OP_CALL_CLOSURE / lambda lifting / captures); the C-emit backend was never given the mirror because no gated `tests/selfhost/cgen_c` fixture or self-hosted module uses a higher-order function (the compiler internals don't). Surfaced landing Phase 3's HIGHER-ORDER combinators: `map<T,U>(o: Option<T>, f: fn(T)->U)` and `and_then` take a function parameter, so their bodies (`Some(f(v))` / `f(v)`) need `rt_call_closure`, and a `o.map(dbl)` call site needs `em_closure(&g_em, <fn>, 0)`. **Consequence:** `map`/`and_then` work end-to-end under stage-0 (VM byte-identical + native, since the C REFERENCE `src/cgen_c.c` has closures) but can't be byte-identical-gated in the self-hosted C-emit until it learns closures. The other 6 combinators (5 leaf + `ok_or`) need no closure and ARE fully gated both backends. | **PARTIAL — NAMED-FN closures DONE 2026-07-11; lambdas remain.** Two of the three C-emit closure points are mirrored into `selfhost/cgen_c.ig`: (1) a bare function name used as a VALUE → `em_closure(&g_em, fn_index(name), 0)` (in `emit_expr`'s `EIdent` case, when the name is not a local but is a function); (3) a call THROUGH a fn-typed local/param → `emit_closure_call` → `rt_call_closure(&g_em, <cname>, <argc>, (Value[])\{…\}, em_invoke)` (in `emit_call`, when the callee ident resolves to a local — a called local can only be a closure value). Plus a fn-typed param is now marked OWNED (`is_fn_ty` → `sc_drop`), so its closure is dropped at scope exit like a string/enum. **`map`/`and_then` are now byte-identical on BOTH backends with a NAMED-function argument** — gated in `tests/selfhost/{codegen,cgen_c}/combinators.ig` + `tests/run/combinators.ig` (`a.map(dbl)` / `a.and_then(half_even)`); `make selfhost` 1429/0, `make verify` green. **REMAINING = point (2), LAMBDA LIFTING:** a lambda `\|x\| …` argument still stubs to `INT_VAL(0)` in the C-emit (stage-0 lifts it to a synthetic `em_fn_<k>` + `em_closure(&g_em, k, ncap, captures)`). Needs a per-function lambda-collection pre-pass (assign lifted indices), emission of the lifted fn bodies + their `em_invoke`/forward-decl entries, and capture handling — mirroring the VM's `LambdaSpec` machinery (`selfhost/codegen.ig`). Users CAN pass a lambda to `map`/`and_then` today (it works natively via the C reference + on the VM); only the self-hosted C-emit differential can't yet cover lambda args. Door-opener status: named-fn HOFs (a subset of `std/list` usage) now C-emit byte-identical; lambda HOFs await point (2). |
| OFI-205 | **The self-hosted codegen backends don't compile a UFCS call to a GENERIC free function BYTE-IDENTICALLY to stage-0** — surfaced gating Phase 3's combinators (`a.unwrap_or(0)`, `a.is_some()`), the FIRST code to combine UFCS with a generic free function (the `ufcs.ig` fixtures use non-generic enums; existing generic calls are all plain, not method-style). TWO distinct gaps: **(1) VM instance collection** — stage-0's checker rewrites `a.unwrap_or(0)` → `unwrap_or(a, 0)` and monomorphizes it (a base slot + a concrete instance slot; the CALL targets the instance). The self-hosted `codegen_dump.ig` has no checker, so `FnInstColl.walk_expr`'s `EGet` branch must collect the instance itself — but for a UFCS call on a TRACKED value whose type is a non-struct (`a: Option<int>`), it falls through both the struct-method path (no `rargs`) and the free-fn path (guarded by `cg_index_of(snames, rname) < 0`, i.e. receiver NOT tracked), so NO instance is registered → the self-hosted VM emits only the base and the CALL index diverges (`CALL 1` vs stage-0's `CALL 5`). The combinator BODIES already match; only the instance numbering differs. **(2) Erased-generic ownership** — in a generic function stage-0 treats a generic-`T` payload binding / param as OWNED: `case Some(v) { return v }` binds `v` via `own_into_slot` and `return d` (a `d: T` param) via `own_into_slot`. The self-hosted C-emit retain-dances instead (`payload_refc(f<0)=false` — correct for the modules' CONCRETE `Option` matches, wrong for a GENERIC combinator; distinguishing needs subject-is-`Option<TypeParam>` awareness the self-hosted cgen_c lacks). **What IS byte-identical:** cgen_c's BOOL combinators (`is_some`/`is_none`/`is_ok`/`is_err` — bind but never return the payload); only `unwrap_or` (returns the payload) trips gap 2, and the VM trips gap 1 for all. The combinators WORK end-to-end on both backends (OFI-187 M1+M2) and the self-hosted drivers now INJECT them (M3 infra, commit `6bb4ab4`); only the byte-identical FIXTURE gate is blocked. | **CLOSED 2026-07-09** — both gaps fixed, all five combinators byte-identical on BOTH backends + the serializer, gated by `tests/selfhost/{codegen,cgen_c}/combinators.ig` + behavioural `tests/run/combinators.ig`; `make selfhost` 1429/0, reproduction fixed point intact. **Gap 1 (VM instance collection):** `FnInstColl.walk_expr`'s `EGet` branch now guards the generic-struct-method path with `struct_named(rty)` (a generic ENUM instance like `Option<int>` tracks an rty/rargs that mimic a struct's but names no struct) and adds a UFCS branch — a VALUE receiver whose type is not a struct + `mname ∈ generic_fns` (never a struct method — those live in `DStruct.methods`) registers the instance keyed by the RECEIVER (`mono_arg_key(obj)`), exactly as `gen_ufcs` retargets. **Gap 2 (erased-generic ownership):** an erased generic-`T` value (a `d: T` param, a `Some(v)` payload of an `Option<T>` param) is a refcounted borrow — own_into_slot / INCREF on return, never dropped. cgen_c: `ty_is_generic_param` marks a `d: T` param `sc_refc`; `cur_gopt` + `subject_is_gopt` mark the payload; `ret_owns(EIdent)` honours `lookup_refc` for the no-drops return path. VM: `ty_is_gopt` + `gopt_params` + `subject_is_gopt` bind the payload `is_str=true` (the param path already INCREF'd via `erased_gnames`). The concrete-`Option` module matches are untouched (the subject is `Option<int>`, not `Option<TypeParam>`). A pre-existing latent bug this exposed: a plain generic-return `fn id<T>(x: T) -> T { return x }` also now own_into_slots (was emitted bare). Residual (not corpus-reachable, deferred): the three-driver duplication of the prelude-inject helpers could be a shared module; `Result<T, ConcreteE>` would mark an `Err(e)` payload refcounted off `args[0]=T` (combinators + corpus never hit it). |
| OFI-204 | **The self-hosted C-EMIT backend (`selfhost/cgen_c.ig`) mis-compiles a `match` on a GENERIC enum PARAMETER (`fn f(o: Option<int>) { match o { case Some(v) … } }`).** It emits the value-binding path — `if (1) { Value v1 = v0; return ; }` — instead of the tag test `int v1 = em_tag(v0); if (v1 == 0) …`: `self.en.variant_flat("Some")` returns −1 in the C-emit's enum table, so `Some(v)` is misclassified as a scalar value binding (2b) rather than a variant. The self-hosted VM codegen handles the same match BYTE-IDENTICALLY (its variant resolution finds Some/None), so this is a C-emit-only enum-table gap for a generic-instance-typed subject. Pre-existing (predates Phase 3 UFCS — surfaced writing an `unwrap_or(o: Option<int>, d)` combinator); no self-hosted module or gated fixture defines a function that matches a generic `Option`/`Result` param, so `make selfhost` stays green. **Consequence:** Phase 3's Option/Result COMBINATORS (which match `Option<T>`/`Result<T,E>` params) work under stage-0 but can't yet be byte-identical-gated in the self-hosted C-emit; UFCS itself is byte-identical (its fixtures use a non-generic enum to stay off this gap). | **CLOSED 2026-07-09** — fixed exactly as the OPEN analysis prescribed: a targeted C-emit match-lowering + construction change that does NOT touch `build_enum_tab` (so module reproduction is untouched — `make selfhost` 1420/0). (1) MATCH: `EnumTab` gains `prelude_variant_tag` / `is_case_variant` / `case_tag`, so a `match` on a generic `Option`/`Result` param classifies `Some/Ok/None/Err` as variants + tags them (Some/Ok=0, None/Err=1) via the prelude fallback rather than `variant_flat` (which stays −1); `payload_refc(f<0)` returns false so the payload binding inherits stage-0's runtime `IS_OBJ`-retain dance (no `pf_ty[-1]` crash). (2) CONSTRUCTION: `emit_enum_ctor` + `is_enum_expr` route prelude variants via `is_case_variant`, and `prelude_enum_id` returns the DECL_ENUM id (user-enum count for Option, +1 for Result) — so `Some(5)` → `em_enum(&g_em, N, 0, 1, INT_VAL(5))` byte-identical to stage-0, and a prelude-`Option`-typed `let` is marked owned (`own_into_slot` on move-into-call + scope-exit `drop_value`). Gated byte-identical BOTH backends: `tests/selfhost/cgen_c/prelude_option.ig` (C-emit) + `tests/selfhost/codegen/prelude_option.ig` (VM bytecode); behavioural `tests/run/prelude_option.ig`. This is the foundation for the OFI-187 combinators. (Value-struct enum payloads remain OFI-203 — orthogonal; combinators use scalar/generic payloads only.) |
| OFI-203 | **The self-hosted backends diverge from stage-0 on a VALUE-STRUCT ENUM PAYLOAD** (`enum Shape { Cell(p: Point) }` where `Point` is an all-scalar `struct`) — both the construction and the destructure. Stage-0 boxes the struct into the enum (`NEW_STRUCT` before `NEW_ENUM` on the VM; `em_box_struct` / `em_unbox_struct` + an `em_s` value in C-emit); the self-hosted VM codegen omits the `NEW_STRUCT` box, and the self-hosted C-emit binds the payload as a bare `Value v = em_enum_field(...)` and then reads `v.f0` — which is *invalid C* (a `Value` has no `.f0`). Pre-existing (predates Phase 2d — no gated fixture or self-hosted module constructs a struct-typed enum payload, so `make selfhost` stays green); the VM/native single-name binding of such a payload is where it surfaces. **Consequence for Phase 2d:** one-level nested struct destructuring (`case Some(Point(x, y))`) builds directly on this path, so its self-hosted **codegen/C-emit cannot be gated byte-identical** until this is fixed — the 2d nested logic IS mirrored in `selfhost/{codegen,cgen_c}.ig` (forward-correct, using the C's `em_unbox`/double-`GET_FIELD` approach), but no nested `tests/selfhost/{codegen,cgen_c}` fixture is added. The 2d PARSER (byte-identical AST over the whole corpus) and CHECKER (accepts `match_nested.ig`, no false-reject) ARE gated. | **OPEN** — pre-existing self-hosting fidelity gap in the value-struct-in-enum representation. Fix: in `selfhost/codegen.ig` emit `NEW_STRUCT` (box) when constructing a value-struct enum payload, and in `selfhost/cgen_c.ig` emit `em_box_struct` on construction + `em_s` + `em_unbox_struct` on the single-name payload binding (the machinery already exists for value-struct params/returns — reuse it). Then the single-name AND the 2d nested paths become byte-identical, and a nested fixture can join the gated set. |
| OFI-202 | **The self-hosted C-emit backend (`selfhost/cgen_c.ig`) stubs a QUALIFIED nullary enum construction (`Color.Red`) to `INT_VAL(0)` instead of `em_enum(&g_em, id, tag, 0)`.** Bare construction (`Red`, `Rect(3,5)`) is emitted correctly; only the `Type.Variant` *value* form (an `EGet` on an enum type name) is unhandled — the `emit_expr` `EGet` path doesn't recognise the object as an enum type and fall through to a variant construction. Pre-existing (predates Phase 2c — the existing `tests/selfhost/cgen_c/enums_match.ig` fixture constructs only bare variants), orthogonal to pattern matching; surfaced when a Phase 2c or-pattern test used `Color.Red` in `main()`. Not corpus-reachable on the byte-identical fixtures (they use bare construction), so `make selfhost` stays green. Behaviour is only wrong under the *self-built* C-emit compiler; stage-0 and the VM path both handle `Color.Red`. | **OPEN** — self-hosting fidelity gap in one backend. Fix in `selfhost/cgen_c.ig`'s `emit_expr` `EGet` case: when the object resolves to an enum type and the name is a nullary variant, emit `em_enum(&g_em, <id>, <tag>, 0)` (mirror stage-0's `cgen_c.c`). Low priority (bare construction is the common form). |
| OFI-201 | **The self-hosted CODEGEN could not emit a scalar VALUE-BINDING pattern (`case n` — Phase 2b `PAT_BIND`) — no scrutinee-type inference to tell a value binding from a variant.** — **CLOSED 2026-07-06.** AST-stamping (the C compiler's route) is impossible in the self-hosted compiler — each pass RE-PARSES source, so the checker and codegen never share an AST to stamp — and the codegen tracks per-slot struct/string/array types but not enum types. **Resolution: use the self-hosted backend's OWN by-name resolution** — a bare pattern name that the enum table does NOT resolve (`variant_flat`/`resolve_case_vi` < 0) is a value binding; a name it DOES resolve is a variant. Both self-hosted backends now classify this way and emit `PAT_BIND` + the guard flag-restructure BYTE-IDENTICAL to stage-0 (cgen_c gated by `tests/selfhost/cgen_c/match_guards.ig`, 19/19; VM codegen opcode-identical, runs correctly, with only the OFI-200-class source-line-map delta). The one ambiguous edge — a value binding named exactly like a visible enum variant — is closed by a checker guard in BOTH compilers (`resolve_variant`/`variant_enum`): such a name is treated as a variant (a clean error on a scalar), so there is NO silent miscompile. The pre-existing OFI-163 (Option/Result generic-payload refcount classification) remains, but 2b's value binding no longer needs it. | **CLOSED** — resolved without OFI-163, via by-name classification + a cross-compiler collision guard. |
| OFI-200 | **The self-hosted CODEGEN emits correct-but-not-byte-identical BYTECODE for a literal-pattern match** (`case 0` / `case "x"`). Output is identical on both backends and the C-emit path is byte-identical to stage-0 (gated by `tests/selfhost/cgen_c/match_literals.ig`), but the self-hosted `--emit=bytecode-bin` `.igb` differs from stage-0's (`cmp`: char 151; the file is 6 bytes larger with a leading count byte `05`→`06`) — a const-pool / source-map difference in the literal path, NOT a soundness issue. Consequence: `tests/run/match_literals.ig` can't join the bytecode-byte-identical `tests/selfhost/codegen/` fixture set. Surfaced landing Phase 2a (OFI-186). Likely causes to check next: (a) `gen_expr(lit, line)` uses the match's `value.line` (the `lit` Expr carries no per-node line in the selfhost) instead of the literal token's line → a source-map delta; (b) a const-pool dedup/fold difference for the `case -1` neg or the string constants. | **OPEN** — minor fidelity gap; correct output. Fix by storing the literal's line in the selfhost `Pattern` (parallel `lit_line`) + matching stage-0's const emission, then add `match_literals` to `tests/selfhost/codegen/`. |
| OFI-199 | **The self-hosted parser ACCEPTS an over-u64 integer literal that stage-0 REJECTS.** Stage-0's `parse_primary` accumulates the magnitude with `__builtin_{mul,add}_overflow` and errors ("integer literal is out of range (larger than u64 / 2^64-1)") past 2⁶⁴−1; `selfhost/parser.ig` `parse_int_lit` uses `wrapping_{mul,add}` with no overflow flag, so `18446744073709551616` (or `0x1_0000_0000_0000_0000`) silently wraps and parses. A pre-existing self-hosting fidelity gap (the decimal case predates the Phase-1 literal work — OFI-185 only extended the same wrapping to the new hex/bin/oct forms); **not reachable by the corpus** (no `tests/**` file has an over-u64 literal), so `make selfhost` stays green. Same class as the checker's 10 tolerated "not-yet-rejected" verdicts. Surfaced by the OFI-185 adversarial-review pass. | **OPEN** — fidelity gap. Fix when the self-hosted parser gains real range diagnostics: detect the wrap (`v < prev` after a `wrapping_mul`/`wrapping_add`, base-aware) and reject, matching stage-0. Low priority (rare input, no gate impact). |
| OFI-198 | **Auto-derived structural equality** — `==` on a struct is a compile error today, yet LLMs write it constantly. Derive *structural equality only* (not general operator overloading). Design-gated. See [ergonomics-campaign](https://github.com/ingle-lang/ingle/blob/main/docs/design/ergonomics-campaign.md). | OPEN |
| OFI-197 | **`select`/timeout over channels** — the one synchronisation primitive the channel-first model actually needs (mutex/atomics left out). Design-gated. | OPEN |
| OFI-196 | **Tuples / multiple return** (`return (a, b)`, `let (x, y) = …`) — tension with the name-your-types ethos; real type-system cost. Design-gated. | OPEN |
| OFI-195 | **`if`/`match` as expressions** (`let x = if c { a } else { b }`) — a block-as-expression grammar change, not a desugar. | OPEN |
| OFI-194 | **Package manager** — manifest + resolution + fetch (MANIFESTO §4.7). Lowest-leverage Tier-1 for an LLM-first language; follows adoption, not precedes it. | OPEN |
| OFI-193 | **Build + test runner** — a canonical `inglec test` over `tests/` and a project build entry (§4.7). | OPEN |
| OFI-192 | **Formatter `inglec fmt`** — one canonical layout → deterministic diffs → one true way to write it (§4.7). *The* LLM-first tool; first in Phase 5. | OPEN |
| OFI-191 | **`std/encoding.ig`** — base64 + hex encode/decode. Pure-Ingle, no runtime change. | **CLOSED 2026-07-11** — shipped `std/encoding.ig`: `to_hex`/`from_hex` (lowercase, `Result` on malformed input) and `to_base64`/`from_base64` (standard alphabet, `=` padding, whitespace-tolerant decode), gated by `tests/run/encoding.ig` against RFC 4648 vectors + round-trips. Content ids render via `to_hex`; the base64-in-TEXT store sidestep (OFI-209) rides `to_base64`. |
| OFI-190 | **Filesystem breadth + path helpers** — `exists`/`delete`/`rename`/`mkdir`/`stat`/`append`; `join`/`basename`/`ext`/`dirname`. | **PARTIAL — `mkdir` + `remove` shipped 2026-07-11** (`std/fs.mkdir`/`std/fs.remove` over `em_mkdir`/`em_remove` FFI, for Quog's `.quog/` layout and `switch`'s working-tree checkout/prune). The rest (exists/rename/stat + the path-string helpers `join`/`basename`/`ext`/`dirname`) remain OPEN. |
| OFI-189 | **Seedable / ranged RNG** — `seed` + `rand_int(lo, hi)`; joins the nondet capture set (verification moat) and unblocks OFI-041's PRNG motivation. | OPEN |
| OFI-188 | **Time + sleep** — wall-clock/epoch time + a `sleep` that parks the *fiber* not the OS thread; wall-clock joins the nondet record/replay set. | **PARTIAL — epoch time shipped 2026-07-11** (`std/time.now()` → whole seconds since the Unix epoch, over a `em_now_unix` FFI; gated by `tests/run/time.ig`). Used for Quog commit/op-log timestamps. Remaining OPEN: the fiber-parking `sleep`, and record/replay CAPTURE of the clock (today `now()` is an uncaptured nondeterministic input). |
| OFI-187 | **`Option`/`Result` combinators** — `unwrap`/`unwrap_or`/`expect`/`is_some`/`is_ok`/`map`/`and_then`/`ok_or` on the prelude enums. Highest ROI-per-line. | **PARTIAL — UFCS enabler DONE 2026-07-08; combinators BLOCKED on OFI-204.** Karl chose UFCS (`opt.unwrap_or(0)` → `unwrap_or(opt, 0)`) as the call syntax. UFCS shipped in BOTH compilers, byte-identical: the C checker rewrites `x.f(args)`→`f(x,args)` on a NON-struct receiver whose name is a free function (a method always wins on a struct → no ambiguity; codegen shares the AST so no codegen change); the self-hosted checker accepts it (lenient) and the self-hosted VM codegen + C-emit each re-resolve it (`gen_ufcs`/`emit_free_call` with the receiver prepended) — byte-identical fixtures `tests/selfhost/{codegen,cgen_c}/ufcs.ig`, corpus `tests/run/ufcs.ig` (chaining + closure + string receiver), `make verify` green, `make selfhost` 1411/0. **The combinators themselves are BLOCKED**: they must be prelude functions (globally visible — `resolve_signature` would need the same `is_global_module` extension the prelude enums get), and prelude functions are emitted in EVERY program, so a combinator matching `Option<T>`/`Result<T,E>` trips **OFI-204** (the self-hosted C-emit mis-compiles a `match` on a generic Option/Result param) and breaks the whole cgen_c reproduction. So combinators land once OFI-204 is fixed (a targeted C-emit match-lowering change) + prelude functions are made global. **M1 DONE 2026-07-09 — OFI-204 CLOSED** (the self-hosted C-emit now constructs + matches prelude Option/Result byte-identically; foundation laid). **M2 DONE 2026-07-09 — the combinators ship in the C reference, usage-gated.** Delivery decided (Karl): the PRELUDE with a user-shadows-prelude rule — NOT an imported std module (proven: UFCS can't reach imported functions; unused-generic DCE means no bloat only in the NATIVE backend, but the VM emits every function, so — Karl's call, "skip unused prelude fns" — a combinator is injected ONLY when the program references it). Shipped in `src/{check.c,main.c}`: `resolve_signature` is two-pass (current-module shadows, then the global/prelude module) so `is_some(a)`/`a.is_some()` both resolve; `program_declares_fn` lets a user's own `unwrap_or` shadow the prelude's; `prelude_fn_dangles` drops a combinator whose enum the user redefined (`type_refs_shadowed_enum`); a `NameSet` of user identifiers gates injection (comment-safe token scan). The five leaf combinators — `is_some`/`is_none`/`unwrap_or<T: Copy>`/`is_ok`/`is_err` — work on BOTH backends via UFCS, and a user may shadow any (verified: user `unwrap_or` wins). `make test` 445/0, `make selfhost` 1420/0 (corpus unchanged — nothing references a combinator, so nothing is injected; all references in the corpus are comments/own-definitions). **M3 DONE 2026-07-09 — the five leaf combinators are COMPLETE end-to-end, byte-identical in BOTH compilers.** The self-hosted VM (`codegen_dump.ig`) + C-emit (`cgen_c_dump.ig`) + serializer (`serialize_dump.ig`) drivers inject the usage-gated + shadow-guarded combinators; `codegen.ig`/`cgen_c.ig` compile the generic combinator bodies byte-identically (the two erased-generic-codegen gaps this surfaced were fixed — see **OFI-205 CLOSED**). Gated: `tests/selfhost/{codegen,cgen_c}/combinators.ig` (both backends) + behavioural `tests/run/combinators.ig`; `make selfhost` 1429/0, `make verify` green. `is_some`/`is_none`/`unwrap_or`/`is_ok`/`is_err` all work via UFCS, user-shadowable, and cost nothing when unused (usage-gated injection + VM/native emission). **HIGHER-ORDER 2026-07-11:** `ok_or<T,E>(o, e)` (Option→Result, no closure) is byte-identical BOTH backends + gated. `map<T,U>(o, f)` + `and_then<T,U>(o, f)` (take a function param) are byte-identical BOTH backends with a NAMED-fn argument (gated in `tests/…/combinators.ig`) — they drove NAMED-fn closure support into the self-hosted C-emit (OFI-206: `em_closure`/`rt_call_closure`/fn-typed-param-owned). A LAMBDA argument to `map`/`and_then` works natively + on the VM but is not yet self-hosted-C-emit byte-identical (OFI-206 point 2: lambda lifting). The usage-gate was made PRECISE (call-position only: an identifier followed by `(`) so `map` — referenced in 36 corpus files as `Map<K,V>`/`.map`/`list.map` — injects only on an actual `map(…)` call. REMAINING leaf item: `unwrap`/`expect` (trap on None/Err) need a DIVERGING panic primitive (`assert(false)` is release-elided + non-diverging, so the None arm can't type-check) — a small language addition, deferred. Plan: docs/design/phase3-combinators-plan.md. |
| OFI-186 | **Pattern-matching richness** — literal patterns (`case 0`), guards (`case n if …`), or-patterns (`A \| B`), nested destructuring (`Some(Point(x, y))`) + exhaustiveness. The single biggest LLM-surprise in the language. Sequenced §1→2a→2b→2c→2d (docs/design/phase2-match-plan.md). | **PARTIAL** — §1 (PatternKind seam) + **2a literal patterns** + **2b guards & scalar value-binding** DONE 2026-07-06. 2a: int/string/bool subjects, `case 0`/`"x"`/`true`/`-1` (both compilers byte-identical, selfhost bytecode delta = OFI-200). 2b: `case pat if <bool>` (guarded arms don't cover; dual-false-path VM lowering + a `matched`-flag cgen_c restructure since guards break the if/else-if chain) + `case n` value binding (`PAT_BIND`) — **FULLY DONE in BOTH compilers**: C reference (both backends, all 8 `make verify` gates green) AND selfhost (parser + checker gated; cgen_c byte-identical, `tests/selfhost/cgen_c/match_guards.ig` 19/19; VM codegen opcode-identical + runs correctly). The selfhost-codegen value-binding blocker was RESOLVED (OFI-201 closed) via by-name classification + a cross-compiler collision guard — no OFI-163 needed. `tests/run/match_guards.ig`. **2c NON-BINDING OR-PATTERNS DONE 2026-07-08** — `case a \| b \| c` fires if any alternative matches; alternatives are non-binding (nullary variants or literals), each contributes to exhaustiveness, and an or-pattern may carry a guard. Both compilers: the VM lowers to an OR-of-tests (no OP_OR/JUMP_IF_TRUE — each alt's compare + JUMP_IF_FALSE, matched alts jump forward to the shared body, the last alt's JUMP_IF_FALSE is the pattern-false path threaded through `emit_match_arm_tail`/`gen_arm_tail` so guard convergence is inherited); cgen_c emits a `\|\|` disjunction. `PAT_OR` carries an `alts[]` list. Gated byte-identical: parser AST 619/619, `tests/selfhost/codegen/match_or.ig` (enum, VM bytecode), `tests/selfhost/cgen_c/match_or.ig` (enum+literal+guard, C-emit); behavioral `tests/run/match_or.ig`; all 8 `make verify` gates green. A pre-existing selfhost cgen_c gap for QUALIFIED nullary construction (`Color.Red`) surfaced en route → OFI-202 (fixtures use bare construction). **2d-i ONE-LEVEL NESTED DESTRUCTURING DONE 2026-07-08** (struct-inner cut, Karl's scope choice) — `case Some(Point(x, y))` destructures an all-scalar VALUE-STRUCT payload inline into its fields (irrefutable inner: a struct always matches; exhaustiveness unchanged); composes with guards, generic `Option<Point>` payloads, mixed nested+plain slots, and ignored `_` inners. Representation = an additive parallel `binding_pats[]` (NULL/kind-5-sentinel = plain name, else a recursive sub-pattern) so every existing pattern stays byte-identical. VM lowers to a double `GET_FIELD` (identical to `payload.field_j`); cgen_c unboxes to an `em_s` once + binds each inner name to an `em_s` Value field. Rejections (clear single errors): literal-in-variant `Some(0)` + depth>1 (parser), refutable enum-inner `Some(Ok(v))` → "use a nested `match`" + non-all-scalar struct + wrong struct name/arity (checker). Both backends identical; C `make verify` green; `tests/run/match_nested.ig` + `error_match_nested.ig`. Selfhost: PARSER byte-identical (AST 620/620 whole corpus) + CHECKER accepts (no false-reject), reproduction fixed point intact (`make selfhost` 1393/0); nested codegen/cgen_c logic mirrored but NOT fixture-gated — blocked on the pre-existing value-struct-enum-payload divergence → **OFI-203**. **2d-ii REFUTABLE ENUM-INNER DONE 2026-07-08** — `case Some(Ok(v))` destructures an ENUM payload one level (refutable: the inner tag may not match). VM lowers to an AND-of-tests (outer tag `&&` each enum-inner tag, short-circuited with JUMP_IF_FALSE into ONE bool for a single `next` — byte-identical to the plain single-tag test when there is no enum-inner, so the existing arm-tail machinery is reused unchanged); cgen_c ANDs `em_tag(em_enum_field(…)) == inner_tag` onto the condition + binds inner fields via em_enum_field. **Exhaustiveness is ONE LEVEL deep** (no product matrix): an outer variant is covered iff a plain/struct/wildcard arm covers it OR ALL of its single enum payload's inner variants are covered (`Some(Ok)`+`Some(Err)`+`None` is exhaustive; drop one → rejected); a plain fallback (`Some(other)`) also covers. Inner payloads limited to scalar/string (a struct/array-payloaded enum-inner is rejected). Both C backends identical; `make verify` green (opcheck for the AND-jumps, crucible/ASan for the string-inner refcounting); `tests/run/match_nested_enum.ig`. **Unlike struct-inner, enum-inner carries NO value struct, so it IS byte-identical-gated in the selfhost** — `tests/selfhost/codegen/match_nested_enum.ig` (VM bytecode) + `tests/selfhost/cgen_c/match_nested_enum.ig` (C-emit), `make selfhost` 1402/0. Adversarially designed (the checker restructures coverage so an enum-inner arm doesn't fully-cover its outer variant + a `nested_seen` flag keeps a plain fallback after nested arms from false-flagging as a duplicate). **OFI-186 (pattern-matching richness) COMPLETE** — §1 + 2a + 2b + 2c + 2d-i + 2d-ii, the full pragmatic subset. |
| OFI-185 | **Non-decimal integer literals + digit separators** — `0xFF` / `0b1010` / `0o755` + `_` separators, in ints and floats; leading-zero is decimal (no C octal footgun). | **CLOSED 2026-07-06** |
| OFI-184 | **Compound assignment** — `+= -= *= /= %= &= \| = ^=` as parse-time desugar to `p = p OP v`; no checker/codegen change. | **CLOSED 2026-07-06** |
| OFI-183 | **A spawned worker fiber ran with an UNINITIALISED VM: `worker_entry` (1:1 `EMBER_PARALLEL`) `malloc`'d the per-worker `VM` and set only a handful of fields, leaving the rest as heap garbage — the real cause of Inglenook's "Ollama model picker spins forever" (and the earlier intermittent Anthropic first-reply hang).** From the UI tape: switching Inglenook to Ollama animated a "Discovering models" spinner ~15 s then never resolved (frames climbing = UI alive, the discovery *worker* dead). Any `.ig` that does its work on a `spawn`ed fiber hits it — reproduced headlessly with a bare `spawn disco_worker` round-trip. The decisive field is **`reentry_floor`**: `OP_RETURN` returns from `run()` exactly when `frame_count <= vm->reentry_floor` (0 for the main VM, set at vm.c ~4399). `worker_entry` never set it, so it was garbage: **≥ 1** → `run()` returned EARLY when a nested call popped (the worker abandoned `discover()` before its `send`, so `recv` on main blocks forever = the hang); **< 0** → disco_worker's own final `RETURN` (frame_count 1→0) failed the `0 <= floor` test, so `run()` ran ON past frame_count 0 into `frames[-1]`, whose `ip` is garbage (null under ASan → SEGV at the dispatch `*frame->ip` vm.c:2076; junk at `-O1` → misexecute then hang). Same class for the other uninitialised control fields (`nondet_mode` → FFI calls wrongly take the record/replay path, `capturing` → output to a garbage `cap_buf`, `exit_requested`, `check_mode`, `route_hop_count`). **"Intermittent / DNS-sensitive" was the uninitialised value in disguise:** the garbage in a freshly-`malloc`'d VM depends on heap history, and `localhost`'s macOS threaded resolver (extra allocations) shifts it, so failure rate tracked the base URL rather than any real race. The MAIN VM was fine (it initialises `reentry_floor` + the control fields); the M:N worker path is fine (it copies an initialised VM via `*w = *vm`). Signature = [[ember-arena-node-init]] (a non-zeroed struct whose every field must be set at its creation site). | **FIXED 2026-07-06 (`src/vm.c` `worker_entry`).** One line: `memset(w, 0, sizeof *w);` right after the `malloc`, so a worker VM STARTS FULLY ZEROED like the main VM, then the explicit assignments set the non-zero fields (zero-then-set is strictly safe — every field it zeroes is meant to be 0 at a fiber's start). Verified against the headless `spawn` round-trip: **25/25** clean at `-O1`/`localhost` (was ~6/8 hang) and **15/15** under ASan (was crashing ~every run); parallel golden suite green. **Correction to this entry's original (wrong) diagnosis:** it was first reported as a `std/json` DOUBLE-FREE, but that was an artefact of the diagnostic instrumentation — the `EMBER_DROP_TRACE` detector stamps a sentinel AFTER `pooled_free` (a write-after-`free()` on oversized objects under ASan), and a home-grown pointer-ring detector false-positived on the pool's normal free→reuse→free of small objects. Under a proper **pool-bypass ASan** build (every object a real `malloc`/`free`) there is **NO heap double-free or UAF** — the corruption is purely the uninitialised-VM logic bug above, not a memory-safety violation of any heap object, and not in `std/json`. Added an inert `#ifdef EMBER_POOL_BYPASS` in `runtime.c`'s `pooled_alloc` (companion to `EMBER_DROP_TRACE`) so the pool can be bypassed to let ASan see true object lifetimes next time. Follow-ups: add a `tests/parallel` regression that does real work (FFI + json) on a spawned fiber; consider `memset`-then-set for the main + M:N VM setups too, as a belt-and-braces guard against the next new VM field. Two unrelated libcurl hardening fixes landed en route (committed `10ae2c4`, NOT part of this bug): `CURLOPT_NOSIGNAL` on all handles + `CURLOPT_CONNECTTIMEOUT` on the streaming/POST handles. See [[inglenook-ide]]. |
| OFI-182 | **On macOS, the render thread can DEADLOCK inside the OpenGL buffer-swap when enough worker fibers churn channels alongside the 60fps GL present.** Surfaced building Inglenook (`public/inglenook`, the `inglec-net-gfx` 1:1 build): under load the app would occasionally hard-freeze — the UI tape's frame counter stalls mid-frame while work is pending. Sampling the frozen process (`sample <pid>`) shows the MAIN thread wedged in `EndDrawing → glfwSwapBuffers → CGLFlushDrawable → SLSFlushSurface → mach_msg`, i.e. blocked forever waiting on a WindowServer reply that never comes. Reproduced WITHOUT std/proc (my first hypothesis — that `posix_spawn` was corrupting the WindowServer connection — was a RED HERRING, fully reverted): a pure program with graphics + N echo-worker fibers hammering channels every frame freezes identically, and it triggers with as few as ~3 workers under aggressive per-frame channel traffic. So the cause is contention between the raylib/OpenGL present (which crosses into SkyLight/WindowServer via mach IPC) and heavy worker-thread + channel activity — a macOS windowing/GL-driver interaction, not an Ingle-runtime memory bug (ASan-clean; both 1:1 and M:N builds affected). LLM-first relevance: an LLM-coding IDE built in Ingle wants many background fibers (per-tool workers), and this caps how freely they can run alongside the GUI on macOS. | **OPEN — mitigated, not eliminated.** Inglenook was hardened (`3d06c8c`) by folding its 3 tooling workers into 1 (6→4 fibers) and cutting per-frame channel drains 5→3, which sharply reduces how often the contention window is hit — but the underlying deadlock is still reachable under enough load. Proper fixes to investigate (measure-first): (a) confine ALL GL/WindowServer calls to a single thread and ensure no other thread holds a lock the present path needs while it blocks in `mach_msg`; (b) batch/throttle main-thread channel drains so the present isn't racing a burst of `try_recv` every frame; (c) check whether a global runtime lock (allocator / refcount / channel) is held across the GL present — if so, drop it before `EndDrawing`; (d) try `NSOpenGLContext`/CVDisplayLink or a triple-buffered swap to decouple the present from the render loop. Instrument next time via `EMBER_TAPE` (frame counter stalls = frozen vs climbing = alive-waiting) + `sample` for the `CGLFlushDrawable` signature. See [[inglenook-ide]]. |
| OFI-181 | **An interpolation-parse error inside a string literal mis-reported its location — it was attributed to line 1 of the ENTRY file with a garbled `unexpected character (near '\')`, not the offending string.** Writing an un-escaped `{` in a string (e.g. a code snippet `"fn f() {\n ...}"`, or JSON `"{\"k\": 1}"`) opens a `{…}` interpolation hole; `build_string_parts` (parser.c) re-lexed the hole's raw contents standalone with `lexer_scan`, which carried the sub-TOKENS' positions back to the file but NOT the sub-LEXER's diagnostics — so a hole that failed to lex (raw text with a `\`) emitted "unexpected character" errors bunched at hole-relative line 1. An LLM-first wound: a model writing a string that contains braces/JSON/code got an error pointing at the wrong line saying something untrue. Surfaced repeatedly building Inglenook's test harnesses. | **CLOSED 2026-07-05.** Root cause was that `diag_error` PRINTS immediately in human mode (only buffers in `--diagnostics=json`), so a post-hoc `diag_truncate` couldn't work — replaced with a nesting **`diag_mute(on)`** (diag.c): `build_string_parts` mutes the SPECULATIVE hole re-lex, and on `sub.had_error` emits ONE clean error at the `{`'s true `file:line:col` with the `\{` hint (works in both human + JSON mode). Separately, the parser's primary-expression `default` now recognises a `TOK_ERROR` token (already reported by the lexer) and recovers quietly + enters panic instead of piling on a second "expected an expression" cascade. A genuine bad-expression hole (`"x{ + }"`) still locates correctly (the parser uses the offset sub-tokens); valid + nested-string interpolation unaffected. Test `tests/run/error_interp_brace.ig`; 435/0 + selfhost 1362/0. **Residual (minor, non-misleading):** the fully-UNTERMINATED case (`"a{b` with no close) still shows one trailing `expected '}' to close a block` at EOF, because `scan_string` (lexer.c) consumes the unterminated interpolation to EOF, eating the rest of the file — bounding that consumption is a separate lexer-recovery change (regression risk), deferred; the PRIMARY error there is already correct + well-located ("unterminated interpolation … write `\{`"). |
| OFI-180 | **The syntax highlighter tokenises PER LINE, so a `/* … */` block comment spanning multiple lines is mis-coloured** — `hl.spans(lang, line)` is called independently per source line in both the read-only code panel (`_paint_code`) and the new editable editor (`_paint_code_editor`), so a `/*` on line 5 closing `*/` on line 7 highlights lines 5 and 7 as comment fragments and line 6 as plain code, instead of one comment span. Inherent to line-at-a-time highlighting, which is what makes editor virtualization cheap (only visible lines are re-highlighted). Pre-existing in `f.code`/markdown fenced blocks; `f.code_editor` (Inglenook Phase 2) inherits it by design. Minor for Ingle (whose `//` line comments dominate) but wrong for C/JS-heavy files. | **OPEN** — accepted limitation while line-at-a-time highlighting stays (the virtualization win). Fix when it bites: carry a 1-bit "in block comment" state across the *visible* line range (cheap — scan only from the first visible line, or precompute per-line block-comment-open flags once per buffer change and cache), so a multi-line `/* */` colours correctly without re-highlighting the whole file every frame. |
| OFI-179 | **The self-hosted compiler resolves type names and enum-variant names GLOBALLY by first-match, but stage-0 resolves them MODULE-SCOPED — the last flare byte-identity gap.** The self-hosted codegen's type/enum tables (`st_names`, `et_names`, `ev_name`/`ev_tag`) are the merged (all-module) lists, and resolution is a bare-name `cg_index_of` that takes the first match regardless of the defining module. Two concrete failure modes, both surfaced by `std/flare.ig` (which imports `std/markdown` AND `std/highlight`): **(a) type-name collision** — `struct Span` (highlight) and `enum Span` (markdown) coexist; `elem_type_code`/`ty_struct_id_g` check structs before enums, so `md.inline() -> [Span]` (the markdown ENUM) mis-resolves to highlight's inline STRUCT `Span`, and `Flare.rich_text`'s `match spans[si]` subject is then treated as an inline-struct owning temp (an extra `DROP` at the match close where stage-0 `POP`s) — cascading every downstream jump (355 diff-lines). Minimal repro: module A `struct Span{..}`, module B `enum Span{..} + fn inline()->[Span]`, a third module that imports both and matches `inline()`'s element. **(b) enum-variant-tag collision** — a `Str`/`Node`/`Value` variant defined in several co-imported enums (e.g. `Json.Str`) resolves to the FIRST enum's tag (`CONST 2`) instead of the scrutinee's enum tag (`CONST 4`) in `_stringify`/`as_str` — variant lookup is not scrutinee-directed across modules. Stage-0 resolves both correctly because the checker threads the resolved `(enum_id, tag)` / module-scoped type onto the AST (cf. OFI-073). | **OPEN** — the "cross-module resolution" milestone already flagged in `selfhost/emberc.ig`. The fix is module-scoped resolution: record each struct/enum/variant's defining module (parallel to the existing per-decl `mod_of` + `cur_module` threading used for module-local free-fn resolution), and disambiguate a name by the resolution CONTEXT's module (a return-element type by the function's own module in `build_fn_rets`; a match/construct variant by the scrutinee's enum). ~815 of flare's remaining ~2719 diff-lines (rich_text 355 + _stringify 307 + as_str 153) are this class; the rest is `DockTree.to_json`'s array-element-field `.len()` (tracked separately). |
| OFI-178 | **Stage-0 codegen skips the drop-on-reassignment of a `var` whose PHYSICAL slot is ≥ its LOGICAL index — an over-retain leak in the reference compiler.** `STMT_ASSIGN` to a plain identifier releases the slot's old owned value before overwriting it (`if (cg->local_drop[slot]) emit DROP`), but `slot` came from `resolve_local` (the PHYSICAL base slot) while `local_drop` is a LOGICAL-indexed array (`cg_declare` writes `local_drop[local_count]`). In a function with a multi-slot struct/value local before the reassigned `var`, the physical slot outruns the logical index, so `local_drop[physical]` reads a stale/foreign entry — usually `false` — and the drop-on-reassign is silently skipped, leaking the old string/array/enum ref (the exit-drop at scope close, which correctly iterates logical `i` and drops `local_phys[i]`, still fires, so it is a pure over-retain, never a double-free). Surfaced by the self-hosting byte-identity campaign on `std/flare.ig` `Flare._paint` (`var lbl = text; lbl = parts[0]` at physical slots 31/32): the self-hosted codegen indexes its `local_drop` by physical slot and so does the *correct* thing (emits the drop), diverging from stage-0 by 4 bytes and cascading every downstream JUMP target (~64 hunks). Minimal repro: a 4-field struct local `let p = P{…}` before `var lbl = text` then a conditional `lbl = parts[0]` — stage-0 omits the `DROP` the self-hosted emits. | **FIXED 2026-07-04** — the reassign now checks the flag by LOGICAL index (`resolve_local_logical`) while keeping the PHYSICAL slot for the `DROP`/`SET_LOCAL` operands, exactly mirroring the correct exit-drop loop. Stage-0 now emits the missing drop (fixing the leak) AND becomes byte-identical to the self-hosted backend. No `.out` golden changes (releasing an already-leaked refcount earlier is not observable). |
| OFI-177 | **The self-hosted codegen over-drops an inner `match`-arm binding at a `return` that FOLLOWS the match, in a deeply-nested loop/match/if body.** Surfaced adding the generic-struct-field method-call feature (`self.wins.keys()`): the new `register_field_method` had the shape `loop { match decls[i] { case DStruct(name, generics, impls, fields, methods, kind) { if name==sname { loop { if fields[fi].name==fname { match fields[fi].ty { case TyGeneric(qual, gname, args) {...} case _ {} } ; return } } } } } }` — at the `return` after the inner match, the self-hosted backend emitted an extra `DROP 13` (slot 13 = `gname`, the inner match's middle STRING binding, whose scope had already closed) that stage-0 does NOT, a 2-byte divergence in that one function. Stage-0's drop set at the return is correct; the self-hosted backend leaks the closed inner-arm's string binding into the enclosing return's drop set. **Not minimally reproducible** — three faithful reconstructions (free-fn, method, 3-field enum with an unused 3rd binding + a re-read of the scrutinee) all stayed byte-identical, so the trigger depends on the specific slot layout (6 outer bindings pushing the inner-arm bindings to slots 12–14) — a subtle slot-accounting interaction in the match-binding scope/return-drop machinery, same family as OFI-118 (owning `match` scrutinee leaked on an early case exit). | **WORKED AROUND 2026-07-04** — removed the inner `match` from `register_field_method`: `ty_args_key(fields[fi].ty)` is non-empty only for a generic type, so a `let targs = ty_args_key(...)` + `if targs != ""` guard + `ty_key_name(...)` for the base name registers `Map.keys<int_int>` with no inner match / no return-after-match. Cleaner code, and codegen.ig is byte-identical again. The self-hosted drop-tracking bug itself is OPEN (low priority — only reachable with this exact nesting+slot shape; the language accepts the pattern, only the self-hosted backend mis-drops; needs a scoped audit of where match-arm string bindings are removed from the owning-drop set when an arm closes vs. when an enclosing `return` computes its drop set). |
| OFI-176 | **The self-hosted codegen mis-compiles a nested array-returning CALL passed directly as a call argument** (`self.gen_struct_fields(sid, fields, line, ty_type_args(ty.value))` — where `ty_type_args` returns `[ps.Ty]`): the self-hosted output diverged from stage-0 by ~3 bytes (a missing INCREF / arg-staging difference on the inner call's array result) in `Chunk.gen_struct_construct`. Same owning-temp-as-argument family as the OFI-166/173 `arr[i] = other[j]` gaps — the self-hosted arg-staging discipline for a fresh owned-array temp produced by an inline call doesn't match stage-0's PICK/DROP_UNDER. Surfaced while adding the generic-struct empty-array-field element-kind substitution. | **WORKED AROUND 2026-07-03** — bind the inner call to a local first (`let targs = ty_type_args(ty.value); self.gen_struct_fields(..., targs)`), the same safe pattern the OFI-166/173 notes prescribe. The self-hosted arg-staging bug itself is OPEN (low priority — a targeted codegen fix for a fresh-array-temp argument; the language accepts the pattern, only the self-hosted backend mis-stages it). |
| OFI-175 | **The self-hosted RUNNABLE path (`serialize.ig` → `.igb`) DIVERGED from the byte-identity path (`codegen.ig` `disassemble_program` → `--emit=bytecode`): `serialize_program` was a 1-pass stopgap that emits ONLY declared fns/methods — it never got the lifted-lambda + generic-instance machinery `disassemble_program` grew.** So a program using inline lambdas serialized to a BROKEN image (`lambdas.ig` → the `.igb` omits all 7 `<lambda>` functions entirely → runs to empty output), and a bounded generic serialized without its witness instances. This is the ROOT CAUSE beneath much of OFI-174's run-correctness failure: cgdiff (the `make selfhost` codegen gate) only exercises `disassemble_program`, so the serializer's divergence was invisible to it — the compiler's OWN modules happen to use no lifted lambdas and no divergent free-fn instances, so the 38 serialize byte-identity fixtures stayed green while arbitrary lambda/generic PROGRAMS silently mis-serialized. Two compilation drivers, one gated, one not — the same two-sources-of-truth hazard as [[ember-opcode-operand-gotcha]] / OFI-153. | **CLOSED 2026-07-03 — `serialize_program` unified with `disassemble_program`'s 4-pass structure** (count lambdas → declared fns → lifted lambdas → generic instances), emitting each chunk to the `.igb` Writer instead of the disassembler. `func_count` now includes lambdas + instances; lambdas serialize as `<lambda>` with the enclosing fn's source (matching stage-0). Verified: `lambdas.ig` + `generic_copy/fn/infer.ig` now RUN byte-identically via the `.igb` path; the 38 serialize fixtures + full `make selfhost` gate stay green. The two drivers still duplicate the 4-pass loop (Ingle's ownership model forbids holding `Chunk`s across passes to factor a shared `[CompiledFn]`); kept structurally identical so drift is visible. This unblocked the OFI-174 witness work (witnesses now emit through the shared `compile_fn`, so both paths get them). |
| OFI-174 | **The self-hosted bytecode backend (`selfhost/codegen.ig`) SILENTLY MISCOMPILES bounded/witness generics (Tier 3) and higher-order-fn / array-through-generic code (Tier 2) — it emits a runnable `.igb` with NO error or checker rejection, but the image computes WRONG results or crashes.** Verified 2026-07-03 by run-correctness testing (NOT cgdiff — the disassembly differs but, more importantly, the images actually *run wrong*), adversarially re-confirmed. **Tier 3 (wrong answers, silent):** `bounded_generic.ig` `max<T: Ord>` never emits the `CALL_INDIRECT` witness dispatch — stage-0 lowers `a.compare(b)` via the Ord witness; the self-hosted backend lowers it to `CONST 0; GE` (`0 >= 0`, always true), so `max(3,8)`→3 and the program returns 11 not 16. `multibound_generic.ig` `describe<T: Named+Aged>` never calls `name()`/`age()` (prints the raw receiver `<obj>` not `"Ada"`, and leaks an uninitialised pointer that VARIES run-to-run). `hash_eq_bound.ig`→1000000001 not 3000000000; `generic_bounded_ctor.ig` builds `Bag<K>` without its witness slots (`NEW_STRUCT` too few fields)→count 0 not 3. **Tier 2 (crashes):** `generic_hof_strings.ig` SIGSEGVs (exit 139, zero output) — erased-generic-body retain/drop underflows refcounts on string elements the caller still owns (the OFI-015 use-after-free class); `map_array_value.ig` faults `index_out_of_bounds` — an array stored as a `Map` value through an erased generic is not value-semantics deep-cloned, so it comes back empty. Root cause: the self-hosted codegen has **zero witness-dictionary machinery** and an incomplete erased-generic memory model. NB: this **contradicts the `selfhost/emberc.ig` header's claim** that generic divergence is "the one residual numbering difference — internally consistent, so the image still runs correctly" — true for Tier 1/1b/1.5, FALSE for Tier 2/3 (header corrected 2026-07-03). | **OPEN — the Tier 2/3 self-hosting-completeness campaign; the honest gate between today and a self-hosted compiler that can compile the full language.** Needs, in the self-hosted bytecode backend: (a) **witness-dictionary codegen** — `NEW_ENUM` witness dicts as hidden struct fields + as hidden leading call args, `GET_FIELD` + `CALL_INDIRECT` dispatch, and monomorphized bounded methods; (b) **erased-generic-body retain/drop discipline** — conditional `INCREF` on erased-T reads that feed a closure call, `DROP` of shifted elements; (c) **value-semantic deep-clone** of aggregates (arrays/structs) passed through erased generics; (d) a **mini type-inferencer in the mono pre-pass** to key instances by element/annotation type (`list.map<[int]>` vs `<[string]>` — current keying is on literal arg-kinds only, which can't see a variable's type). Tiers 1 (`generic_copy`), 1b (`generic_fn`), 1.5 (`generic_fn_infer`) are DONE + byte-identical. **Strategic reframe (2026-07-03, precedent-checked): the acceptance test for these is CORRECT BEHAVIOUR (matches stage-0's golden `.out` when run) + the self-reproduction fixed point — NOT permanent byte-identity to stage-0**, which no self-hosting language treats as its finish line (it is a transitional oracle everywhere it appears — Go's C→Go 1.5 transition — and is dropped once the port is trusted). Byte-identity stays the convenient DEV oracle while stage-0 is the reference. |
| OFI-173 | **The self-hosted C-emit backend (`selfhost/cgen_c.ig`) SILENTLY lowers an unsupported expression to `INT_VAL(0)` instead of erroring** — so a construct it can't emit becomes valid-but-WRONG C (a literal 0) rather than a compile failure. Concretely surfaced twice: `f.enss[ek].value` (field access on a boxed generic `Box<Expr>` array element, the contracts campaign) and `self.ext_pquals[j].bytes()` (`.bytes()` on an indexed string-FIELD element, the FFI campaign) both emitted `Value v = INT_VAL(0)` where stage-0 emitted the real `em_enum_field(...)` / `em_str_bytes(em_index(...))`. The bytecode backend (`codegen.ig`) lowers both correctly — it is specifically the C-emitter that has the hole, and it fills it with `0` rather than failing loudly. The only thing that catches it is the Stage 6 C-emit reproduction / `ccdiff` byte-identity; a construct outside the gated module set would ship wrong C undetected. | **OPEN (worked around each time; the silent-fallback is the real hazard).** Both instances fixed by restructuring the self-hosted source to stay in the shared subset both backends emit identically (contracts → flat `[Expr]`/`[int]` arrays not `[Box<Expr>]`; FFI → bind the string to a local before `.bytes()`). General fix (deferred): `cgen_c.ig`'s expression emitter should emit a hard `internal_error`/abort on an unhandled expression kind (mirror stage-0's `cgen.c` `internal_error`), NOT a `0` placeholder, so a coverage gap fails the build instead of miscompiling. The two lowering gaps themselves (boxed-generic-array-element field read; `.bytes()`/method-call on an indexed field element) are the sibling feature work. |
| OFI-172 | **Self-hosted `build_enums` (`selfhost/codegen.ig`) injected a DUPLICATE prelude `Option`/`Result` when the program REDECLARED them** — so a self-contained program that writes `enum Option<T> { Some(value: T) None }` (a common teaching idiom, e.g. `tests/codegen/channel.ig`, and needed to make `recv`'s `Option` explicit) produced a serialized `.igb` whose enum table carried a phantom second `Option` (Some/None listed twice) vs stage-0's single one. Stage-0 uses the user's declaration AS the prelude enum and adds no duplicate. Latent (the phantom enum id was never referenced, so the `.igb` still RAN correctly) but a real self-hosting-fidelity divergence — the self-hosted compiler's `.igb` didn't byte-match stage-0's for any Option/Result-redeclaring program. **Invisible to cgdiff** (the `--emit=bytecode` disassembly omits the enum table, same blind-spot class as OFI-171); surfaced only via the Stage 8 `.igb` byte-identity, and only once a `tests/selfhost/codegen/` fixture declared its own `Option`. | **CLOSED 2026-07-03 — found landing the concurrency codegen** (the fixture `tests/selfhost/codegen/concurrency.ig` declares `enum Option<T>`; its `.igb` differed from stage-0's at the enum-table header — read the actual bytes with `xxd` rather than guessing). Fix: `build_enums` now appends a prelude `Option`/`Result` only when `cg_index_of(en, "Option"/"Result") < 0` (the user hasn't declared it), matching stage-0's dedup. Enum-id numbering stays exact (a redeclared `Option` is id 0, `Result` the next id). Verified byte-identical `.igb` on channel/option/spawn_immediate + the fixture; modules + gate green (1307/0). |
| OFI-171 | **The codegen disassembly differential (`tools/cgdiff.sh`, the `--emit=bytecode` diff that gates the self-hosted codegen) is BLIND to the string pool — a divergence confined to string-pool CONTENTS passes it silently.** `--emit=bytecode` prints opcodes + operands + inline CONST values but NOT the per-function string pool, so cgdiff (and the 589-file corpus sweep) compare everything *except* the strings. Surfaced landing self-hosted **contracts**: the self-hosted codegen emitted `OP_CONTRACT_CHECK <idx>` with the right operand but never added the synthesized violation-message string (`"postcondition failed in '<fn>' (ensures, line N)"`) to the pool, so cgdiff reported PASS=1 DIFF=0 while the actual `.igb` string pool was short — at runtime `chunk.strings[msg_index]` indexed out of bounds and the container SEGFAULTED (exit 139). The disassembly differential could never have caught it; only the Stage 8 serializer byte-identity (which serializes the whole pool) can. | **OPEN (tooling blind-spot; the contract instance is now guarded).** Fixed the contract codegen (messages are now `add_string`'d and their pool index is the operand, byte-identical to stage-0) and added `tests/selfhost/codegen/contracts.ig` so **Stage 8** (self-hosted serialize.ig `.igb` vs stage-0 `--emit=bytecode-bin`, a full-byte `cmp`) byte-compares the contract string pool — 33/33. General fix (deferred): either teach `cgdiff` to also diff the string pool, or ensure every string-pool-bearing construct has a fixture in `tests/selfhost/codegen/` (Stage 8's corpus) so the byte-identity gate covers it. Coverage asymmetry to close: cgdiff spans 589 files but skips strings; Stage 8 is full-byte but ~33 files. |
| OFI-170 | **The embedded body font (Inter, font 0) ships a PARTIAL glyph subset — several Geometric-Shapes / dingbat codepoints render as tofu "?" in the Flare/Claude UI.** Confirmed tofu: `✕` U+2715 (dock panel CLOSE button → "Conversations ?" / "Inspector ?"), `▾` U+25BE (dropdown chevron → "Haiku 4.5 ?"), `▸` U+25B8 (submenu disclosure), `☰` U+2630 (`☰ Chats` re-dock button), `ⓘ` U+24D8 (`ⓘ Inspector`), `▌` U+258C (the streaming caret). Confirmed PRESENT: `× ■ → ↓ · — … − ⌘` (so Latin-1 punctuation, arrows, `⌘` U+2318, and even `■` U+25A0 are fine — the subset is per-codepoint, NOT per-block, so you can't predict from the block). Surfaced 2026-07-02 when the new `dropdown` primitive's `▾` showed as `?` in a screenshot; the pre-existing `✕`/`☰`/`ⓘ`/`▌` were the same class. | **CLOSED 2026-07-02.** Chevrons are now DRAWN as vector triangles from 1px `fill_round` bars (`std/flare._tri_down` / `_tri_right`) — font-independent, so `dropdown`/`submenu` disclosures always render; the dock close button uses `×` U+00D7 (Latin-1, in-subset) not `✕`; the app's `☰ Chats`/`ⓘ Inspector` became plain "Chats"/"Inspector" and the caret `▌`→`|`. Verified by `EMBER_CAPTURE=<png>@<frame>` PNG snapshots (the reliable "look at the actual pixels" instrument for a raylib window). Rule going forward: **for a UI glyph, either draw it or verify it against the embedded subset with an `EMBER_CAPTURE` snapshot — do not assume a Unicode codepoint renders.** Goldens `flare_menubar`/`flare_widgets` re-blessed (chevron text ops → triangle bars); 47/0 graphics. (Deferred: embedding a fuller font subset — not worth it while draw-the-glyph works.) |
| OFI-169 | **Flare menu bar (`menubar_begin`/`menu`/`menu_item_accel`/`menu_sep`/`submenu`) — the first-cut immediate-mode open/close state machine could show two menus (or two submenus) open for one frame.** The new top menu-bar primitive (`std/flare.ig`) tracked the open menu/submenu in raw state (`__mb_open` / `__mb_sub`) and *mutated it mid-pass*: a `menu()`/`submenu()` declared earlier in the frame read the state, then a later one's click/hover handler rewrote it — so on a hover-switch (File→Edit, or Export▸→Tools▸) BOTH the old and new dropdown queued a `_POPOVER_BEGIN` in the same frame (double-open / two titles lit), self-correcting the next frame. Two lesser flaws alongside: a menu popover opened at `(0, bar_h)` instead of under its label when the label's last-frame rect didn't exist yet (reachable via `set_str` seeding — asymmetric with `submenu`'s `&& have` guard); and `menu_item_accel`/`submenu` packed `label\taccel` into one string that the paint split on `== 2` parts, so a tab embedded in a label left the accel unrendered. | **CLOSED 2026-07-02 — found by a 5-lens adversarial review of the menu-bar work (state-machine / submenu / paint lenses), all 5 findings verified, fixed same session.** Root fix is a **per-frame snapshot**: `menubar_begin` snapshots `__mb_open`/`__mb_sub` into `__mb_open_f`/`__mb_sub_f`; `menu()`/`submenu()` decide what to SHOW from the snapshot while click/hover writes the persistent state for NEXT frame (input-this-frame → state-next-frame). One `shown` string ⇒ exactly one menu open by construction — no double-open, no double-highlight — which also subsumes the `&& have` position guard (now applied). Labels/accels are tab-sanitized (`str.replace`) so the paint split is always exact. Golden `tests/graphics/flare_menubar.ig` (45/0 graphics), both files typecheck; integrated + tape-verified in `public/claude-desktop/flare_chat.ig`. |
| OFI-168 | **Taking a foreign (`extern "c"`) function as a FUNCTION VALUE crashed the VM (SIGSEGV) / emitted `em_closure(-1)` on native** — a pre-existing FFI soundness hole. The `EXPR_IDENT`→function-value path (`src/check.c`) guarded generic and multi-slot signatures but NOT externs; an extern has `fn_index == -1` (no bytecode slot), so `let f = sin` (a hosted-registry extern) or `let f = uart_putc` (a native direct-extern, OFI-167) set `fn_value = -1`, which codegen encoded as `OP_MAKE_CLOSURE 0xFFFFFFFF` / `em_closure(&g_em, -1, 0)` → the VM indexed `prog->functions[-1]` out of bounds and segfaulted (exit 139); native emitted UB. Registry externs were already affected; the OFI-167 direct-extern work widened the surface. | **CLOSED 2026-07-01 — found by the OFI-167 adversarial review (integration-edge lens), verified pre-existing (registry `sin` crashed identically, exit 139), fixed same session.** The fn-value path now rejects any extern (`cextern_index >= 0 || direct_extern`) with "a foreign (extern \"c\") function cannot be used as a function value; call it directly" — the same spirit as the existing spawn-extern guard. Regression `tests/run/error_extern_fn_value.ig`; 428/0 golden, 1209/0 selfhost. |
| OFI-167 | **`extern "c"` FFI cannot call a bare-metal / arbitrary C function — it is a closed, index-keyed registry of hosted libc/libm symbols.** Both the VM and the native C-emit backend dispatch every extern call through the in-tree registry (`src/cextern.c` `g_sigs`/`g_fns`) **by index**: `src/check.c` rejects any name not found by `cextern_lookup` ("unknown C function"), and `src/cgen_c.c` emits `em_ffi(&g_em, <index>, …)` — the C symbol *name* never reaches the emitted C. So Ingle has no way to call a kernel MMIO helper (`uart_putc`) or any C symbol outside the curated list, which blocked the very first kernel/freestanding milestone (a heap-free `main` writing to the PL011 UART). Surfaced 2026-07-01 on the first bare-metal spike — exactly the "first real compiler task" the kernel brief predicted. | **CLOSED 2026-07-01 — native direct-extern shipped** (Karl chose the real fix over the registry stopgap). An `extern "c"` fn NOT in the hosted registry is now a *direct extern*: the checker registers it (scalar/Ptr params + return only — string/buffer/struct still need a registry entry), the native backend forward-declares it with its EXACT C type and emits a **direct call to the named symbol** (`src/cgen_c.c` `emit_direct_extern_call` + `ember_ctype_of`), linker-resolved against a freestanding shim; the VM rejects it with a clear "no bytecode-VM binding; build native" message (native-only). Drove kernel milestone 1 (`make test-kernel`: "Hello from Ingle!" on QEMU `virt`). 427/0 golden (incl. `tests/run/error_direct_extern_vm.ig`), 1209/0 selfhost, registry FFI unaffected. Residual (deferred): string/buffer/struct direct-externs; MMIO intrinsics (volatile load/store, no C shim) remain the eventual endgame. |
| OFI-166 | **Native backend: a binary-op / call operand with a SIDE EFFECT evaluates in a different order on native than on the VM** — a silent VM/native divergence, and platform-dependent within native (clang vs gcc). The VM evaluates `a + b` (and call args) LEFT-TO-RIGHT (stack order); the C backend lowers them to `em_add(<a>, <b>)` / `em_fn(<a>, <b>)` and inherits C's UNSPECIFIED argument-evaluation order — **gcc evaluates right-to-left, clang/the VM left-to-right**. So when an operand mutates shared state (e.g. bumps a counter), the two orders diverge. Surfaced by the M5 self-hosted C-emit (`selfhost/cgen_c.ig`): `emit_binary` emitted both retain-temp operands in one expression, each calling `fresh_var()`; the gcc-built `cgen_c_dump` SWAPPED the `v1`/`v2` numbers vs the VM, so its `--emit=c` output diverged on Linux gcc — caught by the CI self-hosting differential (`make selfhost` Stage 6), not macOS. | **OPEN** — `src/cgen_c.c` should evaluate a binop's / call's operands into temporaries in SOURCE ORDER before combining, so native matches the VM's left-to-right semantics regardless of the C compiler. Worked around in `selfhost/cgen_c.ig` by sequencing the operands into `let`s (statements are ordered) — the general rule for any self-hosted code that side-effects within one expression. (Ingle intends left-to-right evaluation, like the VM; this is a real native soundness gap, pre-existing, surfaced here.) |
| OFI-165 | **Self-hosted codegen (M4): a method call does not apply the owning-temp keep+drop discipline to its arguments** — an inline owning-temp ARRAY arg passed to a method's BORROW parameter (`obj.m(clone_bools(x))`) leaks (no `PICK`/`DROP_UNDER`), diverging from stage-0. `gen_method_call` pushes args via `gen_call_args`, which (unlike `gen_user_call`/`gen_builtin_call`) lacks the keep-below + PICK-as-borrow-alias + DROP_UNDER-from-under-the-result dance. Surfaced building the M3 checker's Ptr leak scan (`SBreak` AND-merge): `self.merge_into_break(clone_bools(self.local_consumed))` emitted one fewer instruction than stage-0 and shifted every downstream offset by 3 bytes (cgdiff caught it). The receiver `self` is already on the stack when the args push, so the keep-below interleaving is non-trivial. | **OPEN** — `selfhost/codegen.ig` `gen_method_call` should route args through the same owning-temp discipline as `gen_user_call` (keep the temp below the args, PICK a borrow alias, DROP_UNDER after the call), accounting for the pre-pushed receiver. Worked around in `selfhost/checker.ig` by binding the clone to a named local (`let snap = clone_bools(…); self.merge_into_break(snap)`) — the file's established idiom; the inline-temp form was the lone outlier. Pattern appears nowhere else in the self-hosted compiler. |
| OFI-164 | **Self-hosted codegen (M4): a non-empty array literal of INLINE-struct elements (`[t]` where `t: Tok`, Tok all-scalar/refcounted-no-unique-owner) emits `NEW_ARRAY` (boxed) instead of `NEW_STRUCT_ARRAY` (inline-packed).** Stage-0 packs `[tok_value]` inline (`NEW_STRUCT_ARRAY <count> <sid>`); the self-hosted `EArray` lowering always emits `NEW_ARRAY <count> <elem_kind>`. Surfaced writing the `bindings_arrays` fixture (a `kids: [t]` field value). NOT on parser.ig's critical path — the compiler builds arrays via `.append()` in loops, never struct-literal arrays — so low priority. | **OPEN** — `selfhost/codegen.ig` `EArray` case should, when every element is a struct value whose `struct_array_inline(sid)` holds, emit `NEW_STRUCT_ARRAY` (mirror `emit_empty_array`). Fixture works around it by building via `.append()`. |
| OFI-163 | **Self-hosted codegen (M4): a generic `Option<T>`/`Result<T>` payload binding (`case Some(v) { … }`) cannot classify `v`'s concrete refcounting without scrutinee type inference.** The erased prelude payload `T` may be a scalar (`Option<int>` → no INCREF on consume) or refcounted (`Option<Expr>` → INCREF); the self-hosted backend has no checker to resolve the instantiation, so it leaves prelude payloads UNCLASSIFIED (no INCREF, matching the scalar case). CONCRETE user-enum payload bindings (string/struct/array/enum) ARE now classified and byte-identical. | **OPEN** — needs lightweight scrutinee-type tracking: thread the matched value's `Option<X>`/`Result<X>` instantiation to the binding so `v` inherits `X`'s element/refcount discipline. Deferred; surfaced completing match-binding payload classification (`ty_str`, `bindings_arrays`). |
| OFI-162 | **Native backend: a METHOD (`fn …(self, …)`) that RETURNS a value-struct mis-compiles** — silent VM/native divergence, with a hard `em_s13`-type C error in some uses. Building the M4 self-hosted codegen, an `expr_ret_info(self, e) -> RetInfo` method (RetInfo = an all-scalar value struct) returned WRONG field data on native (every field read as a zero/false) while the VM returned the correct struct; adding a `println` that interpolated the returned struct's fields turned it into a hard native compile error (`initializing 'Value' with an expression of incompatible type 'em_s13'`). A FREE function returning the same value-struct (`ret_info(...) -> RetInfo`) compiles AND returns correct data on both backends — so the trigger is specifically the `self`-carrying method return path in `src/cgen_c.c`. Sibling of OFI-155/158/161 (the native value-struct-by-value family). | **OPEN** — `src/cgen_c.c` method-return lowering for an all-scalar value struct. Worked around soundly in `selfhost/codegen.ig` by returning a single `int` classification code from the method instead of a struct (`expr_ret_kind`), and keeping the value-struct return only on free functions (which are correct). |
| OFI-161 | **Native backend: a free function with a `mut` STRUCT parameter does not persist mutation** — silent VM/native divergence. `fn bump(mut p: P) { p.pos = p.pos + 1 }` mutates the caller's struct on the VM (boxed reference) but NOT on native, which passes the struct by value (a copy), so the write is lost. Verified: a driver calling `bump(p)` three times prints `pos=3` on VM, `pos=0` native. `mut self` METHODS are unaffected (they mutate the boxed receiver in place on both backends — the lexer's whole scanner relies on this). Sibling of OFI-155 (native struct-mutation by-value). | **OPEN** — `src/cgen_c.c` must pass a `mut`/`move` struct param by reference (or box + write-back) and mirror the VM. Surfaced building the M2 self-hosted parser; worked with by using methods-in-struct (the idiomatic, VM==native form) instead of free-function `mut Parser` threading. |
| OFI-159 | **`--emit=tokens` printed `INVALID` for 5 valid keywords** (`extern`, `type`, `where`, `requires`, `ensures`). Their `TokenType`s exist and lex correctly, but `src/token.c`'s `TOKEN_NAMES[]` had no entry, so `token_type_name()` returned the literal `"INVALID"`. Surfaced by the Stage 1 self-hosted-lexer recon — the lexer differential would otherwise have had to reproduce the bug. | **CLOSED 2026-06-27** — added the 5 designated-initializer entries; no golden used these keywords, so no regression (422/0, 8 gates). The token oracle is now correct, so the self-hosted lexer prints proper names rather than replicating a bug. |
| OFI-157 | **VM/native recursion-depth divergence** (self-hosting). The bytecode VM caps the call stack at `FRAMES_MAX = 256` (`src/vm.c`); the native backend is bound only by the OS stack, so the *same* deeply-recursive program runs on native but aborts on the VM (`call depth exceeded`) — a stdout divergence under the differential. A recursive-descent parser/checker hosted on the VM (Stages 2–3) must depth-guard well under 256, allowing for frames-per-grammar-level (~2), exactly as `std/json.ig` caps nesting at 64. | **OPEN (documented constraint, low)** — by design; the self-hosted front end guards depth in Ingle. Surfaced + encoded by the Stage A `calc.ig` spike (guard lowered 200→100 after the VM aborted at 200). | The bytecode VM caps the call stack at `FRAMES_MAX = 256` (`src/vm.c`); the native backend is bound only by the OS stack, so the *same* deeply-recursive program runs on native but aborts on the VM (`call depth exceeded`) — a stdout divergence under the differential. A recursive-descent parser/checker hosted on the VM (Stages 2–3) must depth-guard well under 256, allowing for frames-per-grammar-level (~2), exactly as `std/json.ig` caps nesting at 64. | **OPEN (documented constraint, low)** — by design; the self-hosted front end guards depth in Ingle. Surfaced + encoded by the Stage A `calc.ig` spike (guard lowered 200→100 after the VM aborted at 200). |
| OFI-156 | **Cross-module construction of an imported enum's bare (zero-field) variant fails** (`undefined variable`). OFI-073 wired cross-module *payload*-variant construction (`json.Obj([...])`, call form), but the zero-field form (which desugars to `EXPR_GET`) was never threaded across modules: after `import "lib" as lx`, both `lx.TEof` and unqualified `TEof` fail on BOTH backends, while same-file `TEof` and imported `lx.TNum(7)` work. On the self-hosting path the lexer's `[Token]` is dominated by bare variants the parser/checker reference across the `lexer.ig` boundary. | **OPEN** — front-end (`src/check.c` `EXPR_GET` / cross-module variant resolution). Sound workaround (both backends): a constructor fn (`fn eof() -> Token { return TEof }`). Write-up below. |
| OFI-158 | **Native: nested value-struct field assign through a NON-LOCAL boxed parent is unsupported** (a clean compile error, not a miscompile). `o.mid.span.col = v` where `mid` is itself non-flat (boxed), and `ns[i].span.col = v` (array-element grandparent): the VM compiles + runs both correctly, but native emits the OFI-155 guard `… non-local boxed parent is not supported — rebuild the struct immutably`. The writeback requires the boxed parent be an `EXPR_IDENT` (emit-twice-safe); a field/index parent would need hoisting + a second writeback level. | **OPEN (low)** — the residual narrowing of the OFI-155 fix; a VM/native capability gap, not garbage. Workaround: immutable rebuild (name every field — Ingle has no `…spread`). Local/`self`/2-level-`self` paths all work. |
| OFI-155 | **Native backend miscompiled in-place mutation of a value-struct FIELD of a non-flat struct** — silent VM/native divergence. For `struct Node { name: string  span: Span }`, `n.span.col = n.span.col + 1` made `cgen_c` emit the inner-field read as a C statement-expression *rvalue* and assign to it → clang `error: expression is not assignable`; the whole-field form `n.span = Span{…}` compiled but the native binary printed **uninitialised garbage** while the VM was correct. | **CLOSED 2026-06-27 — both root causes fixed** (runtime `em_set_field` inline-struct branch + cgen read-modify-writeback); a pre-existing sibling (flat whole-field assign) fixed too. Both backends, 422/0, 8 gates, ASan clean, RSS flat over 50M iters; 19-finding adversarial pass. Residual: OFI-158. Write-up below. |
| OFI-154 | **`tests/selfhost/` differential tier + `make selfhost` gate** (self-hosting bootstrap). A tier of compiler-shaped programs run on BOTH backends with byte-identical stdout required, folded into `make verify` and CI. | **SHIPPED 2026-06-26** — `tests/run-selfhost.sh` + 4 self-checking Stage A spikes (calc/symtab/recursion_scale/file_io); all 8 verify gates green. Write-up under Closed. |
| OFI-153 | **Generate an Ingle-consumable token/vocabulary table from `include/vocab.def`** (self-hosting Stage 1 prerequisite). `vocab.def` is the single source of truth for the lexical vocabulary; the self-hosted lexer's keyword/operator table must be generated from it, never hand-copied (mirrors `tools/gen_editor_assets`). | **OPEN** — opens with Stage 1 (self-hosted lexer). |
| OFI-152 | **Bounds on generic enums / standalone methods** (self-hosting, conditional). `language.md` notes generic enums and standalone methods can't carry interface bounds; needed only if AST/visitor modelling requires an interface bound on a generic enum's type parameter. | **OPEN (conditional)** — open only if Stage 2/3 modelling actually hits it; the recursive `[Json]`/`Box` AST shape does not. |
| OFI-151 | **`system()` / process-spawn builtin** (self-hosting Stage 5). Lets a self-hosted *native* driver invoke `cc` itself. Not needed for the VM-first Stages 1–4; required only for an integrated `inglec -o bin` at Stage 5 (alternative: emit C and let `make`/stage 0 run `cc`, as today). | **OPEN (deferred to Stage 5)**. |
| OFI-150 | **Refinement types** (type-system campaign, part b; `type Percent = int where 0 <= self && self <= 100`). A `where` predicate over `self`, checked AT CONSTRUCTION via the contract machinery (`OP_CONTRACT_CHECK`, debug-checked, release-elided) → a `refinement_violation` Fault; a read needs no recheck. | **SHIPPED 2026-06-26** — VM-only check (like all contracts); 421/0, 7 gates, ASan; full write-up below. |
| OFI-149 | **Newtypes** (type-system campaign, part a; `type UserId = int`, `type Email = string`). A distinct nominal type over a scalar/string base, zero runtime cost (erases to the base); cross-type use is a compile error; auto-inherits the base's compare/hash/show; arithmetic via explicit unwrap (`int(x)`). | **SHIPPED 2026-06-26** — both backends, 412/0, 7 gates, ASan clean; full write-up below. |
| OFI-148 | A STALE comment in `src/check.c` (~line 6981) claimed unit-return `ensures` “is rejected for now / stays disallowed (see OFI-026)” — but OFI-026 is CLOSED (root-caused 2026-06-13 to an uninitialised `closure_call` field in `new_expr`), the code below the comment *allows* unit `ensures`, and `std/ui.ig`'s `begin(mut self)` uses it in production. The contradiction misled an adversarial code-analysis agent into reporting unit `ensures` as a live blocker for field refinements. | **CLOSED 2026-06-26 — comment corrected** to state OFI-026 is closed and unit `ensures` is sound (the corruption was the arena-uninit field, not the contract check). Surfaced during the refinement/newtype validation pass. Comment-only; no logic change. |
| OFI-147 | A top-level constant's TYPE ANNOTATION is not checked against its value: `let X: int = "hello"` (and `let _: int = "hello"`) compiles, and `X` then resolves + prints `"hello"` — the annotation is inert at module scope. `collect_global` (`src/check.c` ~2833) validates only that the initializer is an immutable literal, never that its type matches the declared annotation (the function has no `annotation_type(...)` vs value-type `assignable` check that the function-local `STMT_LET` path runs). Affects EVERY top-level `let` equally, not just discards. | **CLOSED 2026-06-25 — filed + fixed same day** (surfaced by the OFI-098 adversarial review; pre-existing per git blame). `collect_global` (src/check.c) now sets the annotation as the EXPECTED type before checking the value — so a literal ADOPTS the declared width (`let Z: u8 = 5`, `let F: f32 = 1.5`) — then validates `assignable(value, vt, at)`, mirroring the function-local `STMT_LET` path; a mismatch (`let X: int = "hello"`) is now a compile error. Regressions `tests/run/global_annotation.ig` (adopt) + `tests/run/error_global_annotation.ig` (mismatch); 401/0, all 7 verify gates green. |
| OFI-146 | A fresh OWNED-temp string in an interpolation hole LEAKS one reference per evaluation: `"x={f()}"` (f returns an owned string), `"{a + b}"`, or any `.show()` desugar. `OP_TO_STRING` (and native `em_to_string`) unconditionally RETAINS the hole's string so the fold's consuming `OP_CONCAT`/`em_add` has an owned operand — correct for a BORROWED source (a local/field), but for a value that already owns its reference it inflates the count to 2 and only one release follows, leaking the other. Latent + rarely hit (holes are usually borrows), but the OFI-139 `Show` desugar makes it UNAVOIDABLE — `show()` always returns a fresh owned temp, so every `"{struct}"` would leak a string. RSS scaling confirmed it (34 MB → 308 MB at 4M iters; a borrow hole stayed flat at 2 MB). | **CLOSED 2026-06-25** (found while landing OFI-139, via RSS-scaling probes; pre-existing, not introduced by Show). Fix: the checker flags an owned-temp string hole (`StrPart.string_temp = is_owning_temp(hole) && pt == TY_STRING`) and BOTH backends then SKIP the retaining render op for it (`codegen.c` omits `OP_TO_STRING`; `cgen_c.c` emits the expr raw, not wrapped in `em_to_string`) — the value's own reference is exactly the owned operand the fold consumes. A borrowed string still retains; a numeric/bool hole is unaffected. Verified: RSS now flat (~2 MB) at any iteration count on both the Show and plain-fn-string paths; reclaim double-drop detector + ASan clean on a mixed-hole loop (no under-retain → no UAF); VM==native; 392/0 + all 7 verify gates. Regression rides `tests/native/show_interp.ig`. |
| OFI-145 | A `move self` method does NOT consume its receiver: calling `db.close()` where `fn close(move self) …` leaves `db` still readable/usable afterward (no use-after-move error) and the binding still auto-drops at scope end. The checker's method-call path (`src/check.c:~3865`) routes only NON-move receivers through the consume/drop logic; an `OWN_MOVE` receiver is read, never marked moved. Harmless for today's corpus (move-self is rare and no caller relies on post-call death), but a latent soundness gap. | **CLOSED 2026-06-25** (found by the resource-struct adversarial soundness panel, OFI-122, as confirmed hole R5/HOLE-B; the blocking pre-req for OFI-122 Phase 1). Fix (one site, `src/check.c`): at the method-call self handling, when `mi->self_qual == OWN_MOVE`, route the receiver `callee->as.get.object` through `consume()` — it is now marked moved (codegen nils the slot), so use-after-move is a compile error and the caller's scope-exit auto-drop is a no-op (no double-drop of the value the method already released). The corpus has zero move-self methods, so it was inert there (still 386/0); verified with targeted tests on BOTH backends (use-after-move rejected; a 1000× looped move-self with a kept array field is clean under the reclaim double-drop detector and VM==native). Regressions: `tests/native/move_self_consume.ig` + `tests/run/error_move_self_after_move.ig`. Gates: Ledger 300/0, Crucible 187/0. |
| OFI-144 | CLAUDE.md's "Ingle coding rules" state *"5 line spaces between every function/method in a code file"*, but the corpus is **split** on it: the pure-Ingle stdlib `std/string.ig` and `std/json.ig` follow it (~6 blank lines between top-level functions, sparse per-fn comments), while the FFI thin-wrapper modules `std/http.ig`, the C source `src/cextern.c`, and `std/slotmap.ig`'s methods use **2** blank lines with a doc comment hugging each function. So "read like the surrounding code" gives a different answer depending on which neighbour you look at. | **CLOSED 2026-06-25 — documented the split (the de-facto practice is correct, not a defect).** CLAUDE.md's coding rule now states it explicitly: **5 blank lines** in comment-sparse pure-Ingle code where the spacing IS the separator (`std/string.ig`, `std/json.ig`); **2 blank lines** where each function already carries a doc comment — the thin FFI/wrapper modules (`std/http.ig`, `std/sqlite.ig`), `src/cextern.c`, per-method container code (`std/slotmap.ig`) — where the comment is the separator and 5 blanks would just push code off-screen. "Match the file you're in." Normalising 50+ FFI wrappers to 5 blanks would be mechanical busywork against real readability; the split is by design. |
| OFI-143 | `std/sqlite` runs on the bytecode **VM only** (`make db`). The native backend (`inglec -o` → C, linking `libember_rt.a`) builds the runtime's FFI registry from `cextern.c` *without* `-DEMBER_SQLITE`, so the SQLite wrappers aren't registered there — the cextern indices the checker resolves wouldn't line up with the runtime table, so a native build of a `std/sqlite` program would mis-dispatch or fail to link. | **CLOSED 2026-07-13** (forced to the surface by shipping Quog as a native binary). Two coordinated changes: (1) **`RT_LIB_DB`** (`build/libember_rt_db.a`, Makefile) — a runtime variant archiving `runtime.c`+`cextern.c` built `-DEMBER_SQLITE=1` together with the vendored `build/sqlite3.o`; `make db` now builds it next to `build/inglec-db`. (2) an **`#elif EMBER_SQLITE` branch in `compile_native`** (`src/main.c`) that links that lib, so `inglec-db -o app app.ig` yields a standalone binary with the std/sqlite FFI resolved and SQLite statically inside. The registry indices baked into the emitted C (`em_ffi(idx)`) line up because both the emit compiler and the linked `cextern.c` carry the same `-DEMBER_SQLITE`. Built serial (vendored SQLite is `THREADSAFE=0`), so a program using **both** std/sqlite and spawn/channels is *rejected with a clear message* rather than mis-linked. `make quog` is now simply `inglec-db -o`. Regression guard: **`make test-quog-native`** runs the Quog integration golden against the standalone binary — the native path must behave byte-identically to `--emit=run`. Verified: `make test` 453/0, `make selfhost` 1459/0 (reproduction fixed point intact), test-quog + test-quog-native PASS, `otool -L build/quog` = `libSystem` only. Decision recorded in `docs/architecture.md`. |
...
| OFI-141 | The compiler **built and ran on macOS only** — not from any language or Apple-framework dependency (the core is clean POSIX C17: no `__APPLE__`, `mach/*`, frameworks, or arch assumptions) but from a cluster of build-time macOS-isms invisible under Apple clang/libc: **(1)** `-std=c17` on glibc hides the POSIX functions used (`realpath`/`popen`/`strdup`/`random`/`clock_gettime`/`sysconf(_SC_NPROCESSORS_ONLN)`) — strict ISO C with no `_DEFAULT_SOURCE`; **(2)** libm is a separate library on Linux (folded into libc on macOS), so every math builtin (`sqrt`/`pow`/`sin`/…) was an undefined reference at link without `-lm`; **(3)** the parallel/threaded builds relied on pthread living in libc (true on macOS) and passed no `-pthread`; **(4)** gcc `-Werror` flagged two issues clang silently passed — a `char cn[24]` that **truncated a generated C identifier** (cgen_c.c — a latent miscompile) and an `at[MAX_PARAMS]` `-O2` maybe-uninitialized; **(5)** the `tools/*.sh` gate scripts (opcheck/crucible) carried their own un-ported `cc` lines; **(6)** crucible's ASan oracle tripped because gcc enables LeakSanitizer by default on Linux (off on macOS) and its RSS oracle used BSD `/usr/bin/time -l`. | **CLOSED 2026-06-24** — root-caused and fixed against real Linux (Docker `gcc:13`/glibc 2.36 + Ubuntu 24.04, x86_64), not by reasoning: `-D_DEFAULT_SOURCE` + `-lm` + `-pthread` threaded through every Makefile flag group and the native `inglec -o` link; the two gcc-found bugs fixed at root (buffer sized to the cname field; arrays zero-initialised); the tools scripts ported; crucible's ASan oracle set to `detect_leaks=0` for macOS parity and `rss_of` made cross-platform (`time -v`). Result: dev + release + parallel + native all build, **regression 384/0**, **all 7 verify gates green** (opcheck/ceilings/ledger/crucible) on x86_64 Linux; macOS host re-verified unbroken. Installer de-gated for Linux (apt/dnf/pacman/zypper/apk); GitHub Actions CI added (Linux + macOS — the project's first CI). Full write-up below. Graphics provisioning tracked separately as OFI-142. |
| OFI-140 | Enum payload fields are declared with a **required name** (`enum Shape { Circle(radius: float) }` — an unnamed `Circle(float)` is rejected: `error: expected ':' after field name`), yet variants are **constructed positionally** (`Circle(2.0)`) and the declared name cannot be used at the construction site (`Circle(radius: 2.0)` → `error: expected ')' after arguments`). Structs are the mirror image: the same `name: type` declaration syntax, but constructed **by name** with braces (`Rect { w: 10.0, h: 5.0 }`). So an enum-payload name is mandatory to declare but inert everywhere it could be used — `match` binds to arbitrary names, not the declared one — making it pure documentation. A model reaches for `Circle(radius: 2.0)` by analogy to struct literals and is rejected. | **CLOSED 2026-06-25 — option (a): named enum construction now works.** `Circle(radius: 2.0)` (and out-of-order `Rect(h: 3.0, w: 4.0)`, generic `Some(value: 5)`, qualified `Shape.Circle(radius: 9)` / `mod.V(x: 1)`) all construct correctly, mirroring struct literals — the declared field names are no longer inert. The parser captures `name:` args (`parse_arg_list` + a `pk2` 2-token lookahead, src/parser.c) onto a new `EXPR_CALL.arg_names`; `VariantInfo` gained `field_names`; the checker `resolve_named_args` (src/check.c) validates (misspelled/duplicate/missing name, mixed positional+named, named-on-a-non-variant-call → clear errors) and REORDERS the args into declared field order, then clears `arg_names` so everything downstream — `infer_variant_type`, BOTH codegen backends — sees a plain positional build. Adversarially reviewed (18 repros): the review caught a real over-rejection — ENUM-NAME-qualified named construction (`Shape.Circle(...)`, `Option.Some(...)`) was wrongly rejected because `resolve_named_args` only tried the import-alias resolver; fixed by also resolving via `resolve_enum`/`enum_variant` (the positional path). Verified VM==native; regressions `tests/native/enum_named_construct.ig` (asymmetric reorder + qualified) + `tests/run/error_enum_named.ig`; all 7 verify gates green. |
| OFI-139 | String interpolation renders only a number, a string, or a bool — a struct or interface value is rejected: `"{shape}"` → `error: an interpolation '{ }' accepts a number, a string, or a bool`. Models (and people) reach for value interpolation by habit — Python f-strings, Rust `{}`/`{:?}`, Go `%v` — so this is the single most common first-write error a zero-/few-shot LLM makes in Ingle (caught in a qwen-coder-30B-generated program: it wrote `println("Processing {shape} ...")` over a `Drawable`). It cuts against the [manifesto](https://github.com/ingle-lang/ingle/blob/main/MANIFESTO.md)'s LLM-first / "least surprise to the model" stance — the model's default assumption is that any value is interpolatable. | **CLOSED 2026-06-25** — shipped as a `Show` interface (`fn show(self) -> string`) in the prelude, with STRUCTURAL detection (the method's presence is the opt-in, like Go's `Stringer` — no `implements Show` needed to interpolate; declaring it also lets the type serve as a `Show` value / a `T: Show` bound). The checker desugars a struct/interface hole `"{v}"` → `"{v.show()}"` by synthesizing a method-call node (`synth_show_call`, src/check.c), so the hole flows through the ORDINARY method-call codegen on BOTH backends — static dispatch for a concrete/generic struct, vtable dispatch for an interface value — with NO runtime/opcode change. A value with no `show` now gets an actionable error naming the contract. Decision taken (Karl): NO implicit auto-stringification — a type opts in by providing `show` (explicit, yet zero-ceremony), per the LLM-first stance. Surfacing the feature exposed + fixed a pre-existing leak (OFI-146) the desugar would otherwise hit on every use. 392/0 both backends, all 7 verify gates green; regressions `tests/native/show_interp.ig` (VM==native) + `tests/run/error_interp_not_showable.ig`. Docs: language.md interpolation + for-llms cheat-sheet updated. |
| OFI-138 | The Flare Claude app SEGFAULTED on close under the **M:N runtime** (`run-flare-mn.sh` → `inglec-mn-net-gfx`): `draw.close()`'s raylib/OpenGL teardown (`UnloadFont` → `glDeleteTextures`) ran on an M:N WORKER thread, not the GL-context thread (worker 0). Root cause: the M:N scheduler's single shared ready-queue (`rq_pop`, vm.c) lets a parked-then-requeued fiber resume on ANY worker thread. The main fiber runs the render loop on worker 0 without yielding (so loop GL calls stay on T0), but the **nursery join at shutdown PARKS it**, and it resumes on a different worker → GL teardown off-context → SEGV (READ at the zero page in `glDeleteTextures`, ASan-traced through `scheduler_worker_main` vm.c:1704 on thread T4). The 1:1 PARALLEL build (`run-flare.sh`) pins the main fiber to its thread, so it never reproduced there; my second (Ollama) worker likely tipped the scheduling but the bug is latent in the runtime. | **CLOSED 2026-06-25** — RUNTIME FIX DONE (the app workaround landed 2026-06-23: `flare_chat.ig` moved `draw.close()` + `tape_off()` BEFORE the nursery join). The M:N scheduler now PINS the main/GL fiber to worker 0: a dedicated `Scheduler.pinned` slot fed by a `Fiber.pin_worker0` flag that ONLY worker 0 services (`src/vm.c`). `rq_push` routes the main fiber's EVERY requeue — nursery-join wake (the exact bug), channel wake, cancel sweep — into that slot and BROADCASTs; `rq_pop(is_worker0)` claims it before the shared queue; the global-deadlock verdict now also requires `pinned == NULL` so a pinned-runnable main is never mistaken for a stall. So ANY graphics/Cocoa-on-M:N program is sound, not just this app. PROVEN: a one-off thread-id probe showed the parked-then-resumed main fiber resumes on worker 0 in 20/20 runs (0 violations); new regression `mn-stress` case `main-parks-at-join` (passed 7/0, watchdog-guarded so a lost wakeup = HANG fail); TSan-MN + ASan-MN clean; all 7 verify gates green. **Closes OFI-089.** |
| OFI-137 | A COLD local model gives no specific feedback during load: the first Ollama send after launch (or after the keep-alive unloads the model) makes Ollama spend ~10–15s loading the 12B weights into the GPU BEFORE the first SSE token, during which the app shows only the generic animated "Claude is thinking" spinner. Root-caused Karl's "GPU heats then cools after ~40s, nothing output" — the path is functionally CORRECT (verified end-to-end: a cold-start trace showed `pend=true str=false dN=0` for ~15s, then 231 deltas flowed and the reply committed; a warm 723-token reply streamed + displayed + saved cleanly), but a user watching the GPU work with no tokens reasonably reads the silent wait as a hang. | **CLOSED 2026-06-25** — `thinking_turn` (flare_chat.ig) is now provider/model-aware: while `pending && provider==Ollama && no deltas yet` it renders "Loading <model> <spinner>" instead of the generic "Claude is thinking", so a cold-start GPU-warming wait reads as a clear state, not a hang. A warm model flashes it sub-second; Claude is unchanged. Compiles under `net-graphics`. |
| OFI-136 | Local-model discovery (`oll.list_models` → `GET /api/tags`) runs SYNCHRONOUSLY on the render thread — at launch (when the saved provider is Ollama), on a provider switch, and on "Refresh models". For a local daemon this is sub-millisecond (or an instant connection-refused when it's down, via the 4 s connect timeout), so the UI never visibly stalls today. A future REMOTE OpenAI-compatible endpoint behind the same picker would, however, block the frame for the round-trip. | **CLOSED 2026-06-25** — discovery is now ASYNC: a long-lived `oll.disco_worker` fiber (spawned in the app nursery alongside the stream workers) runs the BLOCKING `discover()` on its own OS thread and returns a `{models, tool_models}` JSON envelope on a channel; the render loop kicks it off (launch/switch/refresh) and drains it with `try_recv`, showing a "Discovering models…" hint meanwhile. So the render thread never stalls — closing not just the future-remote case but the REAL current one: a 4 s freeze on launch/switch when the daemon is down (the connect-timeout was on the render thread). `discovering` was added to the idle-gate wake set so the result is polled promptly under event-waiting. Verified live (daemon up: 2 models discovered; the offline harness covers `models_of`/`tool_models_of` + graceful failure). **Follow-up fix (same day, found dogfooding `run-flare-mn.sh`):** the new worker was spawned in the app nursery but its channel was not closed at shutdown, so on app close `disco_worker` stayed parked on `recv` → the nursery join hung → "deadlock: every task in the nursery is blocked". Fixed by adding `close(disco_base_ch)` alongside the existing `close(req_ch)`/`close(oll_req_ch)` (every spawned worker needs a shutdown close, or the structured-concurrency join can't complete). Reproduced + fixed headlessly under `inglec-mn` (3 workers: all-closed → clean exit; one unclosed → the exact deadlock). |
| OFI-135 | The Claude desktop app's agentic tools (`read_file`/`write_file`) are advertised only on the **Anthropic** path — they ride Anthropic's `tool_use`/`tool_result` content-block format. The new **Ollama (local)** provider sends no `tools`, so the agentic loop never triggers and a local model runs as plain chat even when it reports a `tools` capability (e.g. `mistral-nemo`). Ollama's OpenAI-compatible tool shape (`tools[].function` + `tool_calls` deltas + `role:"tool"` messages) is different and `ollama.build_request`/`stream_worker` don't yet emit/parse it. | **CLOSED 2026-06-25** — Ollama now does OpenAI-style function calling. `oll.openai_tools` reshapes the app's ONE Anthropic tool catalogue (`{name,description,input_schema}`) into `{type:function, function:{…}}` (no duplicate schema); `build_request` attaches it (gated, see below) and maps the transcript's tool turns — a tool_use turn (kind 1) → an assistant message with `tool_calls`, a tool_result turn (kind 2) → a `{role:"tool", tool_call_id, content}` message — so the agentic loop replays correctly. `stream_worker` assembles `choices[].delta.tool_calls[0]` fragments (id+name once, `arguments` concatenated) via a PURE testable `feed_tool_call`, then emits the SAME `pack_tool` control message the Anthropic worker does → the app's existing provider-agnostic agentic loop runs the tool + re-sends with NO change. GATED on capability: discovery surfaces the `tools`-advertising subset (`tool_models`), and `send_turn` sends tools only when the selected model is in it. **Verified LIVE end-to-end against qwen3-coder:30b**: the model called `read_file` and `feed_tool_call` reconstructed `args={"path":"notes.txt"}` from the stream; the offline harness covers request shape + tool-turn mapping + fragment accumulation (net 2/0). |
| OFI-134 | The public `curl -fsSL https://ingle-lang.org/install.sh \| sh` installer aborted at `sh: line 210: INGLE_PREFIX?: unbound variable` even though `$INGLE_PREFIX` is set unconditionally on line 26. Root cause: under a UTF-8 locale (reporter's `LANG=C.UTF-8`), bash 3.2 (macOS `/bin/sh`) mis-parses a `$VAR` IMMEDIATELY followed by a multibyte char — `info "Installing to $INGLE_PREFIX…"` let the first byte of the Unicode ellipsis `…` (`E2 80 A6`) be read INTO the variable name, so it looked up an unset `INGLE_PREFIX‹E2›` and tripped `set -u` (the stray byte renders as `?`). Only line 210 had `$VAR…` adjacency; every other ellipsis followed a `)` (`source (main)…`, `take a minute)…`), which terminates the name — so those printed fine, exactly matching the reporter's console output. The installer carried 11 non-ASCII chars (5 `—`, 6 `…`) purely for typography. | **CLOSED 2026-06-23** (reported by Karl; root-caused on his machine by reproducing the exact `curl\|sh` under `LC_ALL=C.UTF-8` with `$INGLE_PREFIX` SET → `INGLE_PREFIX�: unbound variable`, while plain `C`/`POSIX` and the file-fed `/bin/sh install.sh` path both printed clean). Made `docs/install.sh` pure ASCII (`—`→`-`, `…`→`...`): `grep -P '[^\x00-\x7F]'` now empty, `sh -n` clean, and a full piped install under `C.UTF-8` to a temp prefix succeeds end-to-end. A `curl\|sh` installer must be locale-independent ASCII. NOTE: ingle-lang.org serves the COMMITTED copy, so the live fix needs a commit + push. |
| OFI-133 | Toasts had no INTERACTIVE affordance — a notification could not carry an action (the reversible-"Undo" pattern), so a destructive action like deleting a conversation was instant and unrecoverable. | **CLOSED 2026-06-23.** `f.toast_action(text, label, token)` renders an accent action button on the pill; a RELEASE over it fires `token` for one frame via `f.take_action()` and dismisses the toast (manual release-edge hit-test against the direct-drawn pill). Dogfooded as Undo-delete: deleting a conversation snapshots it (title+turns, `.clone()`d — Ingle's linearity REFUSED the move-out of a field/loop-var, catching a real aliasing bug at compile time) and shows "Conversation deleted · [Undo]"; the token re-inserts + re-selects it. Regression `flare_toast_action.ig` (press ≠ fire; release fires + dismisses); the no-action pill stayed byte-identical (graphics 43/0). |
| OFI-132 | Flare had no transient-notification primitive — action feedback ("Copied", errors, confirmations) had nowhere to surface. | **CLOSED 2026-06-23** (committed `bb3073f`). `f.toast(text)` enqueues; `f.toast_layer()` (called once per frame after `finish()`) draws + ages the queue as a fade+slide pill stack on the modal layer, auto-dismissing on a deterministic frame timer (~3.3s), built on `presence()`. Keeps the loop awake while a toast is alive so the timer advances under idle event-waiting (OFI-126). Dogfooded: code-block + message Copy → "Copied to clipboard". Regression `flare_toast.ig`. |
| OFI-131 | Flare had no opacity/compositing — a subtree could not fade as one, blocking a proper enter/exit (slide-only) plus dimming / disabled / scrim states; text was always fully opaque. | **CLOSED 2026-06-23** (committed `bb3073f`). A fade multiplier (`set_alpha` builtin) captured per draw command and folded into final alpha at flush (one capture site, no per-op cost); `f.fade_begin(amount)/fade_end()` is a nesting paint bracket. No-op at the default — text/rect emit a tape `alpha` field only when <255, so every un-faded golden is byte-identical. Dogfooded: the message enter is now fade+slide. Regression `flare_fade.ig`. |
| OFI-130 | Flare elements SNAPPED in and out — additions popped, deletions teleported; no enter/exit motion (the biggest animation gap left after OFI-093's springs + FLIP). | **CLOSED 2026-06-23** (committed `bb3073f`). `f.presence(key, present) -> float` springs 0→1 on first sight (enter) and 1→0 when `present` flips false (exit, after which the caller drops the element), on the keyed-state surface so it survives list reorders; pairs with `at`/`fade` for fade+slide. Dogfooded on the chat composer/messages. Regression `flare_presence.ig`. |
| OFI-129 | Flare rebuilt + re-laid-out the ENTIRE transcript every frame (O(total)), so a long chat dropped below 60fps — OFI-120's cull skipped DRAWING off-screen rows but still BUILT and laid them out. "A single screen's worth is cheap" (forrestthewoods) was broken for unbounded content. | **CLOSED 2026-06-23** — the standing answer to OFI-121. Immediate-mode list virtualization (Dear ImGui `ListClipper`, adapted): `f.virtual_begin(key,count)` / `virtual_item(i)` / `virtual_end()` build ONLY the rows whose extent falls in the scroll viewport + overscan; spacer struts of the skipped rows' summed height keep scroll-height + sticky-follow exact; per-row heights are LEARNED from last frame's solved rows (estimated until first seen — react-window's variable-height model). O(total) → O(visible): proven flat at ~1.5 ms/frame for 40 AND 400 turns. The app transcript virtualizes over visual BLOCKS (a tool_use folds its result into one item, so the window never aliases mid-pair). Regression `flare_virtual.ig`. Research-grounded (pretext, react-window, ImGui). |
...
| OFI-103 | `std/string.repeat(s, n)` infinite-loops for any negative `n`. The loop's only exit is `if i == n { return concat(out) }` with `i` starting at 0 and only incrementing (std/string.ig ~52-61, guard at line 56), so a negative `n` is never reached and the loop appends `s` unboundedly (hang / OOM). | **CLOSED 2026-06-21** (toward v0.3.40) — changed the loop guard to `if i >= n` (std/string.ig:56) so any `n <= 0` returns `""` immediately. Regression `tests/run/string_repeat.ig` (+ golden) covers n=3/0/-2; VM==native, both emit `a=[ababab] b=[] c=[]`. Reviewed solid — `repeat` was the only `==`-against-a-user-supplied-bound loop guard in string.ig (every other compares `i` against a non-negative `.len()`). |
...
| OFI-068 | Graphics render goldens are pixel-exact, so a freetype version change shifts text metrics ±1px and fails them | **CLOSED 2026-06-25** — the font is ALREADY pinned (Inter is embedded in the binary, `font_inter.h`), so only freetype-LIBRARY-version metric drift remained. Added a tolerant comparator `tools/tape-cmp.py` (wired into tests/run-graphics.sh, with an exact-compare fallback when python3 is absent): it compares each frame as JSON requiring op/layer/colour/size/text/structure to match EXACTLY while allowing the position+size fields (x,y,w,h) of each draw to differ by up to `EMBER_TAPE_TOL` px (default 3). The mouse field is ignored. Proven: a +2px x/w perturbation PASSES (drift tolerated) while a +10px shift, a colour change, and a draw-count change all still FAIL (no masking of real regressions); graphics 44/0. |
| OFI-066 | A user function named like a width type (`f32`/`i32`/…) is unreachable — the call parses as a width conversion | CLOSED 2026-06-19 (reserved the names: `collect_signature` rejects a FREE function whose name is a `numeric_typename`, with a clear message. Locals resolve before the conversion so `let int = …` is fine, and methods `x.i32()` use other syntax — both stay legal. `tests/run/many_functions.ig` renamed its incidental `f32`/`f64`; regression `error_fn_named_like_type.ig`.) |
| OFI-065 | `net`, `net-graphics`, `test-parallel` were real targets but missing from `.PHONY` | CLOSED 2026-06-18 (added to `.PHONY`; found while documenting the Makefile in THE_INGLE_BOOK) |
...
| OFI-055 | Code-point string helpers duplicated + `text_field` lacked h-scroll | CLOSED 2026-06-18 (`cp_*` in std/string UTF-8-verified by `tools/string-diff.py`; std/ui + gui.ig migrated render-identical; text_field h-scroll ported + render-tested) |
...
| OFI-018 | Parallel cross-thread frees defer reclamation to program exit | **CLOSED 2026-06-25 — accepted trade-off (sound by design, not an open bug).** The audit confirmed the defer-to-exit model is correct-by-construction: an object is freed exactly once, by its HOME context, compared by identity; a cross-thread-received object is spliced to the shared graveyard and reclaimed at the exit sweep — never double-freed, never use-after-freed. RSS is bounded for normal programs; only a pathological channel-heavy long-running workload that ships many objects across threads would accrue (the documented MPSC per-home-reclaim is the fix-path IF that workload ever appears). No code change — a deliberate, sound deferral, recorded as accepted. |
...
### OFI-155 — Native miscompiled in-place mutation of a value-struct field of a non-flat struct — CLOSED 2026-06-27
*Found by the Stage A self-hosting adversarial pass (2026-06-26), reproduced directly against `build/inglec`, fixed + adversarially verified 2026-06-27. A real native-codegen correctness defect, not an agent artifact.*

Mutating a value-struct-typed **field** of a **non-flat** struct (one carrying a `string`/array/boxed field, so the value-struct field is stored boxed-inline) was broken on the native backend, while the VM was correct. With `struct Node { name: string  span: Span }`:

- **`n.span.col = n.span.col + 1`** → native C **failed to compile**: `cgen_c` emitted the inner-field read as a C statement-expression *rvalue* and assigned into it (`({ … em_unbox_struct(…); r5; }).f1 = …`) → clang `error: expression is not assignable` (and it would have mutated a throwaway copy).
- **`n.span = Span{ line: 99, col: 8 }`** → **compiled but printed uninitialised garbage** on native (`col=4316438560`) vs the VM's `line=99 col=8` — a *silent* divergence, the worst class for a differential-gated bootstrap.

**Two root causes, both fixed:**
1. **Runtime** — `em_set_field` (`src/runtime.c`) only handled `AEK_BOXED`; for an `AEK_INLINE_STRUCT` field it fell through to `value_unbox`, writing a boxed pointer into a slot that holds packed leaves. Added the `AEK_INLINE_STRUCT` branch (memcpy the boxed source's packed bytes + reclaim the shell), mirroring the VM's `OP_SET_FIELD`. Fixes the whole-field assign. Sound with no old-value release because an inline value-struct field is provably all-scalar (`nested_inline_sid`/`is_scalar_type` exclude `TY_STRING`/boxed leaves → such a struct is forced boxed, not inline).
2. **cgen** — `STMT_ASSIGN` now detects (via `is_addressable_vstruct`) when the value-struct target is reached by unboxing out of a boxed parent and lowers to a **read-modify-writeback**: find the box boundary, `em_struct_field_inline` the inline field into an addressable `em_s` temp, set the leaf via the C lvalue chain on the temp, then `em_box_struct` + `em_set_field` back. The boxed parent is required to be an `EXPR_IDENT` (emit-twice-safe); a non-local boxed parent is a **clean compile error** → **OFI-158** (was previously garbage/uncompilable, so no regression).

A **pre-existing sibling** surfaced by the adversarial pass was fixed in the same change: a whole value-struct field assign on a flat *addressable* local (`o.b = In{…}`) emitted `em_s = Value` (clang error) because the direct path boxed the RHS — now it assigns the raw `em_s` when the target field is an inline value-struct (`field_struct[idx] >= 0`).

**Verification:** both backends byte-identical across leaf / whole-field / sequential / `self`-method / flat-local / flat-whole / mixed-width / multi-inline / loop shapes; the real `std/flare` 2-level `self.ui.style.x` shape works; 422/0, all 8 verify gates, ASan-native clean over 100k allocate-mutate-drop cycles, RSS flat over 50M iterations. A 3-agent adversarial workflow (divergence / memory-safety / false-rejection) confirmed memory safety and found only the flat-whole sibling (fixed) and the OFI-158 residual. Regression: `tests/native/nested_value_struct_field_assign.ig` (6 cases).


### OFI-156 — Cross-module construction of an imported enum's bare (zero-field) variant fails — OPEN
*Found by the Stage A self-hosting adversarial pass (2026-06-26), reproduced against `build/inglec`.*

OFI-073 closed cross-module *payload*-variant construction (`json.Obj([...])`, the call form, by threading `(enum_id, variant_tag)` onto the AST). The **zero-field / bare** variant form — which desugars to `EXPR_GET`, not a call — was never wired across modules. After `import "lib" as lx` where `lib.ig` defines `enum Token { TNum(v: int)  TPlus  TEof }`:

- `let t = lx.TEof` → `error: undefined variable` (qualified form).
- plain `TEof` after the import → `error: undefined variable` (unqualified form).
- both fail identically on **VM and native** (it's a front-end resolution gap, before either backend).
- but **same-file** `TEof` works, and the imported **payload** variant `lx.TNum(7)` works, and *matching* an imported bare variant (`case TEof`) works — only cross-module **construction** of a bare variant fails.

On the self-hosting path the lexer's `[Token]` is dominated by bare variants (`TPlus TStar TLParen TEof …`) defined in `lexer.ig` and referenced by `parser.ig` / `check.ig` across the module boundary, so this sits on the Stage 2 path.

**Sound workaround (both backends, proven):** export a constructor fn from the defining module — `fn eof() -> Token { return TEof }`, call `lx.eof()`. A 3-module lexer→ast→parser→main chain compiles and runs VM==native once bare tokens route through such constructors. **Fix direction:** extend the cross-module variant resolution (`resolve_qualified_variant` / the `EXPR_GET` bare-variant path in `src/check.c`) so `alias.Variant` and an unqualified imported `Variant` resolve a zero-field variant of an imported enum to its `(enum_id, tag)` and emit a bare construction — the same threading OFI-073 added for the call form, applied to `EXPR_GET`. Regression: `tests/run/variant_cross_module_bare.ig`.


### OFI-149 — Newtypes (type-system campaign, part a) — SHIPPED 2026-06-26
*Built in two stages over the live `inglec` (design: [docs/design/newtypes-refinements.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/newtypes-refinements.md)). Both backends; 412/0, all 7 verify gates, ASan clean.*

A `type Name = Base` declaration introduces a **distinct nominal type** over a scalar (`int`/widths/`float`/`f32`/`bool`) or `string` base. Constructed via `Name(x)`; the checker's `assignable` rejects any cross-type use (an `OrderId` where a `UserId` is wanted, a raw `int` where a `UserId` is wanted) — turning the **unit-confusion / swapped-argument bug class** (a top LLM-codegen error) into a compile error. **Zero runtime cost:** a newtype erases to its base (`is_move_type`=0 for a scalar base ⇒ freely copyable like its base; codegen-passthrough construction; no wrapper, no opcode). SemType band `NEWTYPE_BASE = 9_000_000` + a `NewtypeInfo` registry parallel to enums.

**Per Karl's design call, a newtype auto-inherits its base's COMPARE/HASH/SHOW** (Stage 2): `==`/`!=` (any base), `<`/`<=`/`>`/`>=` (numeric bases), `Hash`/`Eq` (so it keys a `Map`), and interpolation (`"{u}"` renders the base) — all via base look-through in the operator checker, `type_satisfies_bound`, `is_refcounted`, and the interpolation render. **Arithmetic requires an explicit unwrap** — done via the existing conversion-call form `int(u)` (chosen over a new `as` cast to avoid adding an `ExprKind` across ~8 exhaustive switches; entirely a checker change).

**The adversarial review caught 3 real bugs, all fixed:** (1) CRITICAL — `is_slice_type` was unbounded (`t >= SLICE_BASE`) and the new `NEWTYPE_BASE` sat above it, so every newtype was mis-read as a slice → compiler SEGV on `u[0]`/`u.len()`/`for`/var-reassign; fixed by bounding it `< NEWTYPE_BASE` (it was the only unbounded band predicate). (2) a width-base ctor (`Small(200)`, `type Small = u8`) didn't set `c->expected = base`, so a literal couldn't adopt the base width. (3) SOUNDNESS — a string-newtype ctor over an existing var (`Email(s)`) aliased without retaining → double-free/UAF; fixed by `consume()`-ing the ctor argument (incref an existing owner, leave a fresh temp).

**Tests:** `tests/run/newtype_basic.ig`, `newtype_ops.ig` (compare/show/map-key/unwrap/string), `newtype_soundness.ig` (the 3 review cases), `error_newtype_mismatch.ig`, `error_newtype_arith.ig`, `error_newtype_not_iterable.ig`, + `tests/native/newtype.ig` (VM==native differential).

**Documented v1 limitations (not bugs):** a `string` newtype has no unwrap-to-`string` form (compare/show it directly); and a newtype resolves only within its declaring module (no `mod.UserId` qualified form). Both are natural Stage-3/later additions. Next: **OFI-150** ranged/refinement types.


### OFI-150 — Refinement types (type-system campaign, part b) — SHIPPED 2026-06-26
*Built over OFI-149 newtypes (design: [docs/design/newtypes-refinements.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/newtypes-refinements.md)). 421/0, all 7 verify gates, ASan clean.*

A `where` clause turns a newtype into a **refinement type**: `type Percent = int where 0 <= self && self <= 100`, `type Nat = int where self >= 0`. The predicate (over `self`, an ordinary bool expression that may call predicate functions) is **checked at construction** — `Percent(150)` traps with a structured `refinement_violation` Fault naming the type + the construction site, on the existing `OP_CONTRACT_CHECK` machinery (debug-checked, **release-elided**; VM-only, like every contract — OFI-109). A read needs no recheck: *the type is the proof of validity* (parse-don't-validate, §5e/§5j).

**Sound at any nesting depth (the hard part).** The check is emitted by **codegen-time `self`-substitution**: while emitting the predicate, `EXPR_IDENT "self"` re-emits the (pure) constructor argument, then `OP_CONTRACT_CHECK`. Because that sequence is **stack-balanced** (its temporaries are pushed and the single bool it leaves is popped by the check), it is correct no matter how deeply the construction is nested as a sub-expression (a 2nd argument, a binary operand, an array element) — **no stack-slot tracking**. The value is produced *after* the check. Two soundness constraints fall out and are enforced: the base must be **numeric or bool** (a refcounted string base would be re-read by the predicate AND produced as the value, double-counting its reference — string *refinements* are deferred; a string *newtype* without a predicate is fine), and the argument must be **pure** (re-read must match the stored value — a `requires`-style error directs a computed value to a `let` first).

**Adversarial verification found 4 bugs, all fixed:** the review caught (1) the refinement silently bypassed for *any nested* construction (a stack-discipline hole — the original inline approach bound `self` at the wrong slot), (2) a method-`self`-shadowing compile error, (3) a self-referential-predicate compiler crash (guarded with a cycle check); then I caught (4) a residual variant of #1 — a refined construction after a **multi-slot value-struct argument** still bypassed the check. #1/#4 are why the inline-slot approach was replaced wholesale by the stack-balanced substitution above.

**Tests:** `tests/run/refinement.ig` (construct/read/predicate-fn), `refinement_nested.ig` + `refinement_multislot.ig` (nesting + the multi-slot gap), `refinement_method_self.ig`, `error_refinement_arith`/`_self_cycle`/`_nested`, `tests/fault/refinement_violation.ig` + `refinement_multislot.ig`. **v1 limitations (not bugs):** string-base refinements, refined mutable struct fields (no auto-invariant), and non-pure constructor args are deferred; static discharge of the check via the prover is a future bonus (today every check is runtime). Completes the type-system campaign (OFI-149 + OFI-150).


### OFI-122 — `Ptr` cannot be stored, so no type can own a C resource (no RAII handle/wrapper) — CLOSED (Phase 1) 2026-06-25
*Filed 2026-06-22 from an external language review (the no-owning-wrapper consequence it flagged as the sharpest FFI gap).*

**RESOLVED (Phase 1) 2026-06-25** — shipped as `resource struct` + a user `drop` (the owned dual of
`rc struct`). A resource owns its handle and its `drop` closes it automatically, on every path; `open`/
`prepare`-style constructors return `Result<R>` and `?` "just works" (an owned resource drops on the
early-return path). Designed + adversarially hardened (19-agent panel → revisions R1-R7 below, all
implemented; pre-req OFI-145 first), built across the checker + BOTH backends, dogfooded by refactoring
`std/sqlite`'s `Db`/`Stmt` onto it (the borrow-worker ceremony deleted). 390/0 both backends, all 7
gates green, reclaim-clean. Phase 2 (resources in collections) deferred. See docs/design/ptr-owning.md.
The original gap analysis follows (for the decision trail).
...
until this is resolved. **`std/sqlite` (2026-06-24) is the sharper case yet:** a connection and a
prepared statement are both bare `Ptr`s, so neither can be a `Result<Ptr,_>` (a checked open) nor a
struct field (a `Db` wrapper, a statement cache, a connection pool). The thin binding works around it
with the borrow-into-worker idiom (owner closes unconditionally; the worker uses `?` on a *borrowed*
handle), and the planned `query`/`exec` helpers hide the handle inside one call — but an owning `Ptr`
is what would let `std/sqlite` offer an ordinary `Db` value with RAII close. See [docs/sqlite.md](sqlite.md).

**Why it exists (legitimate v1 choice).** A `Ptr` has no Ingle destructor — the compiler can't know
...
### OFI-123 — The value model is width-erased: scalar widths are semantic-only, not stored at width — CLOSED 2026-06-25
*Filed 2026-06-22 from an external language review (the "real widths" caveat — true for type-checking, not yet for layout). **CLOSED 2026-06-25** — both gaps shipped: (a) full-range `u64` literals (parser parses via `strtoull`, `int_fits` admits a sign-bit-overflowing magnitude only as `u64`); (b) the native backend stores sized scalar LOCALS at their declared C width (box on read, unbox on write) — VM stays uniform (canonical reference). Verified VM==native + all 7 verify gates (394/0, Crucible clean). See the index row above for the full resolution.*
...
**Resolution.** Wired a read-only input layer onto rendered code blocks, reusing the existing field selection/clipboard machinery: `std/ui.code_caret_at` + `pressed_down` + `_code_input` (drag-select, Ctrl/Cmd+A, Ctrl/Cmd+C; no mutation), and `std/flare._code_block`/`_paint_code` (translucent per-line highlight behind the spans). The Claude app gets it free via `f.markdown`. Regression `tests/graphics/flare_code_select.ig`; graphics 20/20. Deferred: whole-document continuous selection across prose+code. (Full write-up in git.)


### OFI-083 — `.slice()` on a value-struct array reached through a struct field mis-sized its copy — CLOSED 2026-06-20 (sized the copy by the struct's `total_size` via `alloc_struct_array`; both backends, ASan-clean, Crucible 60/60; regression `tests/run/slice_value_struct.ig`. The "infinite loop" was the same mis-size. Header reconciled 2026-06-22; body below is the original OPEN filing.)
...
- **VM** (canonical, both `.ig` apps run here): lowers to "push receiver, **OP_INCREF**" — OP_INCREF *is*
...
- **Tests/fuzz:** `tests/run/clone.ig` (VM golden: struct-out-of-array, plain-local struct, array deep copy,
  nested-array element, field-of-index, Map independence); `tests/native/clone.ig` (array cases, VM==binary);
  ...
  (`inglec-trace`) clean on the headline cases + the Map.
...
- **Works** — `arr[i].field = wholeArray` and `arr[i].scalar = v` (the **assignment** path resolves the
  nested place and stores back — this is what `std/layout.ig` leans on: `self.nodes[i].rw = …`).
...
**Workaround (still valid)** (`flare_chat.ig` multi-conversation): the *checkout pattern* — keep the active
...
persist (`tests/run/array_index_mutate.ig` + the `tests/native/` dual-run proving VM==binary); ASan+UBSan
...
`tests/graphics/*.ig` assert the EXACT rendered draw-list (op + x/y/w/h…). Widget sizes that come from
...
though the code is unchanged. Hit when freetype 26.6.20 was freshly installed here: `flare.ig` went red
(`w:39`→`w:38`, `x:59`→`x:58`, …) while the other 10 cases passed (they don't measure text). Recalibrated
`flare.ig` to this machine's freetype so the suite is 11/11, but the golden will drift again on the next
...
bug; the affected case is `flare.ig` only.
...
text-measuring goldens drift — `flare.ig`, `text_field.ig`, `unicode_text.ig`, `wrap.ig` (e.g. button `w:38`→`39`,
a `wrap.ig` line break moving by one word) — so "the affected case is flare.ig only" understated it. Equal
libtool version ≠ identical advances (different patch/build, libpng/harfbuzz, or raylib font path). I did **not**
re-bless these (that would just trade Karl's calibration for mine); the new `tests/graphics/flare_modal.ig`
...
### OFI-055 — Code-point string helpers were duplicated (gui.ig + std/ui) + `text_field` lacked h-scroll — CLOSED 2026-06-18 (canonical `cp_*` family added to `std/string`, UTF-8-verified against CPython by `tools/string-diff.py` over 10k+ fuzzed cases; std/ui + gui.ig de-duped to it render-identical (flare draw-list md5 unchanged); `text_field` gained `clip`-masked horizontal scroll that keeps the caret in view. Regressions `string_codepoints.ig` + `text_field.ig`. Body condensed 2026-06-22; detail in git.)
...
The native backend (`inglec --emit=c` / `inglec -o`, docs/architecture.md "Decision: a native backend
...
install` doesn't yet ship the runtime headers, so `inglec -o` resolves `ember_rt.h` relative to the
...
- **M2b** — STRUCTS DONE (construct + field read): the C emitter emits an `em_struct(...)`
  ...
  (`build/libember_rt.a`, located next to `inglec`). Ownership for a field read is derived from the
  ...
  (tests/native/struct*.ig). Representation is ALWAYS BOXED — multi-slot hints ignored.
...
- **M2 STRUCTS — RESHAPED to real C structs (DONE).** The boxed-`Value` representation double-freed on
  ...
  structs".) "Copy structs" turned out MOOT — structs are NEVER `Copy` in Ingle (only scalars/strings/
...
- **M2f — STRINGS DONE (boxed, refcounted, immutable).** Literals → `em_str(ctx, bytes, len)`
  ...
  (tests/native/string.ig + string_loop.ig), incl. a 20k-iter concat/interpolation loop and an aliased
  ...
  of headers + the `.a` still pending (inglec -o works from the build tree today).
- **M3a — UNBOUNDED GENERICS + Option/Result + the `?` operator DONE.** Generics are ERASED: a generic
  ...
  struct param. tests/native/generics.ig.
- **M3b — CLOSURES DONE (lambdas, captures, function values, higher-order calls).** A closure is the boxed
  ...
  lambdas (refcount-balanced, no crash). tests/native/closures.ig. **GAP: a closure/function value with a
...
- **M3c — BOUNDED GENERICS DONE.** Witnesses are dictionary-passed: a bounded free function (`max<T: Ord>`)
  ...
  and `Map<K,V>` all run native (differential + 3000-round build/drop stress). tests/native/bounded.ig +
  collections.ig.
...
- **M3 KNOWN GAPS (clean errors / honest frontier, not miscompiles):** generics/interfaces/containers over
  ...
  examples/15_wordcount.ig), most string/array methods, slices.
- **M4 — CONCURRENCY DONE (spawn / nursery / typed channels on real OS threads).** A spawned task runs
  ...
  defer via `Obj.home`; main's exit sweep frees the graveyard. **Conditional build:** `inglec` detects
  ...
  — merge + deferral sound), and the deadlock detector. tests/native/concurrency.ig. **Fixed along the way
  (latent, pre-existing):** a `match` lowered to a C `switch`, so an Ingle `break` in a case (the
  ...
  method/closure or a bounded-generic function; `05_concurrency.ig` additionally needs string methods
  (`str.contains`).
...
**`make install` doesn't yet ship the runtime headers**, so `inglec -o` finds `ember_rt.h`/`value.h`
...
buffer (`fread`, `memcpy`-style) has a second output channel: the **bytes it wrote into the Ingle
...
`tests/replay/ffi.ig`, libm). **Fix (future):** in record mode, after a call with a `mut` buffer arg,
...
The pointer/buffer FFI lets C *receive* Ingle heap values as borrowed pointers — a `string` as a
`const char*`, a packed `[u8]` as a buffer (`mut` if written), a `Ptr` as an opaque handle — and
that is enough to bind real C (libc file I/O, string functions; see `examples/16_ffi.ig`). What it
does **not** yet handle is the other direction: a C function that *returns* memory it owns — a
`char*` from `strdup`/`getenv`-style APIs, or a freshly-`malloc`'d buffer. Ingle would have to either
...
wrong leaks or double-frees. The borrow model (Ingle owns nothing C owns) was the sound, simple first
cut. **Fix (future):** annotate a returned `char*` as copy-on-return (the safe default — wrapper does
`strdup`-then-Ingle-copy, C frees its own), with an opt-in "Ingle adopts + frees" form for APIs that
...
1. **Inferred return lifetimes — essentially MOOT for Ingle, dropped as a goal.** Lifetime
   inference only matters when a function can *return a borrow* (a reference into its input that
   the caller keeps using, e.g. Rust's `fn first(xs: &[T]) -> &T`). Ingle has **no first-class
   ...
   Ingle stays reference-free, which the LLM-first "no lifetime syntax" goal wants.) The "genuinely
...
2. **Generic-body ownership — the soundness hole is CLOSED 2026-06-13 (Stage 1).** It was worse
   ...
   **Fix:** `is_move_type` now returns true for a type parameter, so generic bodies are ownership-
   ...
   Regression test `tests/run/error_generic_use_after_move.ig` (the old SIGTRAP is now a teacher-grade
   ...
   a `Copy` param is a clean error. Ingle-native: `Copy` = everything except struct/array (shareables are
   immutable + refcounted → copy = a cheap incref). Wired in parser.c (`is_copy`), check.c (`is_copy_param`,
   call-site enforcement, move-exemption); tests `tests/run/generic_copy.ig`, `struct_return_copy_param.ig`,
   `error_copy_struct.ig`. So the ownership tail is fully resolved.
...
### OFI-185 — Non-decimal integer literals + digit separators — CLOSED 2026-07-06
*Phase 1b of the [ergonomics & ecosystem campaign](https://github.com/ingle-lang/ingle/blob/main/docs/design/ergonomics-campaign.md). Opened & closed same day.*

Integer literals were **decimal-only** — no `0xFF`, `0b1010`, `0o755`, and no `_` digit separators
— striking for a systems language where every bitmask was written in decimal, and a
zero-shot-LLM surprise (a model writing a mask emits `0xFF` reflexively). **Resolution:** the lexer's
`scan_number` (src/lexer.c) now recognises a leading `0x`/`0b`/`0o` (case-insensitive) followed by a
base digit, scanning base digits + `_` separators; `_` is also permitted between decimal digits and
in floats. A base literal is only recognised when a real base digit follows the marker, so `0border`
still lexes as `0` then the identifier `border`. The parser's `TOK_INT` conversion (src/parser.c) was
replaced with a base-aware, `_`-skipping accumulator that folds the magnitude as unsigned-64 (via
`__builtin_{mul,add}_overflow` for the > u64 trap, preserving OFI-123's full-range-u64 semantics and
the width-suffix path `0xFFu8`); floats strip `_` before `strtod`. **Deliberately NOT octal-by-leading-zero:**
`0755` is decimal 755 (the C footgun is avoided; octal is the explicit `0o755`). Verified on both
backends by `tests/run/numeric_literals.ig` (hex/bin/oct, separators, u64-max via `0xFFFF_FFFF_FFFF_FFFF`,
width-suffixed hex, a `_`-separated float, and hex/binary composing with compound assignment);
overflow past u64 still traps with a clear error; the existing u64/overflow/literal-range goldens are
unchanged. Grammar (`docs/grammar.ebnf`) + `docs/language.md` updated. 437/0. **Self-hosted parity (per the campaign rule):**
`selfhost/lexer.ig` (base-aware `scan_number`) + `selfhost/parser.ig` (`parse_int_lit`/`int_suffix_kind` base-aware,
underscore-stripped floats) ported; the checker false-reject cleared (595/605, 0 false-rejects). `make selfhost`
byte-identical (1370/0). **Adversarial review pass (5-way, empirically verified) fixed two real bugs it surfaced:**
(a) `parse_float_lexeme` only stripped `_` when the lexeme was `< 64` bytes, else fell back to raw `strtod` which stops
at the first `_` — a `≥64`-byte underscored float truncated (`1_000…0.0` → `1` not `1e61`) and diverged from the
self-hosted parser (which strips unconditionally); rewrote it to strip into an arena buffer at any length + a regression
line in `tests/run/numeric_literals.ig`. (b) The `from_bytes` builtin (native id 23) was genuinely **not lowered by the
self-hosted C-emit backend** (`selfhost/cgen_c.ig` stubbed it to `INT_VAL(0)`) — a pre-existing hole, live in
`serialize.ig`; added it to `native_id_for_name`/`is_em_native_id`/`native_ret_kind` (mirroring `byte_slice`, id 22),
then used it in `strip_underscores` so `parser.ig`'s reproduction validates the fix. `make verify` (8 gates) green.
The over-u64-literal accept divergence the review also flagged is a pre-existing self-hosting fidelity gap → **OFI-199**.

### OFI-184 — Compound assignment operators — CLOSED 2026-07-06
*Phase 1a of the [ergonomics & ecosystem campaign](https://github.com/ingle-lang/ingle/blob/main/docs/design/ergonomics-campaign.md). Opened & closed same day.*

There was no `+= -= *= /= %= &= |= ^=` — every mutation was written `x = x + 1`, a first-hour
friction and an LLM-surprise (models emit `x += 1` / `flags |= FLAG` constantly). **Resolution:** eight
compound-assignment tokens added to the lexer (src/lexer.c) + token table (include/token.h,
src/token.c), **desugared in the parser** at the assignment site: `place OP= rhs` becomes a normal
`STMT_ASSIGN` whose value is `EXPR_BINARY(OP, place, rhs)`, reusing the *exact* op-tokens the
precedence parser stores for the infix form — so the checker and **both** backends see an ordinary
assignment with **zero** checker/codegen change. The target expression node is shared as the binary's
left operand: compound assignment only type-checks on numeric/int (Copy) operands, so there is no
move/borrow aliasing hazard; the immutable-`let` error and overflow trap apply unchanged (a `let`
target is still rejected, `i += 1` still traps on overflow). **Scoped out (deliberate):** `<<=`/`>>=`
(they collide with the generic-close `>>`-split trick — deferred until the type parser is touched) and
`++`/`--` (expression-position mutation is a surprise generator, and `for x in range` removes the
need). **Known limitation:** a side-effecting place subexpression (`a[f()] += 1`) re-evaluates the
place, exactly as the hand-written `a[f()] = a[f()] + 1` would — faithful to the explicit form, but a
future evaluate-place-once pass could tighten it. Verified on both backends by
`tests/run/compound_assign.ig` (locals, struct fields, array elements, floats, all eight operators) +
a `let`-target rejection. Grammar + `docs/language.md` updated. 436/0. **Self-hosted parity (per the campaign rule):**
`selfhost/lexer.ig` emits the 8 tokens and `selfhost/parser.ig` desugars — but the owned `Box<Expr>` AST forbids sharing
the target node stage-0 pointer-shares, so the self-hosted parser **re-parses the LHS** from a saved cursor (identical
tokens + parser flags ⇒ a structurally identical subtree) for the binary's left operand. `make selfhost` byte-identical (1370/0).

### OFI-154 — `tests/selfhost/` differential tier + `make selfhost` gate — SHIPPED 2026-06-26
*The first concrete deliverable of the self-hosting bootstrap ([docs/design/self-hosting.md](https://github.com/ingle-lang/ingle/blob/main/docs/design/self-hosting.md)), Stage A.*

A new test tier, `tests/selfhost/`, of **compiler-shaped programs** — recursive ASTs, symbol tables, the lex/parse/eval shapes the reference compiler is built from — each run on **both backends** (`inglec --emit=run` and a native `inglec -o` binary) with byte-identical stdout required. The runner `tests/run-selfhost.sh` mirrors the `tests/native` VM-vs-binary differential; `make selfhost` (depends on `all` for the runtime static libs the native link needs) is folded into `tools/verify.sh` (both the full and `fast` gate lists) and into CI (`ci.yml` Linux job; macOS via `make verify`). The golden loop in `tests/run.sh` skips the `selfhost` stage so the auto-discovered directory isn't mistreated as a golden tier.

**Stage A spikes (4), all self-checking** (each asserts its hand-derived results and returns `Err` on mismatch → a non-zero exit the runner scores FAIL, giving the VM==native differential an oracle so a *shared*-wrong answer can't stay green):
- `calc.ig` — a complete `lex → parse → eval` pipeline: byte-level scanning, a recursive `enum Expr` with both `Box<Expr>` and `[Expr]` recursion, exhaustive `match`, a `Map<string,int>` env, `Result`+`?`.
- `symtab.ig` — string interner (2000 symbols, deduped over repeats), keyword `Set`, `[Map<string,int>]` scope stack with shadowing.
- `recursion_scale.ig` — a 150-deep `Box` spine (under the VM's 256-frame cap), an 8000-wide `[Tree]`, a depth-16 balanced tree (65 536 leaves).
- `file_io.ig` — `write_file`/`read_file` round-trip + UTF-8 byte/char ops, the self-hosted lexer's input path.

The harness also checks the native binary's exit code (a native crash after correct stdout is a divergence, not a PASS) and warns loudly when no C compiler is present (`SELFHOST_REQUIRE_NATIVE=1` turns the absence into a hard failure). Outcome: the central "can the language express + run a compiler's shapes, identically on both backends?" risk is **retired**; all 8 verify gates green. Surfaced OFI-155, OFI-156, OFI-157 along the way.
...
2. **libm.** On Linux libm is a separate library; on macOS it is folded into libc. Every math builtin (`sqrt`/`pow`/`sin`/`floor`/…, from cextern.c/vm.c/runtime.c) was an undefined reference at link. Fix: `-lm` appended **after** the objects on every link line (link order matters with `--as-needed`), and in the native `inglec -o` command (src/main.c).
...
**Symptom.** In VS Code, `examples/05_concurrency.ig` showed stray colours, including *inside comments*.
...
*Opened + CLOSED 2026-06-20 (surfaced by OFI-078: errors from inside `std/ui.ig` appeared on the open file).*

**Symptom.** Opening a file that imports a module with errors painted squiggles on the importer at the
imported file's line numbers — e.g. `examples/11_menus.ig` (72 lines) showed diagnostics up to line
1168, the lines of `std/ui.ig` (1215 lines). **Root cause.** Every checker diagnostic used `c->src` — a
...
reports 0 diagnostics on `examples/{09_ui,11_menus,17_flare}.ig`; a wrong-arity graphics call and a
...
**Fixes (`std/flare.ig`).** (1) `_code_block`/`_quote_block` now take an **explicit width** (`leaf(width,h)`),
...
correctly). Regression `tests/graphics/flare_codeblock.ig` (a fenced block + bold inside a START column —
...
best achievable purely in `std/flare.ig`.
...
`tests/graphics/flare.ig`): `f.row(flare.START, flare.CENTER)` … `f.end()`, `f.heading("Counters")`, and a
correct `f.begin()` … `f.finish()` frame. Also documented the new `modal_begin`/`modal_end`, `segmented`, and
`divider` in the same pass so the API reference matches `std/flare.ig`. (Lesson: the doc example should ideally
...
*Opened + CLOSED 2026-06-20 (found while making `inglec --lsp` compatible with Zed; it bit VS Code latently too).*
...
Not Zed-specific — VS Code was equally affected; it just rarely showed because Ingle code is mostly ASCII.
...
**Symptom.** `examples/14_cli.ig` suddenly failed to compile with a flurry of spurious "cannot mutate an array
...
`tests/run/interpolation_ownership.ig`. **Residual (separate, rarer):** general `f() + g()` / `acc + ownedTemp`
...
string, releasing the old); VM↔native differential `tests/native/struct_array_field.ig`; 200k string-field
...
naming all writeback temps `v%d`. Covered by a method case in `tests/native/struct_array_field.ig`. Found by
...
are logical, so unaffected). Applies to ALL graphics apps, not just gui.ig.
...
struct-with-string borrow stays flat-RSS (the retain is balanced). Regression: `tests/run/struct_borrow_arg.ig`.
...
`--emit=c` output for the borrow-unbox repros + ALL 39 `tests/native/*.ig` was compiled against it and run →
...
heavy `a.append(S{..})` under `build/inglec-asan`. **ASan is blind to it normally** (the pool keeps memory
...
goldens + 9 graphics + the 265-program ASan corpus all clean. Regression: `tests/run/inline_struct_array.ig`.
...
in scope), so the change is purely the collision suppression. Regression: `tests/native/generic_name_collision.ig`
...
`tests/run/error_mut_self_on_let.ig` (rejected) and the positive `var` case verified.
...
`tests/run/generic_bounded_ctor.ig` (positive — a bounded constructor now compiles + runs); both negatives
...
the full suite (292 green) + tests/native/heap_struct.ig + struct_box_bridge.ig + a 13-case RSS leak sweep
...
Ingle's `+ - *` **trap on overflow** (OFI-005 — the right default), but hashes/PRNGs/checksums
*depend* on modular (2^width) arithmetic, so they couldn't be written in pure Ingle. **Fixed** by
...
kind byte. **Showcase + regression:** FNV-1a in pure Ingle — `tests/run/wrapping_arith.ig`,
...
`std/map.ig`); rewiring built-in-key Hash witnesses to pure-Ingle hashing is now *possible* but is a
...
specified in grammar note (S). Regression: `tests/run/generic_literal_vs_lt.ig` (both readings side by
...
unrelated to hover: `editors/vscode/extension.js` launches `$(PREFIX)/bin/inglec --lsp` (the
*installed* binary at `~/.ingle/bin/inglec`), not `build/inglec`. The install target did
`cp $(RELEASE_BIN) "$(PREFIX)/bin/inglec"` **over the existing file**, which keeps the destination's
...
from `build/` (fresh inode) but was killed from `~/.ingle/bin/` (overwritten inode); `rm`-then-`cp`
...
*Opened & closed 2026-06-15 (user-reported: hovering `send` and `close` in `examples/05_concurrency.ig` gave no popup).*
...
*Opened & closed 2026-06-15 (user-reported: hovering `tokens.append(…)` in `examples/06_calculator.ig` gave no popup).*
...
because `06_calculator.ig` only ever calls intrinsics. **Resolution:** new `sem_record_intrinsic`
...
*Opened & closed 2026-06-14 (found from a user-reported "Ingle Language Server crashed 5 times" loop).*
...
Verified: a position sweep of `examples/09_ui.ig` over the LSP, which SIGSEGV'd around request ~2365,
...
The set of keywords, builtins, and primitive types — Ingle's lexical vocabulary — was copied by
...
`inglec` = what users/editors run; `tools/` = what language developers run to maintain checked-in
artifacts (so the generator is a standalone tool, not an `inglec --emit` mode — `--emit` transforms a
...
*Opened & closed 2026-06-14 (found while diagnosing "no colours in .ig files").*

The VS Code client written last session was placed *directly* in `~/.vscode/extensions/ingle-lang`
...
contribution**, so VS Code had no TextMate grammar and `.ig` files got zero syntax colouring
...
`~/.vscode/extensions/ingle-lang` (which keeps the extension GLOBAL — it colours any `.ig` file
...
(reusing the AEK_INLINE_STRUCT array machinery). Tests nested_struct.ig (=>26), field_mutation_nested.ig,
nested_struct_value.ig (=>179, copy-independence + 3-level write-back). 226 green, RSS flat over a
...
drifted from the implemented language without anyone noticing: `examples/03_errors.ig` used
`read_file(path)?` (read_file isn't a `Result`, so `?` is rejected) and undefined helpers
`parse_host`/`parse_port`; `examples/05_concurrency.ig` called undefined `contains(...)` /
...
1. **Rewrote both examples against the real stdlib.** `03_errors.ig` now showcases `?` through a
   ...
   short-circuit Err path; `05_concurrency.ig` is a real worker-pool — a `dispatch` task feeds
...
2. **Killed the drift class.** The smoke tier in `tests/run.sh` now FULL-compiles every example
   ...
   stay lex+parse there and are full-compiled in `tests/run-graphics.sh` under `inglec-gfx`. So a
...
positives — every existing function already returns on all paths. Test `error_missing_return.ig`.
...
unique-owner move types (arrays, structs with a boxed field), verified by `error_escape_borrow.ig`
(repurposed to a `string`-field struct, which still errors). New regression test
`struct_return_copy_param.ig` (=>20) returns copy-type params by value and confirms the source stays
...
- **multi-arg / non-first-arg / method-arg — FIXED 2026-06-13 (the general case).** Generalised
  ...
  temp arg is evaluated slightly early — observably equivalent in Ingle (no mutable globals; value
...
Regression `tests/run/struct_temp_drop.ig`; 216 green. The value-types inline-array storage switch
...
`contract_violation` on the tape. Regression `tests/run/unit_method_ensures.ig` (=>5); 215 green,
graphics 4/0.
...
A window (`window_begin`, [std/ui.ig](std/ui.ig)) was registered at a fixed 220×180 and never fit
...
2. **Auto-size** — `advance` records each window's content extent (`content_x/content_y`) while
   ...
   Verified by `tests/graphics/windows.ig`: an empty window collapses to its title bar
...
[std/ui.ig](std/ui.ig)) — the standard immediate-mode trick, since this frame's window rects
...
`tests/run/global_const.ig` (=>400) + `error_global_non_literal.ig`; 211 green, parallel parity.
...
`tests/lexer/errors_recovery.ig` (which exercises all three sites in one pass) is the regression
...
bookkeeping is already runtime-guarded by `tests/run/many_functions.ig` (259 entries, `main` + a
...
enumeration by the true `free-functions + methods` total (malloc'd), not `MAX_FNS`. **The Ingle
...
misled the manual hunt (see OFI-021). Regression test `tests/run/many_functions.ig` (259 table
...
*Opened & closed 2026-06-12 (found by the new `benchmarks/parallel_bench.ig` nested section).*
...
`tests/run/nested_nursery.ig` (=>2016, depth-3 divide-and-conquer). 198 green.
...
woken task (caught `channel_pipe.ig` mid-implementation: consumer parked on empty, producer
...
broadcast. Lock order is channel→nursery→heap (acyclic). `error_channel_deadlock.ig` now passes
...
Regression test `for_enumerate.ig` (=>5) captures the loop var, a body local, and the enumerate
...
any element type, with capturing lambdas, via import. Tests `stdlib_list.ig` (=>76, string and int
elements) and `generic_hof_strings.ig` (=>60, the crash class: mixed instantiations + sources used
...
interoperate in one `match`. Regression test `qualified_variant.ig` (Option/Result/user enum →
...
statement-position calls still run. Regression test `error_void_method_value.ig`. 184 green.
...
may still reuse a name (types/functions are module-scoped). Tests: `error_duplicate_fn.ig`,
`error_duplicate_type.ig`. (Duplicate *method* names on one struct remain undiagnosed — a smaller,
separate case.)
...
operators and Python-style bracket continuation now work. Test: `multiline_expr.ig`.
...
re-lexes each hole, recursing). Test: `interpolation_nested_string.ig`.
...
Tests: `move_diverging_branch.ig`, and `array_growth.ig` now uses the direct early-return form.
...
witness; an unbounded `T` stays opaque (no methods). Verified by `tests/run/bounded_generic.ig`
...
updated; `tests/run/dynamic_dispatch.ig` (=>1104) + `examples/13_interfaces.ig`. See the
...
CLAUDE.md's no-UB rule, and Ingle had not decided overflow behaviour. **Resolution:** decided

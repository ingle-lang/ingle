...
| OFI-220 | **Self-hosted C-emit does not RETAIN an enum-payload array assigned out into a caller variable.** `case TyFn(fps, fret) { outer = fps }` — stage-0 wraps the payload read in `own_into_slot` (the variable co-owns the array the enum still holds); the self-hosted C-emit emits a plain `outer = fps` (no retain) → a latent under-retain (double-free risk on drop). Surfaced by OFI-206 map work (`hof_srcs_of` originally did `fparams = fps`). | **OPEN — worked around, not fixed.** `hof_srcs_of` was restructured to keep the TyFn payload strictly inside the `tyfn_param_count`/`tyfn_param_tparam` helpers (the proven pattern — a helper reads `fparams[i]` in place and returns a scalar/string), never storing the payload array in a caller var. The retain-on-payload-store gap in the C-emit remains; fix = emit `own_into_slot` when an SLet/SAssign RHS is an enum-payload binding of array/refcounted type. |
| OFI-219 | **Self-hosted C-emit mistypes a NESTED-array element `xs[i]` (of a `[[T]]`) as a STRING.** `is_string_expr`'s `EIndex` arm returns true for any boxed (AEK 0) non-struct element, so `a[i].len()` on a `[[int]]` emits `em_str_len` instead of `em_array_len` (and `is_array_expr` has no `EIndex` arm, so it can't correct it). Stage-0's checker types it correctly. Surfaced by OFI-206 map work using `[[int]]` internal tables. | **OPEN — worked around, not fixed.** The OFI-206 tables were reshaped to FLAT `[int]` (self-describing stride-6 / block encodings) so no nested-array element `.len()` is emitted in cgen_c.ig; reproduction stays byte-identical. The real fix = track element-is-array (a new `sc_elem_is_array` scope flag + `fn_ret_elem_is_array`), have the `is_string_expr` EIndex arm exclude it, and add an `is_array_expr` EIndex arm that includes it. Needed before nested arrays are first-class in the self-hosted C-emit. |

...
[manifesto](https://github.com/kmcnally5/ember-lang/blob/main/MANIFESTO.md) found while building Ember. Raise here instead of coding around them.
...
| OFI-151 | **`system()` / process-spawn builtin** (self-hosting Stage 5). Lets a self-hosted *native* driver invoke `cc` itself. Not needed for the VM-first Stages 1–4; required only for an integrated `emberc -o bin` at Stage 5 (alternative: emit C and let `make`/stage 0 run `cc`, as today). | **OPEN (deferred to Stage 5)**. |
...
*Found by the Stage A self-hosting adversarial pass (2026-06-26), reproduced directly against `build/emberc`, fixed + adversarially verified 2026-06-27. A real native-codegen correctness defect, not an agent artifact.*
...
*Found by the Stage A self-hosting adversarial pass (2026-06-26), reproduced against `build/emberc`.*
...
*Built in two stages over the live `emberc` (design: [docs/design/newtypes-refinements.md](https://github.com/kmcnally5/ember-lang/blob/main/docs/design/newtypes-refinements.md)). Both backends; 412/0, all 7 verify gates, ASan clean.*
...
*Built over OFI-149 newtypes (design: [docs/design/newtypes-refinements.md](https://github.com/kmcnally5/ember-lang/blob/main/docs/design/newtypes-refinements.md)). 421/0, all 7 verify gates, ASan clean.*

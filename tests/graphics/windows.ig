// tests/graphics/windows.ig — regression test for the std/ui window system (Phase B).
// Graphics programs need a display, so this runs out of the default suite via
// `make test-graphics` (which builds/uses build/inglec-gfx). It does NOT rely on real

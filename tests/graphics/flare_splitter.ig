// tests/graphics/flare_splitter.ig — regression for the Flare splitter WIDGET (std/flare f.splitter + the _SPLIT
// paint arm), the layer above the std/ui _split_drag latch that splitter.ig covers. It drives the real widget:
// a sidebar panel (strut sbw) | f.splitter | main panel, taping the painted handle and printing the returned
// width. Input is injected (mx/down/was set after f.begin, like text_field.ig) at the handle's solved position,

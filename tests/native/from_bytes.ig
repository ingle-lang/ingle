// Native backend differential test for from_bytes(bytes: [u8]) -> string — the Ingle-side binary-serializer
// primitive for the .igb bytecode container (docs/design/bytecode-container.md). The harness runs this on
...
fn main() -> int {
    ...
    // them back unchanged (the serializer's actual output path for the .igb container).

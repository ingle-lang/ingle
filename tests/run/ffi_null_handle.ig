// ffi_null_handle.ig — regression for the NULL-FILE* crash (REVIEW_FINDINGS M6). `fopen` of a
...
// and fclose returns EOF (-1), so an Ingle program can detect and handle the failure.
extern "c" {

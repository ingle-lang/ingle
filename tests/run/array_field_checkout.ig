// array_field_checkout.ig — locks the two language primitives the OFI-072 workaround relies on.
// A struct holding an array field is a BOXED array element (unlike the all-scalar inline-value case
// in array_struct_inline.ig), so you can't move its array field out and can't append through the

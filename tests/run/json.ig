// json.ig — regression for std/json: parse → stringify round-trips, accessors, escapes, numbers,
// nesting, building, and error reporting. JSON braces/quotes are written as Ingle `\{ \} \"` because
// `{ }` open interpolation and `"` closes a string in Ingle source. (Accessor results are bound to
// locals before interpolating — Ingle has no nested string literals inside `{ }`, and `{ }` takes a

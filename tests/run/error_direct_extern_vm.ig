// error_direct_extern_vm.ig — a DIRECT extern (OFI-167): a C symbol not in the hosted FFI registry,
...
// message pointing at the native path. It compiles + links only via `inglec --emit=c` / `-o`.
extern "c" {
    fn uart_putc(c: i32)
}

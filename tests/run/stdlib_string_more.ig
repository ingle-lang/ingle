// stdlib_string_more.ig — the second wave of std/string: ends_with, substring,
...
fn main() -> int {
    var n = 0
    if str.ends_with("hello.ig", ".ig")        { n = n + 1 }   // 1
    if !str.ends_with("hello", "xyz")          { n = n + 1 }   // 2
    if str.substring("Ingle", 1, 4) == "mbe"   { n = n + 1 }   // 3
    if str.substring("Ingle", 0, 99) == "Ingle" { n = n + 1 }  // 4  (hi clamped)
    if str.substring("Ingle", 3, 1) == ""      { n = n + 1 }   // 5  (start >= end)

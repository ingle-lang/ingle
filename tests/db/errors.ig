// errors.ig — std/sqlite failure paths surface as Err, never a crash, on the clean `resource` API:
// exec against a missing table returns Err carrying SQLite's message, and a malformed prepare returns
// Err (the failed handle is finalized inside prepare). Needs inglec-db; run via `make test-db`.

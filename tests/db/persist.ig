// persist.ig — proves data reaches DISK *and* that a `resource` Db's auto-`drop` really closes (and
...
// (CREATE IF NOT EXISTS + DELETE) so the run is deterministic. Needs inglec-db; run via `make test-db`.

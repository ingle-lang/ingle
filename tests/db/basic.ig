// basic.ig — std/sqlite CRUD over an in-memory database (deterministic, no files), on the clean
...
// THEMSELVES at scope exit — no close()/finalize() in sight. Needs inglec-db; run via `make test-db`.

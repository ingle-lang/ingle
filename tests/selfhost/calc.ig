// tests/selfhost/calc.ig — Stage A self-hosting spike (docs/design/self-hosting.md §4 "Stage A").
//
// This is a compiler in miniature: a complete lex -> parse -> eval pipeline for a tiny expression
// language, written in pure Ingle. It exists to retire the central self-hosting risk before any real
...
// Parser is a recursive-descent cursor over the token stream, mirroring std/json.ig's design: it
...
struct Parser {
    ...
    // such cap, would keep going, a silent VM/native divergence. Guarding in the Ingle code keeps BOTH

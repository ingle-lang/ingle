...
porting the Ingle compiler into Ingle, one differential-green stage at a time, with the C reference
compiler (stage 0) kept as the oracle the whole way.

Each `*.ig` here is a **compiler-shaped program** — recursive ASTs, symbol tables, the lex/parse/eval
...
- on the bytecode VM — `inglec --emit=run X.ig`
- as a native binary — `inglec -o <bin> X.ig` then run it
...
| File | What it proves |
|---|---|
| [`calc.ig`](calc.ig) | A complete `lex → parse → eval` pipeline in miniature: byte-level string scanning, a recursive `enum` AST with **both** `Box<Expr>` (single/two-child) and `[Expr]` (n-ary) recursion, exhaustive `match`, a `Map<string,int>` variable environment, and `Result` + `?` error propagation. |
| [`symtab.ig`](symtab.ig) | The checker's data structures at scale: a string interner (2000 distinct symbols, deduped over repeats), a keyword `Set`, and a `[Map<string,int>]` scope stack with shadowing. |
| [`recursion_scale.ig`](recursion_scale.ig) | The AST shapes at the size a compiler produces: a 150-deep `Box` spine (kept under the VM's 256-frame call cap, `vm.c` `FRAMES_MAX`), an 8000-wide `[Tree]` node, and a depth-16 balanced tree (65 536 leaves). |

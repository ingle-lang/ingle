...
typedef struct {
    ...
    int                   nursery_depth;
    int                   freestanding;    // kernel/bare-metal emit: freestanding entry; hosted-only
                                           // constructs (registry externs) rejected at emit time
} CgcGen;
...
static void emit_ffi_call(CgcGen *g, const Expr *e) {
    ...
    // A hosted-REGISTRY extern dispatches through em_ffi + the in-tree registry (cextern.c) — libc
    // machinery a freestanding target does not have. Only DIRECT externs (OFI-167) reach bare
    // metal; reject here with the function's name rather than a late "undefined: em_ffi" link error.
    if (g->freestanding) {
        cgc_error(g, e->line,
                  "hosted extern \"c\" function '%s' is not available with --freestanding "
                  "(the registry FFI needs libc); only direct externs reach bare metal",
                  cextern_sig(idx)->name);
        return;
    }
...
static void emit_call(CgcGen *g, const Expr *e) {
    ...
    if (callee->kind == EXPR_IDENT) {
        ...
        if ((nid >= NATIVE_READ_LINE && nid <= NATIVE_EXIT) || nid == NATIVE_BYTE_SLICE ||
            ...
            // the checker also accepts these externs); the flavored runtime lib provides the arm.
            || (nid >= NATIVE_GFX_WINDOW_OPEN && nid <= NATIVE_GFX_DROPPED_FILES)
#endif
            ) {
...
int cgen_c_program(const Program *ast, const ModuleSet *modules,
    ...
    // A concurrent program runs each task on its own pthread — hosted-OS machinery a freestanding
    // target does not have (a kernel IS its own scheduler). Reject at emit time, not link time.
    if (freestanding && program_uses_concurrency(ast)) {
        fprintf(stderr, "%s: error: spawn/nursery need the hosted threaded runtime and are not "
                        "available with --freestanding\n", source_name);
        return 1;
    }
    ...
    if (freestanding) {
        // The include resolves to the freestanding runtime header supplied on the include path
        // (e.g. kernel/ember_rt.h via -Ikernel) — ONE definition of the ABI per build, no drift.
        fputs("// FREESTANDING target: bare `int main(void)` entry (no argc/argv, no stdio, no exit\n"
              "// sweep); main's int result is the process exit code, forwarded by the boot stub.\n",
              out);
    }
    ...
    if (!g.had_error && freestanding) {
        ...
        if (mainfn->ret_struct_id >= 0) {
            // main returns a value-type struct: nothing scalar to surface as an exit code.
            fprintf(out, "    em_s%d r = em_fn_%d();\n", mainfn->ret_struct_id, plan->main_index);
            fputs("    (void)r;\n", out);
            fputs("    return 0;\n", out);
        } else {
            fprintf(out, "    Value r = em_fn_%d();\n", plan->main_index);
            fputs("    return IS_INT(r) ? (int)AS_INT(r) : 0;\n", out);
        }
        fputs("}\n", out);
    } else if (!g.had_error) {

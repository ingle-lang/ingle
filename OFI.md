...
**Items open, none on the critical path** (OFI-082 shipped `.clone()` — VM complete, native arrays complete, native value-struct half a tracked follow-up; OFI-081 documented — the `_`-privacy asymmetry rule is now written down, explicit-`pub` redesign deferred; both 2026-06-20. OFI-049 FULLY closed — leak half done 2026-06-19, new sibling OFI-074 filed; OFI-066 + OFI-073 closed, OFI-071/072 substantially resolved 2026-06-19; OFI-042/046/055/056/062/063/064/065/067/069 closed 2026-06-18). The remaining open set is mostly sound leaks, perf, feature-widening, test-infra, and scheduled future work — **the silent-corruption footgun OFI-083 is now CLOSED** (value-struct `.slice()` mis-sized its copy → heap overflow; fixed both backends 2026-06-20). The open *language*-correctness item left is OFI-072's `remove_last`-as-subexpression (safely a compile error today, a VM-codegen expression-stack limitation underneath, with a `var` workaround) — a focused codegen project, not a quick fix; the rest is perf/runtime/tooling. (OFI-083, OFI-074, OFI-086 all closed 2026-06-20 — the language-correctness sweep.) **2026-06-21 — six items (OFI-100..105) filed from an external project review and ALL CLOSED toward v0.3.40 (each fixed + regression-tested + adversarially reviewed; `make test` 365/0, `make test-lsp`, `make ledger` 300/0, `make test-net` 1/0 all green). Two were on the correctness/safety line: OFI-100 (cross-function `unreachable` leak-scan bleed, HIGH) and OFI-101 (prover stack-buffer overflow, HIGH).** Quick index — full detail in the entries below.
...
| OFI-099 | The linear `Ptr` must-consume obligation is minted ONLY by a direct extern-call result, NOT by a USER function whose declared return type is `Ptr`: `fn opener() -> Ptr { return fopen(...) }  let p = opener()` (never closed) compiles CLEAN, whereas the direct `let p = fopen(...)` is correctly flagged 'opened but not closed'. A genuine OFI-049 leak-detection gap. | OPEN (**PRE-EXISTING** — found during the OFI-095 adversarial review; reproduces identically with a named binding AND with `let _ = opener()`, so OFI-095 neither introduces nor widens it). Fix: propagate the linear obligation through any function whose declared return type is `Ptr` (treat its result at the call site as an opened handle). Soundness-adjacent — a `Ptr` can currently leak through a one-line user wrapper. Worth prioritising. |
...
| OFI-096 | The NATIVE backend (AST→C) **leaks** the result of a bare expression *statement* that yields an owned temporary: `STMT_EXPR` emits `(void)(E);` and ignores the checker-set `release_temp`, so a discarded fresh string/array/struct is never `drop_value`d — whereas the VM correctly emits `OP_RELEASE`. A live VM-vs-native divergence + leak (e.g. `mk("x")` on its own line: native leaks, VM frees). | OPEN (found 2026-06-21 while mapping discard semantics for OFI-095; confirmed by emitting C — `(void)(em_fn_0(...))` with no `drop_value` vs the VM's `RELEASE` at bytecode offset 0005). NOT introduced by OFI-095 and NOT on its path (the `let _ =` discard goes through `STMT_LET`, which drops correctly). Fix: teach native `STMT_EXPR` (src/cgen_c.c:2681) to mirror the VM (codegen.c:1982) — when `release_temp` is set emit `{ Value _d = E; drop_value(&g_em, _d); }` (the inline-release idiom already at cgen_c.c:1766), guarding value-type structs (`sid>=0` → no drop). Small; deliberately NOT bundled into OFI-095 to keep that change checker-only. Native ASan is not yet wired ([[ember-asan-available]]), so this escaped earlier leak detection. |

# CLAUDE.md — Project Ingle

## Ingle coding rules
- Blank lines between functions/methods (OFI-144): **5 blank lines** in comment-sparse pure-Ingle code
  (the stdlib modules like `std/string.ig`, `std/json.ig`) where the spacing IS the separator; **2 blank
  lines** where each function already carries its own doc comment — the thin FFI/wrapper modules
  (`std/http.ig`, `std/sqlite.ig`), `src/cextern.c`, and per-method container code (`std/slotmap.ig`),
...
log for the Ingle project to track opened and closed items.
- Please do not write tests that will be used only once or twice in this Ingle project folder - write these
...
Ingle is a new, statically-typed, **brace-delimited** systems programming language (in the
...
- **The Ingle language** — what we are designing. Influenced by Rust, Zig, Go, and C, but
...
- **The Ingle compiler** — written in C. Lexer → parser → AST → type checker → codegen.
...
- **Discuss design before implementing.** Ingle is a language; a syntax or semantics choice
...
- **Test as we go.** Every language feature lands with example `.ig` programs that must
...
ingle/
  ...
  tools/           — build-time developer tools (not shipped in inglec), e.g. the editor-asset generator
  examples/        — .ig sample programs (double as tests)
  Makefile         — `make` builds the compiler; `make test` runs examples
```

## Conventions

- Ingle source files use the `.ig` extension.

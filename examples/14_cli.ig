// 14_cli.ig — a real command-line program. Ingle can now read the arguments it was
// launched with (`args()`), consult the environment (`env()`), and set an exit code
// (`exit()`) — the basics every CLI tool needs.
//
// Run it:   inglec --emit=run examples/14_cli.ig Ada Grace
//           GREETING=Hi inglec --emit=run examples/14_cli.ig Ada
//

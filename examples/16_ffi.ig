// 16_ffi.ig — binding real C through the pointer/buffer FFI (MANIFESTO §5h).
//
// An `extern "c"` block is the trust boundary: each signature names a C function and the types
// Ingle marshals across. Beyond scalars and structs-by-value, Ingle passes three pointer flavours
// — all BORROWED for the duration of the call, so Ingle keeps ownership and frees nothing C owns:
//

// 05_concurrency.ig — structured concurrency. The evolution of FROG's async()/channel model.
//
// FROG spawned free-floating goroutines and you manually collected results into arrays:
//     let worker = async(fn() { ... })
//     counts[j] = await(tasks[j])
//
// Ingle keeps real concurrency and typed channels, but scopes tasks to a `nursery` block:

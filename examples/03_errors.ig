// 03_errors.ig — errors as values, no null. THE big improvement over FROG.
//
// FROG returned (value, err) tuples and you checked `if err != null`. That's explicit (good)
// but verbose and `null` is unsafe. Ingle keeps the explicitness and adds:
//   - `Result<T, E>`  : Ok(value) or Err(error). No exceptions, no hidden control flow.
//   - `Option<T>`     : Some(value) or None. There is no `null` in Ingle, at all.

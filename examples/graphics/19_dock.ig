// examples/graphics/19_dock.ig — an INTERACTIVE docked workspace on Flare's DockTree. Three panels are
...
//
//   make graphics && build/inglec-gfx --emit=run examples/graphics/19_dock.ig
...
fn explorer_panel(mut f: flare.Flare, sel: int) -> int {
    var pick = sel
    let files = ["main.ig", "flare.ig", "ui.ig", "layout.ig", "README.md"]
    var i = 0
    loop {
...
fn output_panel(mut f: flare.Flare) {
    f.scroll_begin("out")
    f.text_muted("$ inglec --emit=run workspace.ig")
...
fn main() -> int {
    draw.window(1180, 760, "Ingle — Flare Dock")
    var f = flare.new()
    f.use_dark()
    f.set_zoom(85)

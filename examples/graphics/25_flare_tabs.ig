// 25_flare_tabs.ig — closeable, reorderable TABS on Flare (MANIFESTO §5g). A browser/editor-style tab strip:
...
//
//   make graphics && build/inglec-gfx --emit=run examples/graphics/25_flare_tabs.ig

import "std/draw" as draw
import "std/flare" as flare


// insert_at returns `arr` with `val` inserted before index `idx` (idx == len → appended). Ingle arrays have

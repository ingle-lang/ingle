// 01_hello.ig — the basics. Carries FROG's `let`/`fn`, adds static types.
//
...
fn main() {
    let name = "Ingle"          // type inferred as `string` — inference is fine for locals
...
// A plain function. `->` gives the return type. FROG had optional types here; Ingle requires
